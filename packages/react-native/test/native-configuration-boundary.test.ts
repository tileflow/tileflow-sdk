...
export const configurationFiles = [
	...
	'src/hosted-manifest-identity.ts',
...
test('configuration stays private with the exact peer matrix behind the inert public Map entry', async () => {
	...
	for (const file of ['src/index.ts', 'dist/index.d.ts']) {
		assert.doesNotMatch(await read(file),
			/MobileConfiguration|NativeConfiguration|readConfiguration|HostedNativeBindingResolver|HostedNativeStyleMismatch|snapshotHostedNativeManifest/u);
	}
	for (const name of ['configuration', 'native-configuration-bridge', 'hosted-binding', 'hosted-manifest-identity']) {
...
test('configuration and identity checks form a closed renderer-neutral import graph', async () => {
	const graph: Record<string, readonly string[]> = {
		'src/mobile-configuration.ts': [],
		'src/hosted-binding.ts': ['./mobile-configuration', './hosted-manifest-identity'],
		'src/hosted-manifest-identity.ts': ['./mobile-configuration'],
		'src/native-configuration-reader.ts': ['./mobile-configuration'],
	};
	for (const [file, allowed] of Object.entries(graph)) {
		...
		for (const statement of syntax.statements) {
			...
			assert.ok(allowed.includes(name), `${file}: ${name}`);
			assert.ok(Object.hasOwn(graph, `src/${name.slice(2)}.ts`), 'Every runtime dependency is inspected.');
		}
		assert.doesNotMatch(text,
			/\bfetch\s*\(|NativeModules|NativeEventEmitter|\.acquire\s*\(|console\.|\bimport\s*\(|\brequire\s*\(/u);
	}
});
