...
export function createNativeReadiness(notify: (status: 'loading' | 'ready' | 'error') => void) {
	...
	const publish = (next: 'loading' | 'ready' | 'error') => {
		...
		status = next;
		try { notify(next); } catch { /* Observers do not own readiness. */ }
	};
	return Object.freeze({
		get ready() { return !disposed && !failed && status === 'ready'; },
		...
		commit(token: string, layout: number): void {
			if (disposed || failed || token !== style || !Number.isSafeInteger(layout) || layout < 1) return;
			committed = layout;
		},
		invalidate(): void {
		...
		native(event: NativeReadinessEvidence): void {
			if (disposed || failed || event.style !== style ||
				!Number.isSafeInteger(event.sequence) || event.sequence <= sequence ||
				!Number.isSafeInteger(event.layout) || event.layout < 1) return;
