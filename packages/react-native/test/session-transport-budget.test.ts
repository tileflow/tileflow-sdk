...
import {apiOrigin, credential, createClock, createFetchQueue, createIds, deferred, mapId, response, success} from './session-fixture';

function fixture() {
	const clock = createClock();
	const delivered = deferred<ReturnType<typeof response>>();
	const queue = createFetchQueue([() => delivered.promise]);
	const binding = {kind: 'hosted' as const, apiOrigin, credential, mapId, surfaceId: 'store-locator'};
	const controller = createHostedNativeSessionController({binding, fetch: queue.fetch, now: clock.now, sessionIdFactory: createIds()});
