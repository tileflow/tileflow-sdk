...
import {directSourceFixture, hostedSourceFixture, type ReadyHostedSource} from './hosted-source-fixture';
...
test('a reflected rejection cannot publish an error after replacing its own resolution', async () => {
	...
	const resolver = createHostedNativeBindingResolver(async () => { throw error; });
	try {
		await assert.rejects(resolver.replace(hostedSourceFixture()), {code: 'NATIVE_CONFIGURATION_REPLACED'});
...
test('Core API precedence does not authorize contradictory Hosted origin declarations', async () => {
	...
	const state = await resolveFixture(input);
	assert.equal(state.map.apiUrl, apiOrigin, 'Core still preserves the existing map-level API precedence.');
	let configurations = 0;
	const resolver = createHostedNativeBindingResolver(async () => {
		configurations++;
		return snapshotMobileConfiguration({apiOrigin, credential});
