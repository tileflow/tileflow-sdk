...
export async function prepareMobileTemplates(context) {
  const expo = await packMobileTemplate(
    context,
    'expo-template-bare-minimum',
    mobileVersions.expoTemplate,
    'expo',
  );
  const bare = await packMobileTemplate(
    context,
    '@react-native-community/template',
    mobileVersions.communityTemplate,
    'bare',
  );
  assertTemplateManifest(
    'expo',
    await readMobileJson(context.root, join(expo.directory, 'package.json')),
  );
  const bareMetadata = await readMobileJson(context.root, join(bare.directory, 'package.json'));
  requireMobile(
    bareMetadata.name === bare.name && bareMetadata.version === bare.version,
    'TEMPLATE_INVALID',
    'templates',
  );
  assertTemplateManifest(
    'bare',
    await readMobileJson(context.root, join(bare.directory, 'template', 'package.json')),
  );
  const tools = join(context.root, 'tools');
  await mkdir(tools);
  await writeMobileJson(context.root, join(tools, 'package.json'), {
    name: 'tileflow-mobile-smoke-tools',
    private: true,
    version: '0.0.0',
    dependencies: {'@react-native-community/cli': mobileVersions.generatorCli},
  });
  await context.run('npm', ['install', '--ignore-scripts', '--no-audit', '--no-fund'], {
    cwd: tools,
    step: 'templates',
  });
  ...
  requireMobile(
    installed.version === mobileVersions.generatorCli &&
      typeof installed.bin?.['rnc-cli'] === 'string',
    'INSTALL_INVALID',
    'templates',
  );
  ...
  return {
    ...
    bare,
    generator,
    toolsLock: await mobileFileIdentity(
      context.root,
      join(tools, 'package-lock.json'),
...
export async function createMobileConsumer(context, kind, packages, templates) {
  ...
  if (kind === 'expo') {
    ...
    await context.run(
      process.execPath,
      [templates.generator, ...bareGeneratorArguments(templates.bare.path, directory)],
      {step: 'bare'},
    );
  }
  ...
  await writeMobileFile(
    context.root,
    podfile,
    addIosPostInstallHooks((await readMobileFile(context.root, podfile)).toString()),
  );
  ...
  const properties = (
    await readMobileFile(context.root, join(directory, 'android', 'gradle.properties'))
  ).toString();
  ...
  await context.run(
    ...
    {cwd: directory, step: kind},
  );

  const installedRoot = await realpath(
    join(directory, 'node_modules', '@tileflow', 'react-native'),
  );
  if (kind === 'expo') {
    for (const platform of ['android', 'ios']) {
      const document = parseCommandJson(
        await context.run(process.execPath, expoAutolinkingArguments(platform), {
          cwd: directory,
          step: kind,
    ...
    requireMobile(
      metadata.version === mobileVersions.consumerCli && metadata.bin?.['rnc-cli'],
      'INSTALL_INVALID',
      kind,
    );
    ...
    const document = parseCommandJson(
      await context.run(process.execPath, [entry, 'config'], {
        cwd: directory,
        step: kind,
        maxOutputBytes: 8 * 1024 * 1024,
      }),
  ...
  return {
    ...
    receipt: {
      ...
      typeScript: 'compiled',
      autolinking: {
        provider: kind === 'expo' ? 'expo' : 'community-cli',
        android: 'discovered',
        ios: 'discovered',
      },
      lock: await mobileFileIdentity(
        context.root,
      ...
      directVersions: {
        ...
        ...Object.fromEntries(packages.tarballs.map(({name, version}) => [name, version])),
      },
    },
  };
}
...
async function assertInstalledMobilePackages(context, directory, manifest, tarballs, kind) {
  for (const [name, expected] of Object.entries({
    ...
    requireMobile(
      installed.name === name &&
        installed.version === (expected.startsWith('file:') ? mobileVersions.package : expected),
      'INSTALL_INVALID',
      kind,
    );
  }
  for (const tarball of tarballs) {
    ...
    for (const file of tarball.files) {
      const expected = await readMobileFile(
        context.root,
        join(tarball.directory, file),
        128 * 1024 * 1024,
      );
...
process.stdout.write(JSON.stringify([...direct, fromNative.resolve(names[0]), fromNative.resolve(names[1])]));`;
  const resolved = parseCommandJson(
    await context.run(process.execPath, ['-e', resolver], {cwd: directory, step: kind}),
  );
  const expected = mobilePackages.map((name) =>
    join(directory, 'node_modules', '@tileflow', name, 'dist', 'index.js'),
  );
  requireMobile(
    JSON.stringify(resolved) === JSON.stringify([...expected, expected[0], expected[1]]),
    'INSTALL_INVALID',
    kind,
  );
}
