...
test('decoded artifact bytes are verified when a CDN applies HTTP content encoding', async () => {
  await withIconSetFixture(async (cacheRoot) => {
    ...
    const fetch: typeof globalThis.fetch = async (url, options) => {
      ...
      const file = artifact.files.find((candidate) =>
        String(url).endsWith(`/${candidate.fileName}`),
      )!;
    ...
    assert.equal(
      (await loadTileflowIconSetArtifact(pin, {cacheRoot, fetch})).package.contentHash,
...
test('HTTP encoding never bypasses exact decoded-body bounds', async () => {
  await withIconSetFixture(async (cacheRoot) => {
    ...
    const fetch: typeof globalThis.fetch = async () =>
      new Response(new Uint8Array(artifact.files[0]!.source.byteLength + 1), {
        headers: {
    ...
    await assert.rejects(loadTileflowIconSetArtifact(pin, {cacheRoot, fetch}), {
