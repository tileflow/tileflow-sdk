...
import type {NativeAdmissionNativeModule, NativeBootstrapReply} from '../src/native-bootstrap-contract';
...
class ModuleDouble extends AdmissionBridgeDouble implements NativeAdmissionNativeModule {
	...
	async bootstrap(_installation: string, _context: string, request: string, _url: string, _credential: string, _body: string) {
		const pending = deferred<NativeBootstrapReply>(); this.waiting.set(request, pending); return pending.promise;
	}
	async cancelBootstrap(_installation: string, context: string, request: string): Promise<{cancelled: true}> {
		this.cancellations.push({context, request}); return {cancelled: true};
	}
	override async retireContext(installation: string, context: string): Promise<{retired: true}> {
		await this.retireGate; return super.retireContext(installation, context);
	}
}
function fixture() {
	const native = new ModuleDouble();
	const wire = createNativeAdmissionWire(native, (listener) => native.subscribe(listener));
	const init: HostedNativeSessionFetchInit = {body: JSON.stringify({mapId, sessionId: 'ses_1', surfaceId: 'default'}),
		method: 'POST', credentials: 'omit', headers: {'X-Tileflow-Mobile-Client': credential, 'Content-Type': 'application/json'},
...
test('a reader delivered before removal cannot expose bytes after installation retirement', async () => {
	const {native, wire, init} = fixture(); await wire.bridge.install();
...
test('context retirement invalidates pending callbacks immediately but acknowledges only native completion', async () => {
	...
	const retired = deferred<void>(); native.retireGate = retired.promise;
	let acknowledged = false;
	const retirement = wire.bridge.retireContext(native.installation, 'installation_1.1').then(() => { acknowledged = true; });
	await rejected;
	...
	retired.resolve(); await retirement;
	await assert.rejects(first(`${apiOrigin}/v1/sessions/start`, init), {code: 'NATIVE_ADMISSION_CANCELLED'});
	await wire.bridge.remove(native.installation);
});
...
test('bridge bootstrap calls are bounded even before the native module can process them', async () => {
	...
	const rejections = waiting.map((pending) => assert.rejects(pending, {code: 'NATIVE_ADMISSION_CANCELLED'}));
	await assert.rejects(fetch(`${apiOrigin}/v1/sessions/start`, init), {code: 'NATIVE_ADMISSION_UNAVAILABLE'});
	assert.equal(native.waiting.size, 32);
	await wire.bridge.remove(native.installation); await Promise.all(rejections);
