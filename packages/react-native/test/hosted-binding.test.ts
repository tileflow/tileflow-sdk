...
function source(metadata: Record<string, unknown> = {}) {
	const result = Object.hasOwn(metadata, 'usageMode') && metadata.usageMode === undefined
		? directSourceFixture()
		: hostedSourceFixture({
			...(typeof metadata.mapId === 'string' ? {mapId: metadata.mapId} : {}),
			...(typeof metadata.apiUrl === 'string' ? {apiOrigin: metadata.apiUrl} : {}),
		});
	Object.assign(result.map, metadata);
	Object.assign(result.manifest.maps.streets!, metadata);
	return result;
}
function gate<T>() {
...
test('non-session delivery never locates configuration or creates commercial identity', async () => {
	...
	let identities = 0;
	let requests = 0;
	const resolver = createHostedNativeBindingResolver(() => { reads++; throw new Error('Unexpected configuration.'); });
...
test('requires declared session metadata and exact origin agreement', async () => {
	let reads = 0;
	const resolver = createHostedNativeBindingResolver(async () => { reads++; return config(); });
	for (const metadata of [
		{mapId: undefined}, {mapId: 'streets'}, {mapId: 'map_short'}, {apiUrl: undefined},
		{apiUrl: `${origin}/api`}, {apiUrl: `${origin}?`}, {usageMode: 'request'},
...
test('replacement snapshots metadata and retires stale success or failure immediately', async () => {
	for (const rejects of [false, true]) {
		...
		const resolver = createHostedNativeBindingResolver(() => { entered.resolve(); return response.promise; });
...
test('shared application data never shares controllers or session identities', async () => {
	...
	const controllers = bindings.map((binding) => createHostedNativeSessionController({
		...
		now: () => new Date(0), sessionIdFactory: () => `ses_isolated_${++sequence}`,
	}));
