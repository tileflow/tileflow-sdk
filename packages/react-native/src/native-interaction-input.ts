...
export function nativeInteractionDiagnostic(
...
export function snapshotNativeInteractionJson(input: unknown): TileflowInteractionJsonValue {
  ...
  const copy = (value: unknown, depth: number): TileflowInteractionJsonValue => {
    if (
      ++nodes > tileflowInteractionLimits.maxDocumentNodes ||
      depth > tileflowInteractionLimits.maxDocumentDepth
    )
      throw new Error();
    if (value === null || typeof value === 'boolean') return value;
    if (typeof value === 'number' && Number.isFinite(value)) return value;
    if (typeof value === 'string' && value.length <= tileflowInteractionLimits.maxDocumentBytes)
      return value;
    ...
    if (array) {
      ...
      if (
        ...
        length < 0 ||
        length > tileflowInteractionLimits.maxDocumentProperties - properties
      )
        throw new Error();
...
export function nativeInteractionValuesEqual(left: unknown, right: unknown): boolean {
  ...
  if (
    array &&
    Object.getOwnPropertyDescriptor(left, 'length')?.value !==
      Object.getOwnPropertyDescriptor(right, 'length')?.value
  )
    return false;
...
export function prepareNativeInteractionInputs(
  ...
  const report = (code: TileflowInteractionDiagnosticCode) => {
    if (!diagnostics.some((value) => value.code === code))
      diagnostics.push(nativeInteractionDiagnostic(code));
  };
  try {
    const raw = nativeInteractionField(input, 'annotations');
    let result = validateTileflowAnnotations(raw === undefined ? [] : raw);
    if (result.ok)
      result = validateTileflowAnnotations(snapshotNativeInteractionJson(result.annotations));
    if (result.ok) {
      if (!nativeInteractionValuesEqual(annotations, result.annotations))
        annotations = freezeNativeInteractionValue(result.annotations);
  ...
  try {
    ...
    if (result.ok)
      result = validateTileflowInteractionBindings(snapshotNativeInteractionJson(result.bindings));
    if (result.ok) {
      if (!nativeInteractionValuesEqual(bindings, result.bindings))
        bindings = freezeNativeInteractionValue(result.bindings);
    } else for (const diagnostic of result.diagnostics) report(diagnostic.code);
  } catch {
    report('INVALID_DOCUMENT');
  }
...
export function planNativeAnnotations(
  ...
  return Object.freeze({
    ...
    remove: Object.freeze(
      previous.filter((annotation) => !after.has(annotation.id)).map((annotation) => annotation.id),
    ),
