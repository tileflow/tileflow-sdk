...
const marker = (id = 'one') => ({
	id, kind: 'marker', ariaLabel: 'Place', coordinate: [1, 2],
 ...
 test('uncontrolled touch activation commits one popup and explicit close uses the portable reducer', () => {
	...
	const owner = createNativeInteractionOwner({annotations: [marker()]}, {
		onInteractionStateChange: (state) => states.push(state), onInteractionEvent: (event) => events.push(event),
	});
	...
	assert.deepEqual(events.map((event) => event.type), ['target:activate', 'popup:open']);
...
test('controlled requests do not commit before supplied state, and callback replacement is inert', () => {
	...
	const owner = createNativeInteractionOwner({annotations, interactionState: closed}, {
...
test('invalid documents and ownership switches retain committed values without partial host changes', () => {
	...
	assert.ok(owner.getSnapshot().diagnostics.some((value) => value.code === 'DUPLICATE_ANNOTATION_ID'));
	owner.dispose();
});
...
test('annotation and active binding removal close stale popup state, including a controlled pending close', () => {
	const requests: TileflowInteractionState[] = [];
	const annotations = [marker()];
	const owner = createNativeInteractionOwner({annotations, defaultInteractionState: {popup: {kind: 'annotation', id: 'one'}}}, {
		onInteractionStateChange: (state) => requests.push(state),
	});
	assert.ok(owner.getSnapshot().popup);
	...
	const binding = {id: 'annotation-details', target: {kind: 'annotation', id: 'one'}, popup: {content: {kind: 'text', text: 'Bound'}}};
	const open: TileflowInteractionState = {popup: {kind: 'annotation', id: 'one'}};
	const controlled = createNativeInteractionOwner({annotations, interactions: [binding], interactionState: open}, {
		onInteractionStateChange: (state) => requests.push(state),
	});
...
test('semantic touch emits bounded targets and requires stable identity only for durable popup requests', async () => {
	...
	const owner = createNativeInteractionOwner({interactions: [poiBinding]}, {onInteractionEvent: (event) => events.push(event)});
	...
	await owner.activateTouch(touch);
	assert.deepEqual(owner.getSnapshot().state, {popup: {kind: 'semantic-feature', domain: 'poi', featureId: 0}});
	assert.equal(owner.getSnapshot().popup?.target.kind, 'semantic-feature');
	assert.equal(events.at(-1)?.type, 'popup:open');
	owner.update({interactions: [{...poiBinding, target: {kind: 'semantic-feature', domain: 'poi', categories: ['lodging']}}]});
	assert.deepEqual(owner.getSnapshot().state, {popup: null});
	assert.equal(owner.getSnapshot().diagnostics.at(-1)?.code, 'STALE_TARGET');
	owner.dispose();
});
...
test('style replacement invalidates open semantic targets and never reuses pending native results', async () => {
	const first = queryFixture(); first.setFeatures([poiFeature(1)]);
...
test('two owners and disposal isolate state, query generations, callbacks and annotation plans', async () => {
	const native = queryFixture(); const secondNative = queryFixture();
	const late = barrier<unknown>();
	...
	const second = createNativeInteractionOwner({annotations: [marker()], interactions: [poiBinding]});
	first.subscribe(() => { notifications++; });
	first.replaceStyle(native.lease); second.replaceStyle(secondNative.lease);
	...
	assert.deepEqual(second.getSnapshot().state, {popup: {kind: 'semantic-feature', domain: 'poi', featureId: 'two'}});
	second.dispose();
});

test('observer failures do not own lifetime and observer reentrancy cannot open a removed target', () => {
	const owner = createNativeInteractionOwner({annotations: [marker()]}, {
		onInteractionEvent() { throw new Error('private'); },
		onInteractionStateChange() { return Promise.reject(new Error('private')); },
	});
	owner.subscribe(() => { throw new Error('private'); });
	owner.activateAnnotation('one', 'touch');
	assert.ok(owner.getSnapshot().popup);
	assert.doesNotMatch(JSON.stringify(owner.getSnapshot().diagnostics), /private/u);
	owner.close();
	owner.setCallbacks({onInteractionEvent(event) {
