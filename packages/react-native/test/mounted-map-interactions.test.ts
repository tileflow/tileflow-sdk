...
import type {TileflowAnnotation, TileflowInteractionEvent, TileflowInteractionState} from '@tileflow/interactions';
...
const annotation: TileflowAnnotation = {
	id: 'place', kind: 'marker', coordinate: [1, 2], ariaLabel: 'Place', data: {capacity: 12},
...
test('one marker activation survives both native bubbling orders and suppresses semantic activation', async () => {
	let queries = 0;
	const f = fixture(async () => { queries++; return []; });
	const events: TileflowInteractionEvent[] = [];
	f.owner.update({annotations: [annotation], interactions: [poiBinding], onInteractionEvent: (event) => events.push(event)});
...
test('callbacks update without replacing ownership; retained IDs keep host plans and removed callbacks are inert', () => {
	const f = fixture();
	const first: unknown[] = [], second: TileflowInteractionEvent[] = [];
...
test('controlled state is initialized from the first props and requests rather than owns selection', () => {
	...
	const props = {annotations: [annotation], interactionState: {popup: null},
...
test('invalid replacements retain detached last-valid data and bounded diagnostics', () => {
	...
	f.owner.update({annotations: [annotation], onInteractionDiagnostic: (value) => diagnostics.push(value)});
	const valid = f.owner.getSnapshot()!.annotations;
	f.owner.update({annotations: [annotation, annotation], onInteractionDiagnostic: (value) => diagnostics.push(value)});
...
test('two maps have isolated state, callbacks, touch claims and retirement', async () => {
	...
	right.owner.update({annotations: [annotation], onInteractionEvent: (event) => events.push(event)});
...
test('observer failures never escape the mounted interaction boundary', async () => {
	const f = fixture();
	f.owner.update({annotations: [annotation], onInteractionEvent() { throw new Error('private'); },
		onInteractionStateChange: async () => { throw new Error('private'); }});
	f.owner.beginTouch();
