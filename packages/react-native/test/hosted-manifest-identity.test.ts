  ...
  for (const candidate of [
    {
      ...
      manifest: {
        version: 1 as const,
        maps: {...original.manifest.maps, other: original.manifest.maps.streets!},
      },
    },
    {...original, theme: {...original.theme, revision: undefined}},
    {
      ...original,
      theme: {...original.theme, styleUrl: `${apiOrigin}/maps/${mapId}/native/v8/light.json`},
    },
...
test('stale metadata cannot be composed with another protected style or deployment', () => {
  for (const patch of [
    ...
    assert.throws(() =>
      assertHostedNativeStyleDocument(source(), {
        ...value,
        value: {...value.value, metadata: {...value.value.metadata, ...patch}},
      }),
    );
  }
  assert.throws(() =>
    assertHostedNativeStyleDocument(source(), {
      ...document(),
      url: `${apiOrigin}/maps/${mapId}/native/v8/light.json`,
    }),
  );
});
...
test('non-session metadata does not inspect configuration or incidental delivery fields', async () => {
  ...
  Object.defineProperty(direct.map, 'apiUrl', {
    get() {
