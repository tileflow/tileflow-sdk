...
test('camera is packaged privately while root, peers, exports and existing adapters stay unchanged', async () => {
	...
	assert.doesNotMatch(code, /react-native|maplibre|setTimeout|setInterval|requestAnimationFrame|createTileflowNativeSourceController/u);
	await readFile(new URL('dist/internal/camera.d.ts', root), 'utf8');
	...
	assert.doesNotMatch(declarations, /declare (?:function|const|class) Map\b|createMapCameraController|CameraCommand|CameraToken/u);
	for (const entry of ['native', 'index', 'browser', 'native-profile']) {
		const core = await readFile(new URL(`../core/dist/${entry}.js`, root), 'utf8');
		assert.doesNotMatch(core, /createMapCameraController|CAMERA_MODE_CHANGE|restoreAfterStyleChange/u);
	}
