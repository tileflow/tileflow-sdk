import {
  tileflowIconSpriteIndexSchema as indexSchema,
...
export type VerifiedTileflowIconArtifact = {
  ...
  icons: TileflowRenderedIcon[];
};

/** Verify exact compressed bytes and independently decode/hash every cell at both densities. */
export async function verifyTileflowIconArtifact(
  input: CompiledTileflowIconPackage,
  ...
  try {
    ...
    if (input.files.length !== 4 || new Set(input.files.map((file) => file.fileName)).size !== 4)
      throw new Error('An artifact must contain exactly the four generated files');
    const files = manifest.files.map((expected) => {
      ...
      if (
        ...
        source.source.byteLength !== expected.byteLength
      )
        throw new Error('Generated file identity does not match the manifest');
      return {
    ...
    if ((await hashTileflowIconPackageManifest(manifest)) !== contentHash)
      throw new Error('Artifact manifest hash mismatch');
    for (const [index, file] of files.entries()) {
      if ((await sha256Hex(file.source)) !== manifest.files[index]!.sha256)
        throw new Error(`Generated ${file.fileName} checksum mismatch`);
    }
    const decoder = new TextDecoder('utf-8', {fatal: true});
    const oneIndex = indexSchema.parse(
      parseTileflowIconJson(
        decoder.decode(files[0]!.source),
        tileflowIconPackageLimits.maxGeneratedFileBytes,
      ),
    );
    const twoIndex = indexSchema.parse(
      parseTileflowIconJson(
        decoder.decode(files[2]!.source),
        tileflowIconPackageLimits.maxGeneratedFileBytes,
      ),
    );
    for (const index of [oneIndex, twoIndex]) {
      ...
      if (
        names.length !== manifest.iconNames.length ||
        names.some((name, ordinal) => name !== manifest.iconNames[ordinal])
      )
        throw new Error('Sprite index does not exactly match the manifest icon names');
    }
    for (const [index, dimensions, ratio] of [
      [oneIndex, manifest.sprites.oneX, 1],
      [twoIndex, manifest.sprites.twoX, 2],
    ] as const) {
      const rectangles = Object.values(index);
      for (const [ordinal, rect] of rectangles.entries()) {
        if (
          rect.pixelRatio !== ratio ||
          rect.x + rect.width > dimensions.width ||
          rect.y + rect.height > dimensions.height
        )
          throw new Error('Sprite rectangle is outside the atlas or has the wrong density');
        for (const other of rectangles.slice(0, ordinal)) {
          if (
            ...
            throw new Error('Sprite rectangles must not overlap');
        }
      }
    }
    for (const name of manifest.iconNames) {
      ...
      if (
        ...
        two.height !== one.height * 2
      )
        throw new Error('Sprite densities must have exactly doubled geometry');
    }
    ...
    for (const [ordinal, id] of manifest.iconNames.entries()) {
      ...
      if (
        (await hashTileflowRenderedIconPixels({...oneX, pixelRatio: 1})) !== expected.oneX ||
    ...
    throw new TileflowIconSetError(
      ...
      'Generated icon artifact verification failed',
      {cause},
    );
  }
}
...
async function decodeAtlas(
...
): Promise<TileflowRenderedIconCell> {
  if ([137, 80, 78, 71, 13, 10, 26, 10].some((byte, index) => bytes[index] !== byte))
    throw new Error('Expected PNG bytes');
  const sharp = await loadSharp();
  const image = sharp(bytes, {
    failOn: 'error',
    limitInputPixels: tileflowIconPackageLimits.maxAtlasDimension ** 2,
  });
  ...
  if (
    ...
    throw new Error('PNG geometry does not match the manifest');
  const {data, info} = await image.ensureAlpha().raw().toBuffer({resolveWithObject: true});
  if (
    info.channels !== 4 ||
    info.width !== dimensions.width ||
    info.height !== dimensions.height ||
    data.length !== dimensions.width * dimensions.height * 4
  )
    throw new Error('Expected bounded RGBA atlas pixels');
