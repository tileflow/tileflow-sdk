...
function provenanceFixture() {
        ...
        layers: [
          {
            ...
            category: 'food-drink' satisfies TileflowPoiCategory,
            layerId: 'poi',
            priority: 2,
  ...
  style.metadata = {
    ...style.metadata,
    'tileflow:interaction-manifest': semantic,
...
test('native preparation preserves its web input and authored identity', async () => {
  ...
  assert.equal(
    result.renderers.native.buildManifest.maps.main!.mapRevisionSha256,
...
test('the serialized renderer collection contains finalized overlay and semantic provenance', async () => {
  ...
  assert.deepEqual(metadata['tileflow:interaction-manifest'], {
    ...semantic,
    domains: {
      poi: {...semantic.domains.poi, layers: [{...semantic.domains.poi.layers[0]!, priority}]},
    },
  });
  ...
  assert.equal(
    result.renderers.native.buildManifest.maps.main!.themes.light!.styleSha256,
    digest(native),
  );
  ...
  assert.equal(serializeCanonicalJson(input), before);
  assert.deepEqual(
    await parseTileflowRendererDeploymentArtifact(JSON.parse(serializeCanonicalJson(result))),
    result,
  );
});
...
test('fresh hashes cannot authorize stale priorities or a different POI category', async () => {
  ...
  for (const patch of [{priority: 0}, {category: 'retail'}, {category: 'food'}]) {
    ...
    const semantic = metadata['tileflow:interaction-manifest'] as ReturnType<
      typeof provenanceFixture
    >['semantic'];
    ...
    await assert.rejects(parseTileflowRendererDeploymentArtifact(candidate), {
      code: 'RENDERER_DEPLOYMENT_INVALID',
    });
  }
});
