...
test('loads the strict manifest and resolves all styles from the final approved owner', async () => {
  ...
  assert.equal(
    result.manifest.maps.streets?.themes.light?.styleUrl,
...
test('keeps declared identity and resolves font resources from their owning style', async () => {
  ...
  Object.assign(input.maps.streets, {
    mapId: 'map_123',
    usageMode: 'session',
    worldGeneration: 'v1',
  });
  Object.assign(input.maps.streets.themes.light, {
    styleId: 'style_123',
    fontFaces: [{family: 'Fixture', source: '../../fonts/fixture.ttf'}],
  });
  ...
  assert.equal(
    result.manifest.maps.streets?.themes.light?.fontFaces?.[0]?.source,
...
for (const invalid of [
  ...
  test('rejects noncanonical manifest input without exposing schema details', async () => {
    ...
    await assert.rejects(
      loadTileflowNativeManifest(manifestUrl, options(t)),
      isError('NATIVE_MANIFEST_INVALID'),
    );
...
for (const [name, body, code] of [
  ...
  [
    'deep JSON',
    new TextEncoder().encode('['.repeat(65) + '0' + ']'.repeat(65)),
...
test('counts actual body bytes with absent or false Content-Length', async () => {
  ...
  for (const contentLength of [undefined, '0', '3']) {
    ...
    await assert.rejects(
      loadTileflowNativeManifest(manifestUrl, options(t)),
      isError('NATIVE_MANIFEST_TOO_LARGE'),
    );
    assert.equal(t.readerCancels, 1);
    assert.equal(t.operationCancels, 1);
...
test('accepts exactly the byte ceiling and handles split multibyte characters and a leading BOM', async () => {
  ...
  assert.equal(
    (await loadTileflowNativeManifest(manifestUrl, options(split))).manifest.maps.streets?.themes
      .light?.revision,
    'A🌍é漢',
  );
});
...
for (const [status, code] of [
  [401, 'NATIVE_MANIFEST_ACCESS_DENIED'],
  [403, 'NATIVE_MANIFEST_ACCESS_DENIED'],
  [404, 'NATIVE_MANIFEST_NOT_FOUND'],
  [500, 'NATIVE_MANIFEST_REQUEST_FAILED'],
] as const) {
...
test('requires a safe explicit final URL and never falls back to the requested owner', async () => {
  for (const finalUrl of [
    '',
    './other.json',
    'https://user:secret@cdn.example.test/m.json',
    'http://cdn.example.test/m.json',
  ]) {
    const t = transport(bytes(), finalUrl);
    await assert.rejects(
      loadTileflowNativeManifest(manifestUrl, options(t)),
      isError('NATIVE_MANIFEST_URL_INVALID'),
    );
    assert.equal(t.reads.length, 0);
    assert.equal(t.readerCancels, 1);
  }
...
test('honors an exact development origin with the accepted URL provider', async () => {
  for (const origin of ['http://127.0.0.1:8765', 'http://10.0.2.2:8765']) {
    ...
    const result = await loadTileflowNativeManifest(url, {
      ...options(t),
      developmentOrigin: origin,
    });
    assert.equal(
      result.manifest.maps.streets?.themes.light?.styleUrl,
      `${origin}/generations/r/styles/streets/light.json`,
    );
  }
});
...
test('pre-abort does not acquire and does not read AbortSignal.reason', async () => {
  ...
  const t = transport();
  await assert.rejects(
    loadTileflowNativeManifest(manifestUrl, {...options(t), signal: controller.signal}),
...
test('abort of a hanging read cancels both handles without awaiting their cleanup', async () => {
  ...
  t.response.reader.read = () => {
    reading.resolve();
    return ignored.promise;
  };
  const controller = new AbortController();
  const pending = loadTileflowNativeManifest(manifestUrl, {
    ...options(t),
...
test('sanitizes acquisition/read/cleanup exceptions and malformed chunks', async () => {
  ...
  for (const chunk of [new Uint8Array(), 'text', new Uint8Array(65_537)]) {
    ...
    t.response.reader.cancel = () => {
      throw new Error(secret);
    };
    await assert.rejects(
      loadTileflowNativeManifest(manifestUrl, options(t)),
      isError('NATIVE_MANIFEST_RESPONSE_INVALID'),
    );
    assert.equal(t.operationCancels, 1);
  }
});
...
test('bounds resolved URL expansion as well as the original body', async () => {
  const maps = Object.fromEntries(
    Array.from({length: 800}, (_, index) => [
      `map-${index}`,
      {defaultTheme: 'light', themes: {light: {colorScheme: 'light', styleUrl: './style.json'}}},
    ]),
  );
...
test('an oversized chunk is rejected before retaining it and cleanup may never settle', async () => {
  const t = transport();
  t.response.reader.read = async () => ({
    done: false,
    value: new Uint8Array(tileflowNativeManifestLimits.maximumBytes + 1),
  });
...
test('success and abort detach the external signal listener without reading its reason', async () => {
  ...
  const signal = {
    ...
    removeEventListener(_name: string, listener: EventListenerOrEventListenerObject | null) {
      if (listener && callbacks.delete(listener)) listening--;
    },
  };
  const t = transport();
