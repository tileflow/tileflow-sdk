...
test('success, partial installs and failed generators all remove their exact owned root', async (t) => {
  ...
  for (const failure of [false, true]) {
    let used;
    const work = withMobileWorkspace(
      async (root) => {
        used = root;
...
test('preservation is deliberate, and replaced owner roots are never deleted', async (t) => {
  const parent = await temporaryParent(t);
  let preserved;
  await withMobileWorkspace(async (root) => writeFile(join(root, 'evidence'), 'log'), {
    parent,
    keep: true,
    onPreserved: (root) => {
      preserved = root;
...
test('symlink/path escapes fail and cleanup unlinks an inner symlink without following it', async (t) => {
  ...
  await assert.rejects(assertOwnedPath(owner.root, join(owner.root, 'escape', 'new-file')), {
    code: 'WORKSPACE_UNSAFE',
  });
  await assert.rejects(assertOwnedPath(owner.root, join(owner.root, '..', 'unrelated')), {
    code: 'WORKSPACE_UNSAFE',
  });
  await owner.cleanup();
  assert.equal(await readFile(join(outside, 'retain'), 'utf8'), 'safe');
...
test('system aliases resolving into the owned workspace use their physical path', async (t) => {
  const parent = await temporaryParent(t);
  const owner = await createMobileWorkspace(parent);
  const evidence = join(owner.root, 'evidence');
  const alias = join(parent, 'owned-alias');
  await writeFile(evidence, 'safe');
  await symlink(owner.root, alias, 'dir');
  assert.equal(await assertOwnedPath(owner.root, join(alias, 'evidence')), evidence);
  await owner.cleanup();
});
    ...
    async (root) => {
      ...
      const context = await createMobileContext(root, {
        environment: {
          ...
          npm_config_userconfig: '/unrelated/.npmrc',
          GRADLE_USER_HOME: '/unrelated/gradle',
        },
      ...
      assert.equal(captured.env.LANG, 'en_US.UTF-8');
      assert.equal(captured.env.LC_ALL, 'en_US.UTF-8');
...
test('command completion waits for close, capturing the final stdout bytes without a shell', async () => {
  ...
  let options;
  let settled = false;
  const result = runMobileCommand('fake', ['literal;not-a-shell'], {
    spawnChild: (_command, _args, settings) => {
      options = settings;
      return child;
    },
    killGroup: () => {},
