...
async function consumerFixture(root, kind, defect) {
	...
	await writeMobileFile(root, join(templateDirectory, 'App.js'), 'export default function App() {}');
	await writeMobileFile(root, join(templateDirectory, 'android', 'gradlew'), '# Fixture, never executed.');
	await writeMobileFile(root, join(templateDirectory, 'android', 'gradle.properties'), 'hermesEnabled=true\n');
	...
	if (defect === 'ios-discovery')
		autolinking.dependencies['@tileflow/react-native'].platforms.ios.podspecPath =
			join(root, 'unrelated', 'TileflowNativeAdmission.podspec');
				...
				for (const [name, version] of Object.entries({
					...
					if (packed) {
						...
						if (name === '@react-native-community/cli')
							await writeMobileFile(root, join(packageRoot, 'build', 'bin.js'), '// Never executed.\n');
					}
				}
...
for (const kind of ['expo', 'bare']) {
	test(`${kind} consumer composes packed public usage and ordinary autolinking with injected commands`, async () => {
		await withMobileWorkspace(async (root) => {
			...
			if (kind === 'expo') await assert.rejects(access(join(f.directory, 'App.js')), {code: 'ENOENT'});
		});
	});
	for (const defect of ['android-discovery', 'ios-discovery', 'installed-bytes']) {
		test(`${kind} rejects ${defect} instead of recording consumer success`, async () => {
			let ownedRoot;
			await assert.rejects(withMobileWorkspace(async (root) => {
				ownedRoot = root;
