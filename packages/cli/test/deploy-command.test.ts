...
test('plain validation now reports missing local icon sources', async (t) => {
  ...
  await writeFile(
    fixture.configPath,
    `export default {maps: {madrid: {icons: {source: './missing-icons'}}}};\n`,
  );
  const result = await runCli(
    fixture.directory,
...
test('validation rejects invalid MapLibre semantics with a stable layer path', async (t) => {
  ...
  await writeFile(
    fixture.configPath,
    `export default {maps: {madrid: {layers: {background: {paint: {'background-color': 42}}}}}};\n`,
  );
  const result = await runCli(
    fixture.directory,
    ['validate', '--config', 'tileflow.config.ts'],
  ...
  assert.equal(result.code, 1);
  assert.match(result.stdout, /maps\.madrid\.style\.layers\.background\.paint\.background-color/);
...
test('deploy rejects invalid MapLibre semantics before any remote write', async (t) => {
  ...
  maps: {
    madrid: {
      icons: 'brand',
      layers: {background: {paint: {'background-color': 42}}}
    }
  }
};\n`,
  );
...
test('deploy uploads generated icon files before posting sanitized style JSON', async (t) => {
  ...
  const api = await createFakeApi(t, async (request) => {
    ...
    assert.deepEqual((parsed.config as {icons?: unknown}).icons, {
    ...
    assert.doesNotMatch(JSON.stringify(parsed.config), /\.\/icons|source-secret|"source"\s*:/);
    return {
      changed: true,
      mapId: 'map_test',
      mapUrl: 'https://api.example.test/maps/map_test/style.json',
...
async function createFixture(t: TestContext) {
  ...
  await writeFile(configPath, `export default {maps: {madrid: {name: 'Madrid'}}};\n`, 'utf8');
...
async function createIconFixture(t: TestContext) {
  ...
  await writeFile(
    ...
    `export default {
  icons: {brand: {mapping: {restaurant: 'cafe'}, source: './icons'}},
  maps: {madrid: {icons: 'brand', name: 'Madrid'}}
};\n`,
  );
  return fixture;
}
