...
function fixture() {
	...
	const module: NativeSurfaceModule = {
		async attachSurface() { return {surface: `surface_${++sequence}`}; },
		async expectStyle() { return {accepted: true}; },
		async commitLayout() { return {layout: 1}; },
		async requestFrame() { return {requested: true}; },
		async applyCamera(_surface, command, view) { return {command, view}; },
		async cancelCamera() { return {cancelled: true}; },
		async acknowledgeSurface(surface, value) { acknowledgements.push([surface, value]); return {acknowledged: true}; },
		async retireSurface(surface) { retired.push(surface); return {detached: true}; },
		async retireRoot() { return {detached: true}; },
	};
	const transport = createNativeSurfaceTransport(() => module, (callback) => {
		listeners.add(callback); return () => listeners.delete(callback);
	});
	return {module, transport, retired, acknowledgements, emit(value: unknown) { for (const callback of listeners) callback(value); }};
}

test('surfaces acknowledge only their own immutable ordered native evidence', async () => {
	const f = fixture();
	const first: unknown[] = []; const second: unknown[] = [];
	const one = await f.transport.attach(1, (event) => first.push(event), () => assert.fail());
	const two = await f.transport.attach(2, (event) => second.push(event), () => assert.fail());
	await one.expectStyle('one'); await two.expectStyle('two');
...
test('bad native receipts and malformed owned events fail with no native payload exposure', async () => {
	const f = fixture();
	let failures = 0;
	const surface = await f.transport.attach(1, () => assert.fail(), () => { failures++; });
	await surface.expectStyle('one');
	f.emit({surface: surface.id, style: 'one', sequence: 1, layout: 1, kind: 'gesture-end', view: 'private'});
