...
test('browser Map sources contain only the existing map and manifest fields', () => {
  for (const source of [{map: 'main'}, {map: 'main', manifestUrl: '/custom/manifest.json'}]) {
    ...
    assert.equal(shouldLoadTileflowManifest({source}), true);
  ...
  for (const source of [
    ...
    assert.throws(
      () => resolveTileflowRuntimeStyle({source: source as unknown as TileflowRuntimeSource}),
      TypeError,
    );
  }
});
...
test('native source resolution retains explicit URLs and themes without renderer state', async () => {
  ...
  await controller.replace({map: 'main', manifestUrl}, {theme: 'system', colorScheme: 'dark'});
  ...
  assert.equal(ready.theme.name, 'night');
  assert.equal(ready.theme.styleUrl, 'https://maps.example.test/tileflow/night.json');
