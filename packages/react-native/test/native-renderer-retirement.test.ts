...
const target = {
	source: {
		...
		map: {name: 'main'}, theme: {name: 'light', colorScheme: 'light', styleUrl: 'https://maps.example.test/light.json'},
...
test('an unmounted blank view still waits for native root detachment before releasing ownership', async () => {
	...
	const roots: number[] = [];
	const owner = createNativeRendererOwner('renderer', target, {}, {
		surfaces: {
			async attach() { throw new Error('Unexpected attachment.'); },
			async retireRoot(root) { roots.push(root); await barrier.promise; },
		}, emit() {}, changed() {},
	});
	owner.bindRoot(42);
	let finished = false;
	const cleanup = owner.dispose().then(() => { finished = true; });
	await Promise.resolve();
	assert.equal(finished, false);
	barrier.resolve(); await cleanup;
...
test('a terminal renderer failure ignores late style frame and commit evidence', async () => {
	...
	const owner = createNativeRendererOwner('renderer', target, {}, {
		surfaces: {async attach(_root, listener) { native = listener; return surface; }, async retireRoot() {}},
		emit: (event) => events.push(event), changed() {},
	});
	await owner.attach(42); await owner.whenIdle();
	...
	owner.afterCommit({}); owner.layoutChanged();
	native({surface: 'surface', style: owner.token, sequence: 3, layout: 1, kind: 'render'});
	await owner.whenIdle();
	assert.equal(events.length, count); assert.equal(commands, 0);
	await owner.dispose();
});
...
test('preloading cannot mislabel the previous concrete theme as the requested theme', async () => {
	const events: NativeRendererEvent[] = [];
	const owner = createNativeRendererOwner('renderer', target, {}, {
		surfaces: {async attach() { throw new Error('Not attached.'); }, async retireRoot() {}},
		emit: (event) => events.push(event), changed() {},
	});
	owner.preload(2);
	assert.equal(events.some((event) => event.type === 'theme-change'), false);
	owner.preload(2, {name: 'dark', colorScheme: 'dark'});
	assert.deepEqual(events.filter((event) => event.type === 'theme-change'), [{
		type: 'theme-change', generation: 2, phase: 'preloading', map: 'main', targetTheme: {name: 'dark', colorScheme: 'dark'},
	}]);
	await owner.dispose();
});
