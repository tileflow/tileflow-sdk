...
test('render-time construction is inert and effect replay owns distinct controllers', () => {
	...
	const lifecycle = createMapLifecycle(() => {
		const counts = {updates: 0, disposed: 0}; instances.push(counts);
		return {
			getSnapshot: () => ({revision: 0, mapOptions: {}}), getSourceState: () => undefined,
			subscribe: () => () => undefined,
			update() { counts.updates++; }, async dispose() { counts.disposed++; },
			background() {}, resume() {}, rootMounted() {}, nativeStyleLoaded() {}, layoutChanged() {},
	...
	first();
	lifecycle.update({source: {map: 'main', manifestUrl: 'https://maps.example.test/manifest.json'}});
	assert.deepEqual(instances, [{updates: 1, disposed: 1}, {updates: 1, disposed: 0}]);
	second(); second();
...
test('callbacks from a retired owner cannot publish into a newer effect', () => {
	...
	const lifecycle = createMapLifecycle(() => ({
		getSnapshot: () => ({revision: 0, mapOptions: {}}), getSourceState: () => undefined,
		subscribe(callback: () => void) { callbacks.push(callback); return () => undefined; },
		async dispose() {},
	}) as unknown as ReturnType<typeof createMountedMapOwner>, () => undefined);
	let changes = 0;
	const release = lifecycle.subscribe(() => { changes++; });
	const first = lifecycle.mount(); first(); const second = lifecycle.mount();
