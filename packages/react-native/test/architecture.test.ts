...
test('public runtime is limited to Map while portable types and platform responsibilities stay isolated', async () => {
	const root = fileURLToPath(new URL('../src/', import.meta.url));
	for (const name of ['contract.ts', 'interaction-contract.ts']) {
		const contract = await readFile(join(root, name), 'utf8');
		const syntax = ts.createSourceFile(name, contract, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
	...
	const rendererEntries = new Set([join(root, 'map.tsx'), join(root, 'native-marker.tsx')]);
	for (const file of await sourceFiles(root)) {
		...
		for (const specifier of imports(content)) {
			...
			if (specifier.startsWith('@tileflow/'))
				assert.ok(specifier === '@tileflow/core/native' || specifier === '@tileflow/interactions', `${file}: ${specifier}`);
		...
		for (const statement of syntax.statements) {
			...
			if (specifier === '@tileflow/interactions' && !interactionOwners.has(file))
				assert.equal(statement.importClause?.isTypeOnly, true, file);
			if (specifier === '@maplibre/maplibre-react-native' && !rendererEntries.has(file))
				assert.equal(statement.importClause?.isTypeOnly, true, file);
			const privatePlatformEntry = rendererEntries.has(file) ||
