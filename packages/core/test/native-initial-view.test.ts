...
function invalid(input: unknown): void {
  assert.throws(() => resolveTileflowNativeInitialView(input as TileflowNativeInitialViewOptions),
    (error: unknown) => {
...
test('resolves explicit view > map-options view > manifest view > shared defaults per field', () => {
  ...
  const expected = {...view};
  assert.deepEqual(resolveTileflowNativeInitialView({manifestView, mapOptionsView, view}), expected);
  for (const key of ['center', 'zoom', 'bearing', 'pitch'] as const) {
    const explicit = {...view};
    Reflect.deleteProperty(explicit, key);
    assert.deepEqual(resolveTileflowNativeInitialView({manifestView, mapOptionsView, view: explicit}),
      {...view, [key]: mapOptionsView[key]});
    const options = {...mapOptionsView};
    Reflect.deleteProperty(options, key);
    assert.deepEqual(resolveTileflowNativeInitialView({manifestView, mapOptionsView: options, view: explicit}),
      {...view, [key]: manifestView[key]});
    const manifest = {...manifestView};
    Reflect.deleteProperty(manifest, key);
    assert.deepEqual(resolveTileflowNativeInitialView({manifestView: manifest, mapOptionsView: options, view: explicit}),
      {...view, [key]: defaultTileflowRuntimeView[key]});
  }
  assert.deepEqual(resolveTileflowNativeInitialView({
    manifestView: {zoom: 4}, mapOptionsView: {zoom: undefined, pitch: 20}, view: {zoom: undefined},
...
test('uses the canonical manifest view bounds and preserves longitude/latitude order', () => {
  ...
  for (const view of valid) {
    const manifest = parseTileflowRuntimeManifest({version: 1, maps: {main: {
      defaultTheme: 'light', themes: {light: {colorScheme: 'light', styleUrl: './s.json'}}, view,
    }}});
    assert.deepEqual(resolveTileflowNativeInitialView({manifestView: manifest.maps.main!.view}), view);
  }
  assert.deepEqual(resolveTileflowNativeInitialView({view: {center: [40.4, -3.7]}}).center, [40.4, -3.7]);
});

test('invalid explicitly supplied values never disappear behind higher precedence or defaults', () => {
  const inputs = [
    {zoom: -0.01}, {zoom: 24.01}, {bearing: -180.01}, {bearing: 180.01},
    {pitch: -0.01}, {pitch: 85.01}, {center: [-180.01, 0]}, {center: [180.01, 0]},
    {center: [0, -90.01]}, {center: [0, 90.01]}, {center: [0]}, {center: [0, 0, 0]},
    {center: {lng: 0, lat: 0}}, {zoom: 'private'}, {pitch: null}, {unknown: 'private'},
  ];
  for (const value of [NaN, Infinity, -Infinity]) {
    inputs.push({zoom: value}, {bearing: value}, {pitch: value}, {center: [value, 0]}, {center: [0, value]});
  }
  ...
  for (const input of [null, [], 0, 'private', {extra: true}, {view: null}, {mapOptionsView: []}]) invalid(input);
});

test('does not evaluate view/tuple accessors or accept unsafe prototypes and hidden fields', () => {
  let reads = 0;
  const get = () => { reads++; throw new Error('private'); };
...
test('clones all view inputs and leaves caller-owned tuples and defaults untouched', () => {
  ...
  assert.throws(() => { (result.center as [number, number])[0] = 10; }, TypeError);
});
...
test('view resolution is independent of acquisition and does not advance source generation', async () => {
  ...
  const controller = createTileflowNativeSourceController({acquire: () => {
    calls++; throw new Error('No acquisition for direct styles.');
  }});
