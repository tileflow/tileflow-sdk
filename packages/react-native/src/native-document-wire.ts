...
import {hasReservedNativeContext, isNativeToken, nativeResourceOrigin} from './native-admission-url';
import {
  NativeDocumentError,
...
function safeUrl(value: unknown): string {
  if (
    ...
    resolveTileflowNativeManifestUrl(value) !== value || hasReservedNativeContext(value) ||
...
function decode(value: unknown, maximumBytes: number): Uint8Array {
  ...
  for (let index = 0; index < value.length; index += 4) {
    ...
    if (index + 4 === value.length &&
      ((padding === 2 && (b & 15) !== 0) || (padding === 1 && (c & 3) !== 0))) throw invalid();
  ...
  function acquire(
    ...
    try {
      ...
      if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 1 ||
        ...
        (scope !== undefined && (!isNativeToken(scope.installation) || !isNativeToken(scope.context)))) throw invalid();
    } catch { throw invalid(); }
    ...
    const checkOwner = () => {
      try {
        if (!module || locate() !== module || [...methods].some(([name, method]) =>
          (module as unknown as Record<string, unknown>)[name] !== method)) throw unavailable();
    ...
    function cancel(): Promise<void> {
      ...
      if (finished) { release(); return Promise.resolve(); }
      const attempt = allocation.then(async (id) => {
        try {
    ...
    try {
      ...
      allocation = Promise.resolve(module.openDocument(url, maximumBytes,
        scope?.installation ?? null, scope?.context ?? null)).then((ack) => {
    ...
    const response = (async (): Promise<TileflowNativeManifestResponse> => {
      try {
        ...
        checkOwner();
        let reply;
        try { reply = await Promise.race([module!.documentResponse(id), stop]); }
        catch { if (closed) throw cancelled(); throw unavailable(); }
        if (closed) throw cancelled();
        checkOwner();
        let finalUrl: string;
        try {
          ...
          if (!Number.isInteger(reply.status) || reply.status < 100 || reply.status > 599 ||
            nativeResourceOrigin(finalUrl) !== nativeResourceOrigin(url)) throw invalid();
        } catch { throw invalid(); }
        ...
        return Object.freeze({
          ...
          reader: Object.freeze({
            async read(bound: number): Promise<
            ...
            > {
              ...
              const limit = Math.min(bound, nativeDocumentLimits.chunkBytes, maximumBytes - received + 1);
              reading = true;
              try {
                checkOwner();
                let chunk;
                try { chunk = await Promise.race([module!.documentChunk(id, limit), stop]); }
                catch { if (closed) throw cancelled(); throw unavailable(); }
                if (closed) throw cancelled();
                checkOwner();
                const bytes = decode(chunk.bodyBase64, limit);
                if (typeof chunk.last !== 'boolean' || (bytes.length === 0 && !chunk.last) ||
                  received + bytes.length > maximumBytes) { bytes.fill(0); throw invalid(); }
                if (closed) { bytes.fill(0); throw cancelled(); }
                received += bytes.length;
                if (chunk.last) { finished = true; release(); }
