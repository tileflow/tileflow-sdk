...
test('a retirement observer cannot make an older preparation failure roll back a newer theme', async () => {
	...
	const owner = createNativeRendererOwner('map', target('light', 1), {}, {
	...
	const emit = (kind: 'style' | 'render') => receive({
		kind, surface: 'surface', style: owner.token, sequence: ++sequence, layout: 1,
	});
