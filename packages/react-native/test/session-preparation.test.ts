...
import {apiOrigin, createClock, createIds, credential, deferred, grant, mapId, response, success} from './session-fixture';
...
function fixture() {
	...
	const controller = createHostedNativeSessionController({
		...
		fetch: async (_url, init) => {
			...
			if (restart) {
				restart = false;
				return response(409, {code: 'SESSION_ID_CONFLICT', error: 'Conflict', retryWithNewSession: true, sessionId: body.sessionId});
			}
			return response(201, success({
				sessionId: body.sessionId, surfaceId: body.surfaceId,
				issuedAt: clock.now().toISOString(), serverTime: clock.now().toISOString(),
				expiresAt: new Date(clock.value + 900000).toISOString(),
			}));
		},
	});
	return {controller, bodies, clock, hold(value: Promise<void>, notify: () => void) { gate = value; entered = notify; }, restart() { restart = true; }};
}
...
test('preparation returns immutable policy only and leaves all ten thousand admissions available', async () => {
	...
	assert.deepEqual(Object.keys(policy!).sort(), ['mapId', 'resourceOrigins', 'resourceScopes', 'tilesetIds']);
...
test('non-session preparation does not read identity or fetch; background and retirement cannot deliver policy', async () => {
	const direct = createHostedNativeSessionController({
		binding: {kind: 'direct'}, now: createClock().now,
