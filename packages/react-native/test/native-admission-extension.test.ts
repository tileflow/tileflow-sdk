...
test('catalogs grow only after native acknowledgement and retain original resources', async () => {
  ...
  assert.match(
    first.discriminate('https://tiles.tileflow.test/world/1/0/1.pbf'),
    /__tf_native_context=/u,
  );
  assert.match(first.discriminate(style.url), /__tf_native_context=/u);
  assert.throws(() => second.discriminate(tile.url));
  await assert.rejects(first.extendResources([{...tile, tilesetId: 'other'}]), {
    code: 'NATIVE_ADMISSION_INVALID',
  });
...
test('glyph templates extend only by an acknowledged finite font-stack union', async () => {
  ...
  assert.match(
    map.discriminate(`${apiOrigin}/fonts/Brand%20Regular/0-255.pbf`),
    /__tf_native_context=/u,
  );
  assert.deepEqual(await map.extendResources([{...glyph, fontStacks: ['Brand Bold']}]), {
    resources: 1,
  });
  assert.match(
    map.discriminate(`${apiOrigin}/fonts/Brand%20Bold/0-255.pbf`),
    /__tf_native_context=/u,
  );
  assert.deepEqual(f.bridge.registrations.get(map.context)?.resources[0].fontStacks, [
    'Brand Bold',
    'Brand Regular',
  ]);
  await assert.rejects(map.extendResources([{...glyph, scope: 'font'}]), {
    code: 'NATIVE_ADMISSION_INVALID',
  });
  await f.owner.dispose();
});
...
test('uncertain extension is retryable, but a late acknowledgement cannot revive a context', async () => {
  ...
  const release = deferred<void>();
  f.bridge.gate = release.promise;
  const pending = map.extendResources([
    {url: `${apiOrigin}/styles/other/style.json`, scope: 'style'},
  ]);
...
test('ambiguous template matches fail closed before bootstrapping', async () => {
  const f = fixture();
  const map = await f.owner.openMap({
    ...f.input,
    resources: [
      tile,
      {
        url: 'https://tiles.tileflow.test/world/1/0/0.pbf',
        scope: 'tile',
        tilesetId: 'world',
      },
    ],
  ...
  assert.deepEqual(
    await f.bridge.batch(map.context, ['https://tiles.tileflow.test/world/1/0/0.pbf']).promise,
    [],
  );
  assert.equal(f.requests(), 0);
  await f.owner.dispose();
