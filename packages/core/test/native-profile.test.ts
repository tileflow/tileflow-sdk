...
const labels = () => ({
  ...
  layers: [{
    id: 'labels', type: 'symbol', source: 'world', 'source-layer': 'poi',
    layout: {'text-field': ['get', 'name'] as unknown[], 'text-font': ['Noto Sans Regular'] as unknown[]},
  }],
});

function hasIssue(value: unknown, code: string, path: string): void {
  const issues = validateTileflowNativeStyle(value, {documentUrl});
  assert.ok(issues.some((issue) => issue.code === code && issue.path === path), JSON.stringify(issues));
  for (const issue of issues) assert.equal(tileflowNativeDiagnosticSchema.safeParse(issue).success, true);
}
...
test('defines one strict, immutable static native profile and a separate build record', () => {
  ...
  assert.equal(tileflowNativeProfileSchema.safeParse({...tileflowNativeProfile, qualified: true}).success, false);
  ...
  assert.equal(tileflowNativeBuildRecordSchema.safeParse({...record, available: true}).success, false);
  assert.equal(tileflowNativeBuildRecordSchema.safeParse({...record, profile: 'native-v2'}).success, false);
});

test('rejects unknown renderer/profile values without echoing them', () => {
  assert.throws(() => resolveTileflowRenderer('tf_live_do_not_log'), (error: unknown) => {
...
test('accepts fixed Mercator, hillshade and building extrusion independently of terrain displacement', () => {
  assert.deepEqual(validateTileflowNativeStyle({...background(), projection: {type: 'mercator'}}), []);
  const style = {
    ...background(),
    sources: {
      elevation: {type: 'raster-dem', tiles: ['https://maps.example.test/{z}/{x}/{y}.png'], encoding: 'terrarium'},
      buildings: {type: 'vector', tiles: ['https://maps.example.test/{z}/{x}/{y}.pbf']},
    },
    layers: [
      {id: 'shade', type: 'hillshade', source: 'elevation'},
      {id: 'buildings', type: 'fill-extrusion', source: 'buildings', 'source-layer': 'building', paint: {'fill-extrusion-height': 10}},
    ],
  };
...
test('rejects globe and retains the existing style-spec pitch range', () => {
  hasIssue({...background(), projection: {type: 'globe'}}, 'NATIVE_UNSUPPORTED_STYLE', '/projection');
...
test('checks nested expression support without interpreting literal data or match labels', () => {
  ...
  style.layers[0]!.layout['text-field'] = ['match', ['get', 'kind'], ['global-state', 'water'], 'name', 'fallback'];
  assert.deepEqual(validateTileflowNativeStyle(style), []);
...
for (const url of [
  ...
  test(`rejects a non-portable source URL (${url.split(':')[0]})`, () => {
    ...
    const issues = validateTileflowNativeStyle(value, {documentUrl});
    assert.ok(issues.some(({code, path}) => code === 'NATIVE_UNSUPPORTED_SOURCE' && path === '/sources/world/url'));
...
test('rejects browser sources, unsupported properties and multiple sprite providers', () => {
  hasIssue({...background(), sources: {video: {type: 'video', urls: []}}}, 'NATIVE_UNSUPPORTED_SOURCE', '/sources/video');
  hasIssue({...background(), sprite: [{id: 'one', url: 'https://example.test/sprite'}]}, 'NATIVE_UNSUPPORTED_STYLE', '/sprite');
  hasIssue({...background(), sources: {elevation: {type: 'raster-dem', tiles: ['https://maps.example.test/{z}/{x}/{y}.png'], encoding: 'custom'}}}, 'NATIVE_UNSUPPORTED_SOURCE', '/sources/elevation/encoding');
});
...
test('requires a complete native font provider instead of browser metadata or a system fallback', () => {
  ...
  hasIssue({...style, metadata: {'tileflow:fontFaces': []}}, 'NATIVE_FONT_UNAVAILABLE', '/metadata/tileflow:fontFaces');
  assert.deepEqual(validateTileflowNativeStyle({
    ...style, 'font-faces': {'Noto Sans Regular': 'https://maps.example.test/font.ttf'},
  }), []);
  hasIssue({...style, 'font-faces': {'Noto Sans Regular': 'https://maps.example.test/font.woff2'}}, 'NATIVE_FONT_UNAVAILABLE', '/font-faces/Noto Sans Regular');
  hasIssue({...labels(), glyphs: 'https://maps.example.test/{range}.pbf'}, 'NATIVE_FONT_UNAVAILABLE', '/glyphs');
});
...
test('bounds JSON traversal, resource counts, input bytes and diagnostic output', () => {
  ...
  hasIssue({...background(), metadata: {large: 'x'.repeat(tileflowNativeProfileLimits.maximumStyleBytes + 1)}}, 'NATIVE_UNSUPPORTED_STYLE', '');
  const sources = Object.fromEntries(Array.from({length: 129}, (_, i) => [`source-${i}`, {type: 'canvas'}]));
...
test('does not invoke accessors or mutate inputs while validating', () => {
  ...
  Object.defineProperty(style, 'metadata', {enumerable: true, get: () => {invoked = true; return {};}});
...
test('checks inline format font overrides and accepts explicit literal static stacks', () => {
  ...
  const style = {...base, 'font-faces': {'Noto Sans Regular': 'https://maps.example.test/font.ttf'}};
  style.layers[0]!.layout['text-font'] = ['literal', ['Noto Sans Regular']];
  assert.deepEqual(validateTileflowNativeStyle(style), []);
  style.layers[0]!.layout['text-field'] = ['format', ['get', 'name'], {'text-font': ['literal', ['Missing Face']]}];
...
test('semantic parser failures retain actual dotted source keys and deterministic order', () => {
  const source = {type: 'vector', minzoom: 'invalid', tiles: ['https://maps.example.test/{z}/{x}/{y}.pbf']};
  const first = {...background(), sources: {'a.b': source}};
  const issues = validateTileflowNativeStyle(first);
  assert.ok(issues.some(({path}) => path === '/sources/a.b/minzoom'), JSON.stringify(issues));
  const second = {...background(), sources: {'a.b': {tiles: source.tiles, minzoom: 'invalid', type: 'vector'}}};
...
test('invalid runtime options cannot bypass closure or invoke an accessor', () => {
  let calls = 0;
  const options = Object.defineProperty({}, 'profile', {get() {calls++; throw new Error('private');} });
  assert.equal(validateTileflowNativeStyle(background(), options)[0]?.code, 'NATIVE_RENDERER_UNSUPPORTED');
  assert.equal(calls, 0);
  assert.equal(validateTileflowNativeStyle(background(), {deferFontClosure: 'true' as unknown as boolean})[0]?.code, 'NATIVE_RENDERER_UNSUPPORTED');
});
