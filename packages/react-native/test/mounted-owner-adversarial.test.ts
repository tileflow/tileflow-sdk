...
function fixture() {
  ...
  const ports: MountedMapPorts = {
    documents: {
      acquire(url): TileflowNativeManifestOperation {
        let cancelled = false;
        ...
        const bytes = new TextEncoder().encode(
          JSON.stringify(
            url.endsWith('manifest.json') ? manifest : {version: 8, sources: {}, layers: []},
          ),
        );
        let offset = 0;
          ...
          Promise.resolve(gate).then(() => ({
            ...
            reader: {
              async read(maximumBytes: number) {
                ...
                const value = bytes.slice(offset, offset + maximumBytes);
                offset += value.length;
    ...
    installation: {
      open() {
        ...
        const map: NativeMapAdmission = {
          context,
          generation: 1,
          scope: {installation: 'installation', context},
    ...
    surfaces: {
      available() {},
      async attach() {
        throw new Error('Preparation only.');
      },
      async retireRoot() {},
    },
  ...
  owner.subscribe(() => {
    if (owner.getSourceState()?.status === 'ready') onReady?.();
  });
  return {
    owner,
    source,
    ...
    opens: () => opens,
    darkReads: () => darkReads,
    darkCancels: () => darkCancels,
    onReady(callback: () => void) {
      onReady = callback;
    },
    holdDark(value: Promise<void> | undefined) {
      holdDark = value;
    },
  };
}
...
test('background interruption reselects the requested concrete theme instead of retaining a cancelled job', async () => {
  ...
  assert.ok(first);
  const dark = deferred<void>();
  f.holdDark(dark.promise);
