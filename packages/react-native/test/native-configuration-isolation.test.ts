...
test('the built private factory never looks up configuration for a non-session source', async (t) => {
  ...
  const script = `
		...
		const direct = ${JSON.stringify(directSourceFixture())};
		const hosted = ${JSON.stringify(hostedSourceFixture())};
		try {
			assert.deepEqual(await first.replace(direct), {kind: 'direct'});
			assert.deepEqual(await second.replace(direct), {kind: 'direct'});
			assert.equal(native.lookups, 0);
			await assert.rejects(first.replace({...hosted, manifestUrl: 'https://untrusted.example/manifest.json'}),
				{code: 'NATIVE_CONFIGURATION_SOURCE_INVALID'});
			assert.equal(native.lookups, 0);
			await assert.rejects(first.replace(hosted), {code: 'NATIVE_CONFIGURATION_UNAVAILABLE'});
