...
type Fields = Readonly<
  Record<'category' | 'filterRank' | 'icon' | 'name' | 'sizeRank' | 'type', string>
>;
type Manifest = Readonly<{
  layers: readonly Layer[];
  fields: Fields;
  representationPriority: readonly Representation[];
}>;
...
const failure = (
  code: TileflowInteractionDiagnostic['code'] = 'SEMANTIC_MANIFEST_MISMATCH',
): NativePoiQueryResult =>
...
function category(value: unknown): value is TileflowPoiCategory {
  return (
    typeof value === 'string' && tileflowPoiCategories.some((candidate) => candidate === value)
  );
}
...
function parseManifest(style: unknown): Manifest {
  ...
  if (
    field(hitTesting, 'order') !== 'rendered-topmost' ||
    field(hitTesting, 'frequency') !== 'animation-frame'
  )
    throw new Error();
  ...
  if (
    identity.length !== 3 ||
    identity[0] !== 'source' ||
    identity[1] !== 'source-layer' ||
    identity[2] !== 'feature-id'
  )
    throw new Error();
  const priorityInput = array(field(deduplication, 'representationPriority'), 4);
  if (
    priorityInput.length !== 4 ||
    !priorityInput.every(representation) ||
    new Set(priorityInput).size !== 4
  )
    throw new Error();
  ...
  const fields: Fields = Object.freeze({
    ...
    filterRank: readField('filterRank'),
    icon: readField('icon'),
    name: readField('name'),
    sizeRank: readField('sizeRank'),
    type: readField('type'),
  });
  ...
  for (const input of inputs) {
    ...
    if (
      !name(layerId) ||
      !name(source) ||
      (sourceLayer !== undefined && !name(sourceLayer)) ||
      ...
      priority > 100_000 ||
      ids.has(layerId) ||
      field(input, 'anchor') !== 'pointer-coordinate'
    )
      throw new Error();
    const actual = physical.get(layerId);
    if (
      !actual ||
      field(actual, 'source') !== source ||
      field(actual, 'source-layer') !== sourceLayer
    )
      throw new Error();
    ...
    if (
      ...
      Array.isArray(sourceDefinition)
    )
      throw new Error();
    if (namespace && (namespace.source !== source || namespace.sourceLayer !== sourceLayer))
      throw new Error();
  ...
  return Object.freeze({
    layers: Object.freeze(layers.sort((left, right) => right.priority - left.priority)),
    fields,
    representationPriority: Object.freeze(priorityInput),
  });
}
...
function properties(input: unknown, fields: Fields): TileflowPoiProperties {
  ...
  for (const [key, output, max] of [
    ['filterRank', 'filter_rank', 5],
    ['sizeRank', 'size_rank', 16],
  ] as const) {
  ...
  for (const key of ['name', 'type', 'icon'] as const) {
    ...
    if (
      typeof value === 'string' &&
      value.length <= tileflowInteractionLimits.maxContentTextLength &&
      (key === 'name' || /^[a-z0-9]+(?:_[a-z0-9]+)*$/u.test(value))
    )
      result[key] = value;
  }
  return Object.freeze(result);
}
export function nativePoiBindingMatches(
  binding: TileflowInteractionBinding,
  target: TileflowResolvedPoiFeatureTarget,
): boolean {
...
function choose(
  ...
  for (const [renderedIndex, feature] of features.entries()) {
    ...
    if (
      field(feature, 'source') !== layer.source ||
      field(feature, 'sourceLayer') !== layer.sourceLayer
    )
      throw new Error();
    const binding = bindings.find(
      (value) =>
        value.target.kind === 'semantic-feature' &&
        value.target.domain === 'poi' &&
    ...
    const target: TileflowResolvedPoiFeatureTarget = freezeNativeInteractionValue({
      ...
      feature: {
        ...
        ...(stable === undefined ? {} : {id: stable}),
        properties: properties(field(feature, 'properties'), manifest.fields),
      },
    });
    ...
    if (existingIndex === undefined) {
      ...
      if (
        ...
        (nextPriority === oldPriority && layer.priority > existing.layer.priority)
      )
        candidates[existingIndex] = {
          ...candidate,
          renderedIndex: Math.min(existing.renderedIndex, renderedIndex),
        };
    }
  }
  return candidates.sort(
    (a, b) => a.renderedIndex - b.renderedIndex || b.layer.priority - a.layer.priority,
  )[0]?.match;
}
...
export function createNativePoiAdapter() {
  ...
  const inspect = (
    candidate: NativePoiStyleLease | undefined,
    expected: number,
  ): 'current' | 'stale' | 'error' => {
  ...
  const stopPending = () => {
    ...
    try {
      ...
      /* Cancellation cannot transfer style ownership. */
    }
  };
  const cancelQueries = () => {
    ++intent;
  ...
  return Object.freeze({
    ...
    supports(target: TileflowResolvedPoiFeatureTarget): boolean {
      ...
      return (
        !!parsed &&
        inspect(lease, generation) === 'current' &&
        manifest === parsed &&
    ...
    replaceStyle(next?: NativePoiStyleLease): readonly TileflowInteractionDiagnostic[] {
      ...
      lease = next;
      cancelQueries();
      if (!next || inspect(next, expected) !== 'current')
        return next && generation === expected ? failure().diagnostics : emptyDiagnostics;
      try {
        const parsed = parseManifest(next.style);
        if (inspect(next, expected) === 'current') {
          manifest = parsed;
          return emptyDiagnostics;
        }
      } catch {
        /* Only fixed portable diagnostics leave the native boundary. */
      }
    ...
    queryTouch(
      input: unknown,
      bindingsInput: readonly TileflowInteractionBinding[],
    ): Promise<NativePoiQueryResult> {
      const ticket = ++intent;
      const expected = generation;
      const invocationIsLive = () => !disposed && intent === ticket && generation === expected;
      const answer = (value: NativePoiQueryResult) =>
        Promise.resolve(invocationIsLive() ? value : stale);
      stopPending();
      ...
      try {
        ...
        if (
          ...
          throw new Error();
        point = Object.freeze([p[0], p[1]] as const);
        coordinate = Object.freeze([c[0], c[1]] as const);
      } catch {
        return answer(failure('INVALID_DOCUMENT'));
      }
      const prepared = prepareNativeInteractionInputs({interactions: bindingsInput});
      if (prepared.diagnostics.length)
        return answer(Object.freeze({status: 'error', diagnostics: prepared.diagnostics}));
      const bindings = prepared.bindings.filter(
        (binding) => binding.target.kind === 'semantic-feature' && binding.target.domain === 'poi',
      );
      if (!bindings.length)
        return answer(prepared.bindings.length ? failure('UNSUPPORTED_TARGET') : miss);
      ...
      if (!invocationIsLive()) return Promise.resolve(stale);
      const request: NativePoiQueryRequest = Object.freeze({
        point,
        layers: Object.freeze(layers),
        limit: nativePoiQueryLimit,
      });
      let resolve!: (value: NativePoiQueryResult) => void;
      const result = new Promise<NativePoiQueryResult>((yes) => {
        resolve = yes;
      });
      const job: Pending = {live: true, resolve};
      pending = job;
      const finish = (value: NativePoiQueryResult) => {
        ...
        resolve(
          ownership === 'error'
            ? failure()
            : ownership === 'current' && invocationIsLive()
              ? value
              : stale,
        );
      };
      try {
        ...
        job.cancel = () => {
          if (!cancelled) {
            cancelled = true;
            cancel.call(operation);
          }
        };
        void Promise.resolve(operation.result).then(
          (value) => {
            if (!job.live || pending !== job) return;
            try {
              const ownership = inspect(owner, expected);
              if (ownership !== 'current') {
                finish(ownership === 'error' ? failure() : stale);
                return;
              }
              if (field(value, 'request') !== request) throw new Error();
              const raw = field(value, 'features');
              const length = Array.isArray(raw)
                ? (Object.getOwnPropertyDescriptor(raw, 'length')?.value as unknown)
                : undefined;
              if (typeof length === 'number' && length > nativePoiQueryLimit) {
                finish(failure('LIMIT_EXCEEDED'));
                return;
              }
              const match = choose(
                array(raw, nativePoiQueryLimit),
                selected,
                bindings,
                coordinate,
                layers,
              ...
              finish(
                match
                  ? Object.freeze({status: 'match', match, diagnostics: emptyDiagnostics})
                  : miss,
              );
            } catch {
              finish(failure());
        ...
        if (!job.live || pending !== job || inspect(owner, expected) !== 'current') {
          if (pending === job) cancelQueries();
            ...
            try {
              ...
              /* A retired query has no observers. */
            }
          }
        }
      } catch {
        ...
        try {
          ...
          /* A malformed port cannot expose its cleanup error. */
        }
      }
      return result;
    },
