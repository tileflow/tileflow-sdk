...
type ParseOutput = Expect<
  Equal<ReturnType<typeof parseTileflowRuntimeManifest>, TileflowRuntimeManifest>
>;
type SafeOutput = Expect<
  Equal<
    ReturnType<typeof safeParseTileflowRuntimeManifest>,
    z.ZodSafeParseResult<TileflowRuntimeManifest>
  >
>;
type SchemaOutput = Expect<
  Equal<z.output<typeof tileflowRuntimeManifestSchema>, TileflowRuntimeManifest>
>;
...
async function parity(
  ...
  exerciseClassicFormatting = false,
): Promise<void> {
  ...
  if (issuePath)
    assert.ok(
      host.error.issues.some((issue) => JSON.stringify(issue.path) === JSON.stringify(issuePath)),
    );
  assert.throws(
    () => parseNative(value),
    (error: unknown) => {
  ...
  assert.throws(
    () => parseTileflowRuntimeManifest(value),
    (error: unknown) => {
      assert.ok(error instanceof z.ZodError);
      assert.deepEqual(error.issues, host.error.issues);
      if (exerciseClassicFormatting) {
...
test('preserves the public classic schema, errors and composition methods', async () => {
  ...
  await parity({...manifest(), unexpected: true}, false, [], true);
...
test('keeps strict nested objects and unsafe-input rejection identical', async () => {
  for (const path of [
    [],
    ['maps', 'streets'],
    ['maps', 'streets', 'themes', 'light'],
    ['maps', 'streets', 'systemThemes'],
    ['maps', 'streets', 'view'],
    ['maps', 'streets', 'themes', 'light', 'fontFaces', 0],
  ] as const) {
    const value = manifest();
    Object.assign(value.maps.streets.themes.light, {
      fontFaces: [{family: 'Fixture', source: './a.ttf'}],
    });
  ...
  inherited.maps.streets.view = Object.assign(
    Object.create({hidden: 1}),
    inherited.maps.streets.view,
  );
  await parity(inherited, false, ['maps', 'streets', 'view']);
  const unsafe = manifest();
  Object.defineProperty(unsafe.maps.streets.themes.light, '__proto__', {
    value: {},
    enumerable: true,
  });
...
test('retains identity checks, optional fields and cross-field refinement order', async () => {
    ...
    await parity(
      {
        ...
        maps: {
          main: {
            defaultTheme: name,
            themes: {[name]: {colorScheme: 'light', styleUrl: './style.json'}},
          },
        },
      },
      valid,
  ...
  for (const change of [
    ...
    {systemThemes: {light: 'dark', dark: 'light'}},
    {systemThemes: {light: 'absent', dark: 'dark'}},
    {usageMode: 'session'},
    {worldGeneration: 'v1'},
  ]) {
    const value = manifest();
  ...
  Object.assign(valid.maps.streets, {
    apiUrl: undefined,
    usageMode: 'session',
    worldGeneration: 'v1',
  });
  ...
  for (const faces of [
    [
      {family: 'A', source: './a.ttf'},
      {family: 'A', source: './b.ttf', style: 'normal', weight: '400'},
    ],
...
test('retains numeric, collection, UTF-8 byte and URL boundaries', async () => {
  for (const [pitch, valid] of [
    ...
    const value = manifest();
    value.maps.streets.view.pitch = pitch;
  ...
  for (const size of [64, 65]) {
    const themes = Object.fromEntries(
      Array.from({length: size}, (_, i) => [
        `t${i}`,
        {colorScheme: 'light', styleUrl: './style.json'},
      ]),
    );
  ...
  const maps = Object.fromEntries(
    Array.from({length: 240}, (_, i) => [
      ...
      {
        defaultTheme: 'light',
        themes: {light: {colorScheme: 'light', styleUrl: './' + '界'.repeat(1500)}},
      },
    ]),
  );
  ...
  for (const [url, valid] of [
    ...
    ['../a.json', true],
    ['/a.json', true],
    ['https://bücher.example.test/a.json', true],
    ['a.json', false],
    ['//host/a', false],
    ['https://user:secret@host/a', false],
    ['https://host/a#fragment', false],
    ['https://host/%2fsecret', false],
    ['file:///a', false],
  ] as const) {
    const value = manifest();
    value.maps.streets.themes.light.styleUrl = url;
