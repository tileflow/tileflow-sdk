...
test('a queued upstream command is not an application receipt', async () => {
  ...
  const finished = operation.finished.then(() => {
    applied = true;
  });
...
test('foreign tokens, partial or wrong views, and teardown never acknowledge application', async () => {
  for (const receipt of [
    {command: 2, view},
    {command: 1, view: {...view, zoom: 4}},
    {command: 1, view: {zoom: 3}},
  ]) {
    const port = createNativeCameraPort({
      async applyCamera() {
        return receipt as {command: number; view: typeof view};
      },
      async cancelCamera() {
        return {cancelled: true as const};
      },
    });
    await assert.rejects(port.apply({token: {sequence: 1}, view}).finished, {
      message: 'Native camera operation failed.',
    });
    port.dispose();
  }
  const pending = deferred<{command: number; view: typeof view}>();
  const port = createNativeCameraPort({
    applyCamera: () => pending.promise,
    cancelCamera: async () => ({cancelled: true}),
  });
  const operation = port.apply({token: {sequence: 1}, view});
  await Promise.resolve();
  port.dispose();
