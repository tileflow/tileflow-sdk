...
export function response(
	status: number,
	body: unknown,
	options: {headers?: Record<string, string>; chunks?: readonly Uint8Array[]; nullBody?: boolean} = {},
): HostedNativeSessionFetchResponse {
	const bytes = options.chunks ?? [new TextEncoder().encode(typeof body === 'string' ? body : JSON.stringify(body))];
	let index = 0;
	let cancelled = false;
	return {
		status,
...
export function createFetchQueue(entries: Array<HostedNativeSessionFetchResponse | (() => Promise<HostedNativeSessionFetchResponse>)>) {
