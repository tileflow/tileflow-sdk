...
export function createNativeManifestCache(acquire: TileflowNativeManifestAcquire) {
  ...
  const clear = () => {
    ...
    active = undefined;
    try {
      void Promise.resolve(previous?.cancel()).catch(() => undefined);
    } catch {
      /* The owning document pool retries cleanup. */
    }
  };
      ...
      response: Promise.resolve({
        ...
        reader: {
          async read(bound) {
            ...
            const bytes = value.bytes.slice(offset, offset + bound);
            offset += bytes.length;
          ...
          reader: {
            async read(bound) {
              ...
              if (
                ...
                offset + next.value.length > maximum
              )
                throw new NativePreparationError();
