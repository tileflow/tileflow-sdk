...
function fixture(
    ...
    return {
      ...
      value: {
        ...
        metadata: {
          ...
          'tileflow:deploymentVersion': version,
          'tileflow:nativeStyleSha256': 'a'.repeat(64),
        },
      },
    };
  };
  const ports: MountedMapPorts = {
    createBinding: () =>
      createHostedNativeBindingResolver(async () => {
        configurationReads++;
      ...
      open(input) {
        ...
        const controller = createHostedNativeSessionController({
          ...
          fetch: async (url, init) => {
            ...
            return response(201, success(body));
          },
        });
        ...
        const map: NativeMapAdmission = {
          context: id,
          generation: 1,
          scope: {installation: 'installation', context: id},
          get state() {
            return {context: id, status: active ? 'active' : 'retired'};
          },
    ...
    documents: {
      acquire(url, _bounds, scope): TileflowNativeManifestOperation {
        ...
        const pending = Promise.resolve().then(
          async (): Promise<TileflowNativeManifestResponse> => {
            ...
            if (url === source.manifestUrl) {
              ...
              assert.ok(
                catalogs.get(scope.context)?.has(url),
                'The exact style is admitted before acquisition.',
              );
  ...
  return {
    ...
    start() {
      owner.update({source, onError: (error) => errors.push(error)});
    },
    async mount() {
...
test('cold start reads public metadata then bootstraps one session before its protected style', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      style.metadata,
      (
        f.prepareStyle(`${apiOrigin}/maps/${mapId}/native/v7/light.json`).value as {
          metadata: unknown;
        }
      ).metadata,
    );
...
test('a stale public manifest is reloaded once without replacing the session or context', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      f.calls.map(({url}) => url),
      [
        f.source.manifestUrl,
        `${apiOrigin}/maps/${mapId}/native/v7/light.json`,
        f.source.manifestUrl,
        `${apiOrigin}/maps/${mapId}/native/v8/light.json`,
      ],
    );
    assert.deepEqual(f.errors, []);
  } finally {
  ...
  try {
    ...
    assert.equal(
      await Promise.race([
        ...
        first.owner.whenIdle().then(() => false),
      ]),
      true,
      'Preparation ended before the protected style request.',
    );
