...
export class NativeLoweringError extends Error {
  constructor(
    ...
    readonly reason: 'shape' | 'budget' = 'shape',
  ) {
    super(
      reason === 'budget'
        ? 'Native decision lowering exceeds its finite budget.'
...
function operation(value: unknown, name: string, length?: number): value is unknown[] {
  return (
    Array.isArray(value) && value[0] === name && (length === undefined || value.length === length)
  );
}
...
function combine(operator: 'all' | 'any', values: Predicate[]): Predicate {
  ...
  for (const value of values.flatMap((item) =>
    operation(item, operator) ? (item.slice(1) as Predicate[]) : [item],
  )) {
...
class Decisions {
  ...
  private tick(depth: number): void {
    if (
      ++this.visited > nativeLoweringLimits.maximumDecisionNodes ||
      depth > nativeLoweringLimits.maximumDecisionDepth
    ) {
  ...
  tree(value: unknown, property: Property, depth = 0): Tree {
    ...
    if (
      property === 'line-cap' &&
      typeof leaf === 'string' &&
      ['butt', 'round', 'square'].includes(leaf)
    ) {
    ...
    if (
      ...
      Array.isArray(leaf) &&
      leaf.length > 0 &&
      leaf.length <= nativeLoweringLimits.maximumDashLength &&
      leaf.every((item) => finite(item) && item >= 0)
    ) {
    ...
    if (operation(value, 'match') && value.length >= 5 && value.length % 2 === 1) {
      ...
      return {
        kind: 'case',
        arms: labels.map((when, index) => ({
          when,
          value: this.tree(value[index * 2 + 3], property, depth + 1),
        })),
    ...
    if (operation(value, 'step') && value.length >= 5 && value.length % 2 === 1) {
      ...
      return {
        kind: 'step',
        input: value[1],
        fallback: this.tree(value[2], property, depth + 1),
        stops,
      };
    }
  ...
  private scalar(value: unknown, depth: number): 'number' | 'scalar' {
    ...
    if (operation(value, 'geometry-type', 1) || (operation(value, 'id', 1) && this.vector))
      return 'scalar';
  ...
  private matchLabels(value: unknown[], depth: number): Predicate[] {
    ...
    for (let index = 2; index < value.length - 1; index += 2) {
      ...
      for (const label of labels) {
        this.tick(depth);
        if (!(typeof label === 'string' || (finite(label) && Number.isInteger(label))))
          fail(this.path);
  ...
  private predicate(value: unknown, depth: number): Predicate {
    ...
    if (operation(value, 'all') || operation(value, 'any')) {
      return combine(
        value[0] as 'all' | 'any',
        value.slice(1).map((child) => this.predicate(child, depth + 1)),
      );
    }
    ...
    if (operation(value, 'case') && value.length >= 4 && value.length % 2 === 0) {
      ...
      for (let index = 1; index < value.length - 1; index += 2) {
        ...
        terms.push(
          combine('all', [remaining, condition, this.predicate(value[index + 1], depth + 1)]),
        );
        remaining = combine('all', [remaining, negate(condition)]);
      }
      terms.push(combine('all', [remaining, this.predicate(value.at(-1), depth + 1)]));
    ...
    if (
      operation(value, 'in', 3) &&
      operation(value[2], 'literal', 2) &&
      Array.isArray(value[2][1])
    ) {
      this.scalar(value[1], depth + 1);
      if (operation(value[1], 'zoom', 1)) fail(this.path);
  ...
  private atZoom(value: unknown, zoom: number): Predicate {
    ...
    if (expr[0] === 'all' || expr[0] === 'any')
      return combine(
        expr[0],
        expr.slice(1).map((child) => this.atZoom(child, zoom)),
      );
  ...
  outcomes(tree: Tree, zoom: number): Outcome[] {
    ...
    return [...grouped.entries()].map(([id, outcome]) => ({
      value: outcome.value,
      when: this.accepts(tree, id, zoom),
    }));
  }
  ...
  private accepts(tree: Tree, outcome: string, zoom: number): Predicate {
    ...
    if (tree.kind === 'case') {
      return decision(
        tree.arms.map((arm) => ({
          when: this.atZoom(arm.when, zoom),
          value: this.accepts(arm.value, outcome, zoom),
        })),
    ...
    return decision(
      tree.stops.map((stop, index) => ({
        when: ['<', tree.input, stop.at],
        value: this.accepts(
          index === 0 ? tree.fallback : tree.stops[index - 1]!.value,
          outcome,
          zoom,
        ),
      })),
...
function lowerLayer(
  ...
  index: number,
  vector: boolean,
): {variants: Variant[]; properties: Property[]} | undefined {
  ...
  for (const [group, property] of [
    ...
    if (property === 'line-cap') cap = {decisions, tree};
    else dash = {decisions, tree};
  }
  ...
  const cuts = [
    ...new Set([
      minimum,
      maximum,
      ...[...(cap?.decisions.cuts ?? []), ...(dash?.decisions.cuts ?? [])].filter(
        (cut) => cut > minimum && cut < maximum,
      ),
    ]),
  ].sort((a, b) => a - b);
  if (cuts.length - 1 > nativeLoweringLimits.maximumZoomIntervals)
  ...
  for (let part = 0; part < cuts.length - 1; part++) {
    ...
    const caps = cap
      ? cap.decisions.outcomes(cap.tree, Math.floor(low))
      : [{when: true as const, value: undefined}];
    const dashes = dash
      ? dash.decisions.outcomes(dash.tree, Math.floor(low))
      : [{when: true as const, value: undefined}];
    for (const c of caps)
      for (const d of dashes) {
        ...
        if (variants.length > nativeLoweringLimits.maximumLayerVariants)
          fail(`/layers/${index}`, 'budget');
      }
  }
...
export function lowerNativeStyleRepresentation(
  ...
  if (!Array.isArray(input.layers) || input.layers.length > nativeLoweringLimits.maximumLayers)
    fail('/layers', 'budget');
  const originalLayers = input.layers as NativeLayer[];
  const reserved = new Set<string>();
  for (const [index, layer] of originalLayers.entries()) {
    if (!object(layer) || typeof layer.id !== 'string' || reserved.has(layer.id))
      fail(`/layers/${index}/id`);
  ...
  if (input.projection !== undefined) {
    if (
      ...
      Object.keys(input.projection).length !== 1 ||
      typeof input.projection.type !== 'string' ||
      !['globe', 'mercator'].includes(input.projection.type)
    )
      fail('/projection');
  ...
  for (const [index, layer] of originalLayers.entries()) {
    const source =
      object(input.sources) && typeof layer.source === 'string'
        ? input.sources[layer.source]
        : undefined;
    ...
    if (
      layers.length + lowered.variants.length + originalLayers.length - index - 1 >
      nativeLoweringLimits.maximumLayers
    ) {
      fail(`/layers/${index}`, 'budget');
    }
    ...
    for (const [branch, variant] of lowered.variants.entries()) {
      ...
      if (variant.when !== true)
        next.filter =
          originalFilter === undefined ? variant.when : ['all', originalFilter, variant.when];
    ...
    transformations.push({
      inputLayer: index,
      outputStart,
      outputCount: lowered.variants.length,
      properties: lowered.properties,
    });
  }
  const changed = projection !== 'none' || transformations.length > 0;
  const output: JsonObject & {layers: NativeLayer[]} = changed
    ? {...input, layers}
    : (input as JsonObject & {layers: NativeLayer[]});
  if (changed && projection !== 'none') delete output.projection;
  return {
    style: output,
    projection,
    layers: transformations,
    inputLayers: originalLayers.length,
