...
export async function createAdmissionHarness(input: {
  ...
  const report = (event: Observation) => {
    try {
      ...
      /* Observers do not own transport. */
    }
  };
  const owner = createNativeAdmissionOwner({
    ...
    onObservation(event) {
      const map =
        event.context === first?.context
          ? 'first'
          : event.context === second?.context
            ? 'second'
            : undefined;
  ...
  try {
      ...
      return (
        <View style={{flex: 1}}>
          {views.map((slot) => (
            <MapLibreMap
              key={slot.key}
              testID={`native-admission-${slot.map}`}
              style={{flex: 1}}
              mapStyle={slot.url}
    ...
    return Object.freeze({
      ...
      changeFirstStyle: () =>
        serial(async () => {
          if (!first || first.state.status !== 'active')
            throw new Error('The first Map is retired.');
          alternate = !alternate;
          const url = first.discriminateForTest(
            alternate ? input.alternateStyleUrl : input.styleUrl,
          );
      ...
      replaceFirst: () =>
        serial(async () => {
          ...
          publish([
            {key: first.context, url: first.discriminateForTest(input.styleUrl), map: 'first'},
            ...slots.filter((slot) => slot.map !== 'first'),
          ]);
        }),
      stop,
    });
