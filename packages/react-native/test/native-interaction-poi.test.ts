...
import {
  ...
  queryFixture,
  touch,
} from './native-interaction-fixture';
...
test('queries only declared relevant layers and returns semantic targets without physical identities', async () => {
  ...
  native.setFeatures([
    poiFeature(90, 'undeclared'),
    poiFeature(7, 'food-label'),
    poiFeature(8, 'hotel'),
    poiFeature(7, 'food-icon', {name: 'Preferred icon'}),
  ]);
  const adapter = createNativePoiAdapter();
  assert.deepEqual(adapter.replaceStyle(native.lease), []);
  const result = await adapter.queryTouch(touch, [
    {...poiBinding, target: {kind: 'semantic-feature', domain: 'poi', categories: ['food-drink']}},
  ]);
  ...
  assert.doesNotMatch(
    JSON.stringify(result.match.target),
    /food-icon|food-label|world|sourceLayer|layerId/u,
  );
...
test('rendered order wins between identities and numeric and string IDs stay distinct', async () => {
  ...
  native.setFeatures([
    poiFeature('7', 'food-label', {name: 'String'}),
    poiFeature(7, 'food-icon', {name: 'Number'}),
  ]);
...
test('metadata version fields namespaces and every declared physical layer are verified transactionally', async () => {
  const mutations: Array<(style: ReturnType<typeof poiStyle>) => void> = [
    (style) => {
      style.metadata['tileflow:interaction-manifest'].version = 1;
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].version = 3;
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.identity = 'properties-id';
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.deduplication.identity.reverse();
    },
    (style) => {
      style.metadata[
        'tileflow:interaction-manifest'
      ].domains.poi.deduplication.representationPriority = ['icon', 'icon', 'label', 'marker'];
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.hitTesting.order = 'source';
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.fields.name = '__proto__';
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.layers[0]!.layerId = 'missing';
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.layers[0]!.category =
        'not-a-category';
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.layers[0]!.sourceLayer =
        'another-namespace';
      style.layers[0]!['source-layer'] = 'another-namespace';
    },
    (style) => {
      style.layers[0]!.source = 'another-source';
    },
    (style) => {
      style.layers.push({...style.layers[0]!});
    },
    (style) => {
      style.metadata['tileflow:interaction-manifest'].domains.poi.layers.push({
        ...style.metadata['tileflow:interaction-manifest'].domains.poi.layers[0]!,
      });
    },
  ];
  ...
  for (const style of [{}, null, {version: 8, layers: [], metadata: {}}]) {
    const adapter = createNativePoiAdapter();
    assert.equal(
      adapter.replaceStyle(queryFixture(style).lease)[0]?.code,
...
test('feature source mismatch fails closed and unsafe properties are not evaluated or forwarded', async () => {
  ...
  assert.equal(result.status, 'match');
  if (result.status !== 'match') assert.fail();
  assert.deepEqual(result.match.target.feature.properties, {
    category: 'food-drink',
    filter_rank: 3,
    icon: 'coffee_shop',
  });
...
test('missing feature identity is not invented, query payloads are bounded, and activation is touch only', async () => {
  ...
  assert.equal(
    (await adapter.queryTouch({...touch, point: [NaN, 0]}, [poiBinding])).status,
    'error',
  );
  adapter.dispose();
});
...
test('replacement and disposal settle cancelled queries without waiting for late native completion', async () => {
  ...
  const adapter = createNativePoiAdapter();
  adapter.replaceStyle(old.lease);
  const pending = adapter.queryTouch(touch, [poiBinding]);
  const replacement = queryFixture();
  replacement.setFeatures([poiFeature(2)]);
...
test('latest touch and other-map receipts cannot satisfy a current query', async () => {
  const native = queryFixture();
  const other = queryFixture();
  const adapter = createNativePoiAdapter();
  const second = createNativePoiAdapter();
  adapter.replaceStyle(native.lease);
  second.replaceStyle(other.lease);
...
test('current-style ownership and native failures are checked again after asynchronous results', async () => {
  ...
  const pending = adapter.queryTouch(touch, [poiBinding]);
  native.retire();
  result.resolve({request: native.requests[0], features: [poiFeature(1)]});
  assert.equal((await pending).status, 'stale');
  const broken = queryFixture();
  broken.setQuery(() => {
    throw new Error('https://native.example.test/secret');
  });
