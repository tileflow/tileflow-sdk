...
test('contract source imports are type-only and runtime responsibilities remain isolated', async () => {
  ...
  for (const name of ['contract.ts', 'index.ts']) {
    ...
    for (const statement of syntax.statements) {
      if (ts.isImportDeclaration(statement))
        assert.equal(statement.importClause?.isTypeOnly, true, name);
  ...
  for (const file of await sourceFiles(root)) {
    ...
    for (const statement of syntax.statements) {
      ...
      if (specifier === '@maplibre/maplibre-react-native')
        assert.equal(statement.importClause?.isTypeOnly, true, file);
      if (specifier === 'react-native' && !file.endsWith('native-appearance.ts')) {
        assert.equal(statement.importClause?.isTypeOnly, true, file);
      }
    }
