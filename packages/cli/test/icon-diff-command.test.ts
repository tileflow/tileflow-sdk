...
test('absent proposed packages and unavailable historical mappings remain explicit', async (t) => {
  ...
  await writeFile(
    fixture.configPath,
    `export default {maps: {production: {basemap: {type: 'streets', basemapVersion: 1, variant: 'light'}, name: 'Production'}}};\n`,
  );
  const requests: Array<{authorization?: string; method?: string; url?: string}> = [];
...
test('a mapping-only revision changes policy output without inventing pixel changes', async (t) => {
  ...
  await writeFile(
    fixture.configPath,
    `export default {maps: {production: {basemap: {type: 'streets', basemapVersion: 1, variant: 'light'}, icons: {mapping: {health: 'clinic'}, source: './baseline-icons'}}}};\n`,
  );
  const requests: Array<{authorization?: string; method?: string; url?: string}> = [];
