 ...
 */
export function snapshotHostedNativeManifestIdentity(
  source: TileflowNativeSourceState,
): Identity | null {
  try {
    ...
    if (
      typeof mapId !== 'string' ||
      !mapPattern.test(mapId) ||
      typeof name !== 'string' ||
      !themePattern.test(name) ||
      name !== field(input, 'map')
    )
      throw new Error();
    ...
    if (declaredApi !== apiOrigin || (rootApi !== undefined && rootApi !== apiOrigin))
      throw new Error();
    const themes = field(map, 'themes');
    const declaredThemes = field(declared, 'themes');
    const themeNames = names(themes, 64).sort();
    if (
      themeNames.join('\0') !== names(declaredThemes, 64).sort().join('\0') ||
      field(map, 'defaultTheme') !== field(declared, 'defaultTheme')
    )
      throw new Error();
    ...
    for (const themeName of themeNames) {
      ...
      if (
        ...
        revision !== field(original, 'revision') ||
        styleUrl !== field(original, 'styleUrl') ||
        field(theme, 'colorScheme') !== field(original, 'colorScheme')
      )
        throw new Error();
    ...
    if (
      field(selected, 'styleUrl') !== field(theme, 'styleUrl') ||
      field(selected, 'revision') !== field(theme, 'revision')
    )
      throw new Error();
