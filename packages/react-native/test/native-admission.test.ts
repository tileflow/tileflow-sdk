...
const resource = Object.freeze({
  url: 'https://tiles.tileflow.test/world/0/0/0.pbf?variant=a%2Fb&x=1',
  scope: 'tile' as const,
  tilesetId: 'world',
});

function fixture(
  options: {direct?: boolean; holdAcquire?: Promise<void>; acquired?: () => void} = {},
) {
  ...
  const input = {
    binding: options.direct
      ? {kind: 'direct' as const}
      : {kind: 'hosted' as const, apiOrigin, credential, mapId, surfaceId: 'store-locator'},
...
test('one real acquire per ticket, with ordered results and no batch grant allocation', async () => {
  ...
  const batch = f.bridge.batch(
    map.context,
    Array.from({length: nativeAdmissionLimits.batchSize}, () => resource.url),
  );
  const results = await batch.promise;
  assert.equal(f.acquisitions, nativeAdmissionLimits.batchSize);
  assert.deepEqual(
    results.map((result) => result.ticket),
    batch.tickets.map((ticket) => ticket.ticket),
  );
...
test('mixed scope results do not reuse a successful ticket or drop its neighbour', async () => {
  ...
  const results = await f.bridge.batch(map.context, [resource.url, denied.url, resource.url])
    .promise;
  assert.deepEqual(
...
test('expiry after acquire but before bridge delivery fails without a second acquire', async () => {
  ...
  assert.deepEqual(results, [
    {ticket: batch.tickets[0].ticket, kind: 'reject', code: 'NATIVE_ADMISSION_EXPIRED'},
  ]);
...
test('cancelled tickets keep their accounting but cannot carry authority', async () => {
  ...
  f.bridge.listener({
    kind: 'cancel',
    installation: f.bridge.installation,
    context: map.context,
    generation: map.generation,
    tickets: batch.tickets.map((ticket) => ticket.ticket),
  });
  release.resolve();
...
test('two contexts with identical original URLs never share a controller or callbacks', async () => {
  ...
  assert.notEqual(
    first.discriminateForTest(resource.url),
    second.discriminateForTest(resource.url),
  );
  const results = await Promise.all([
    f.bridge.batch(first.context, [resource.url]).promise,
    f.bridge.batch(second.context, [resource.url]).promise,
  ]);
...
test('unknown and stale contexts cannot spend admission; an oversized batch retires its context', async () => {
  ...
  assert.equal(f.acquisitions, 0);
  const oversized = f.bridge.batch(
    map.context,
    Array.from({length: nativeAdmissionLimits.batchSize + 1}, () => resource.url),
  );
...
test('install and removal promises acknowledge actual bridge completion, including disposal during install', async () => {
  ...
  let settled = false;
  const disposal = f.owner.dispose().then((ack) => {
    settled = true;
    return ack;
  });
...
test('a failed installation acknowledgement can be retried on the same owner', async () => {
  const f = fixture();
  ...
  f.bridge.installGate = Promise.resolve({installation: f.bridge.installation});
  assert.deepEqual(await f.owner.install(), {installation: f.bridge.installation});
  assert.equal(f.owner.state.status, 'ready');
  await f.owner.dispose();
});

test('a failed context retirement acknowledgement can be retried', async () => {
  const f = fixture();
  const map = await f.owner.openMap(f.input);
  f.bridge.retireGate = Promise.reject(new Error(grant));

  await assert.rejects(map.retire(), {
    code: 'NATIVE_ADMISSION_UNAVAILABLE',
  });

  f.bridge.retireGate = undefined;
  assert.deepEqual(await map.retire(), {retired: true});
  assert.deepEqual(f.bridge.retirements, [map.context]);
  await f.owner.dispose();
});

test('a failed installation removal can be retried without reopening the owner', async () => {
  const f = fixture();
  await f.owner.openMap(f.input);
  f.bridge.removeGate = Promise.reject(new Error(grant));

  await assert.rejects(f.owner.dispose(), {
    code: 'NATIVE_ADMISSION_UNAVAILABLE',
  });

  f.bridge.removeGate = Promise.resolve({removed: true, ownershipLost: false});
  assert.deepEqual(await f.owner.dispose(), {removed: true, ownershipLost: false});
  assert.deepEqual(f.bridge.removals, [f.bridge.installation, f.bridge.installation]);
});
...
test('9999/10000/10001 rotate through the real controller and bounded batch boundary', async () => {
  ...
  for (let total = 0; total < 10001; ) {
    ...
    const results = await f.bridge.batch(
      map.context,
      Array.from({length: count}, () => resource.url),
    ).promise;
...
test('six-hour concurrent admission rotates once without JS timers or sleeps', async () => {
  ...
  assert.deepEqual(f.sessions, [
    'ses_test_1',
    ...Array.from({length: nativeAdmissionLimits.batchSize}, () => 'ses_test_2'),
  ]);
  assert.deepEqual(f.bootstraps, ['ses_test_1', 'ses_test_2']);
  await f.owner.dispose();
});
