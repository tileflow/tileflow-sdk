...
test('lowered road branches keep overlay boundaries and POI topmost priorities current', () => {
  ...
  const result = finalizeNativeHostedProvenance(
    original,
    candidate,
  ...
  assert.equal(
    metadata['tileflow:overlay-placement-manifest'].anchors['below-roads'],
    candidate.layers[1]!.id,
  );
  ...
  assert.equal(
    poi.priority,
    result.layers.findIndex((layer) => layer.id === 'poi'),
  );
...
test('lowering correspondence cannot silently borrow another layer or accept stale provenance', () => {
  ...
  assert.throws(() =>
    finalizeNativeHostedProvenance(
      original,
...
test('ordered numeric spans are the only lowering correspondence', () => {
  ...
  for (const invalid of [
    ...
    assert.throws(
      () => finalizeNativeHostedProvenance(original, lowered.styles.main!.light!, invalid),
      {
...
test('multiple expanded input spans preserve intervening POI identity and final priority', () => {
  ...
  assert.throws(() =>
    finalizeNativeHostedProvenance(original, lowered.styles.main!.light!, [...spans].reverse()),
  );
