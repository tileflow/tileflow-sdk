...
function fixture() {
	...
	const port = () => createNativeInteractionQuery(style, () => true, async () => [
		{type: 'Feature', id: 1, properties: {name: 'Place'}},
	]);
	return {style, port};
}
...
test('touch renewal cannot accept a different style object or a retired backing owner', () => {
	...
	const old = f.port();
	owner.replaceStyle(old.lease);
	const other = createNativeInteractionQuery({...f.style}, () => true, async () => []);
