...
function reference(target: NativeInteractionPopup['target']): TileflowInteractionTargetRef | null {
	...
	return target.feature.id === undefined ? null : {
		kind: 'semantic-feature', domain: 'poi', featureId: target.feature.id,
	};
}
...
const ignoreFailure = (work: () => unknown) => {
	...
	catch { /* Observers do not own the interaction lifetime. */ }
};
...
export function createNativeInteractionOwner(
	...
	const preferenceFor = (state: TileflowInteractionState) => {
		...
		if (snapshot.popup && tileflowInteractionTargetRefsEqual(reference(snapshot.popup.target), state.popup)) {
			return snapshot.popup;
		}
		return requestedPopup && tileflowInteractionTargetRefsEqual(reference(requestedPopup.target), state.popup)
			? requestedPopup : null;
	};
	const resolvePopup = (
		...
		const matching = preference && tileflowInteractionTargetRefsEqual(reference(preference.target), target)
			? preference : null;
		if (target.kind === 'annotation') {
			...
			if (matching?.target.bindingId !== undefined && (!binding || !binding.popup ||
				binding.target.kind !== 'annotation' || binding.target.id !== target.id)) return null;
		...
		if (target.kind !== 'semantic-feature' || target.domain !== 'poi' ||
			matching?.target.kind !== 'semantic-feature') return null;
		const binding = data.bindings.find((value) => value.id === matching.target.bindingId);
		if (!binding?.popup || !nativePoiBindingMatches(binding, matching.target) ||
			!semantic.supports(matching.target)) return null;
	...
	const commit = (
		...
		if ((ref === null && previous.state.popup !== null) || (requestedPopup && ref !== null &&
			tileflowInteractionTargetRefsEqual(reference(requestedPopup.target), ref))) requestedPopup = null;
		const diagnostics: TileflowInteractionDiagnostic[] = [];
		const semanticBindings = data.bindings.some((binding) =>
		...
		if (previous.annotations === prepared.annotations && previous.bindings === prepared.bindings &&
			previous.state === state && nativeInteractionValuesEqual(previous.popup, popup) &&
			nativeInteractionValuesEqual(previous.diagnostics, diagnostics) && !requestClose) return;
	...
	const report = (code: TileflowInteractionDiagnostic['code']) => {
		...
		commit(prepared, planNativeAnnotations(prepared.annotations, prepared.annotations), preferenceFor(prepared.state), version, 'programmatic');
	};
	const update = (input: NativeInteractionInput) => {
		...
		const changed = !prepared || next.annotations !== prepared.annotations ||
			next.bindings !== prepared.bindings || next.state !== prepared.state;
		const currentPopup = snapshot.popup;
		const same = !changed && !runtimeDiagnostics.length &&
		...
		runtimeDiagnostics = [];
		commit(next, planNativeAnnotations(prepared?.annotations ?? [], next.annotations), preferenceFor(next.state), version, 'programmatic');
	};
	...
	const dispose = () => {
		...
		requestedPopup = null; blocked = null; staleRequest = null; prepared = undefined;
	...
	return Object.freeze({
		...
		replaceStyle(next?: NativePoiStyleLease): void {
			...
			const preference = snapshot.popup?.target.kind === 'semantic-feature' ? null : preferenceFor(prepared.state);
		...
		activateAnnotation(id: string, inputModality: string): void {
			...
			const binding = prepared.bindings.find((value) => value.target.kind === 'annotation' && value.target.id === id);
			const target: TileflowResolvedAnnotationTarget = popup?.target.kind === 'annotation' ? popup.target :
				freezeNativeInteractionValue({kind: 'annotation', annotation, coordinate: annotation.coordinate,
