...
test('invalid manifest identity is rejected before any cache hydration or fetch', async () => {
	await withIconSetFixture(async (cwd) => {
		...
		let calls = 0;
		await assert.rejects(loadTileflowIconSetArtifact({...set.pin, contentHash: 'f'.repeat(64)}, {cacheRoot: cwd, fetch: async () => { calls += 1; throw new Error('Must not fetch'); }}), {code: 'ICON_LOCK_INVALID'});
...
test('implicit lock reads are watchable and a changed pin is acquired as a new snapshot', async () => {
	await withIconSetFixture(async (cwd) => {
		...
		const first = await composeTileflowIconSources([iconSet('@acme/brand')], {cwd, cacheRoot: cwd, offline: true});
		...
		await writeFile(path, await serializeTileflowIconsLockfile(lock));
		const second = await composeTileflowIconSources([iconSet('@acme/brand')], {cwd, cacheRoot: cwd, offline: true});
...
test('oversized intrinsic patterns are rejected before allocating rendered output', async () => {
	await withIconSetFixture(async (cwd) => {
		...
		await writeFile(join(cwd, 'icons', 'large.pattern.svg'), '<svg xmlns="http://www.w3.org/2000/svg" width="512" height="2048"><rect width="512" height="2048" fill="red"/></svg>');
		await assert.rejects(composeTileflowIconSources(['./icons'], {cwd}), /paired sprite capacity before rasterization/u);
	});
});
