...
const idsSchema = z
  .array(tileflowIconIdSchema)
  .max(tileflowIconPackageLimits.maxIconCount)
  .refine(
    (ids) =>
      new Set(ids).size === ids.length &&
      ids.every((id, index) => index === 0 || compareCodeUnits(ids[index - 1]!, id) < 0),
...
const packageContributorSchema = z
  .object({
...
export const tileflowRenderedIconIdentitySchema = z
  .object({
    ...
    width: z
      ...
      .int()
      .positive()
      .max(tileflowIconPackageLimits.maxAtlasDimension / 2),
    height: z
      .number()
      .int()
      .positive()
    ...
    pixelSha256: z
      .object({
        oneX: tileflowIconPackageContentHashSchema,
        twoX: tileflowIconPackageContentHashSchema,
      })
      .strict(),
  })
  .strict();
...
export const tileflowIconCompositionSchema = z
  .object({
    ...
    contributors: z
      .array(
        z.discriminatedUnion('kind', [
          localContributorSchema,
          packageContributorSchema,
          setContributorSchema,
        ]),
      )
      .min(1)
    ...
    winners: z
      .array(
        tileflowRenderedIconIdentitySchema.omit({kind: true}).extend({
          contributor: z
            .number()
            .int()
            .min(0)
            .max(tileflowIconSourceLimit - 1),
        }),
      )
      .min(1)
      .max(tileflowIconPackageLimits.maxIconCount),
  ...
  .superRefine((receipt, context) => {
    ...
    for (const [ordinal, contributor] of receipt.contributors.entries()) {
      if (contributor.kind === 'icon-set') {
        if (
          references.has(contributor.reference) ||
          sets.has(contributor.setId) ||
          versions.has(contributor.versionId)
        ) {
          context.addIssue({
            code: 'custom',
            path: ['contributors', ordinal],
            message: 'Duplicate icon set dependency',
          });
        }
    ...
    if (teams.size !== 1 || teamSlugs.size !== 1)
      context.addIssue({
        code: 'custom',
        path: ['contributors'],
        message: 'A shared composition must contain exact dependencies from one Team',
      });
    const names = [...expected.keys()].sort(compareCodeUnits);
    if (
      names.length !== receipt.winners.length ||
      receipt.winners.some(
        (winner, index) =>
          names[index] !== winner.id || expected.get(winner.id) !== winner.contributor,
      )
    ) {
      context.addIssue({
        code: 'custom',
        path: ['winners'],
        message: 'Winners must exactly cover the sorted last-wins export closure',
      });
    }
    if (
      new TextEncoder().encode(serializeCanonicalJson(receipt)).byteLength >
      tileflowIconCompositionMaximumBytes
    ) {
...
export function parseTileflowIconComposition(input: unknown): TileflowIconCompositionV1 {
  ...
  if (!parsed.success)
    throw new TileflowIconSetError(
      ...
      'Invalid ordered icon composition receipt',
      {cause: parsed.error},
    );
  return parsed.data;
}
...
export async function hashTileflowIconComposition(
