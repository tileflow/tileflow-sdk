...
test('constructs only the accepted native bootstrap request and publishes secret-free state', async () => {
	...
	assert.deepEqual(authority.resourceScopes, ['style', 'tilejson', 'tile', 'sprite', 'glyph', 'font']);
	...
	assert.equal(Object.keys(call.init).sort().join(','), 'body,headers,method,signal');
...
test('normalizes Surface and rejects malformed credentials and non-canonical trusted origins before fetch', async () => {
	...
	for (const binding of [
		...
		assert.throws(
			() => createHostedNativeSessionController({binding, fetch: queue.fetch, now: createClock().now, sessionIdFactory: createIds()}),
...
test('enforces actual-byte, UTF-8, JSON and strict success-response bounds', async () => {
	...
	const cases = [
		{response: response(201, '', {chunks: [oversized]}), code: 'NATIVE_SESSION_RESPONSE_TOO_LARGE'},
		{response: response(201, '', {chunks: [invalidUtf8]}), code: 'NATIVE_SESSION_RESPONSE_UTF8_INVALID'},
		{response: response(201, '{'), code: 'NATIVE_SESSION_RESPONSE_JSON_INVALID'},
		{response: response(201, success({unexpected: true})), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
		{response: response(201, success({grant: 'x'.repeat(24_577)})), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
		{response: response(201, success({resourceOrigins: ['https://api.tileflow.test/path']})), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
		{response: response(201, success({resourceOrigins: ['https://api.tileflow.test', 'https://tiles.tileflow.test', 'https://third.tileflow.test']})), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
		{response: response(201, success({resourceScopes: ['style', 'admin']})), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
		{response: response(201, success({tilesetIds: Array.from({length: 19}, (_, index) => `tls_${index}`)})), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
		{response: response(201, success(), {headers: {'cache-control': 'public, max-age=60'}}), code: 'NATIVE_SESSION_RESPONSE_INVALID'},
	];
...
test('rejects binding/time mismatches and never extends lifetime when the device clock moves backwards', async () => {
	...
	for (const overrides of mismatchCases) {
		const controller = createHostedNativeSessionController({
			binding: hosted(), fetch: createFetchQueue([response(201, success(overrides))]).fetch,
			now: createClock().now, sessionIdFactory: createIds(),
		});
	...
	const queue = createFetchQueue([
		...
		response(201, success({serverTime: '2026-09-15T20:15:00.000Z', issuedAt: '2026-09-15T20:15:00.000Z', expiresAt: '2026-09-15T20:30:00.000Z'})),
	]);
...
test('coalesces concurrent bootstrap and ordinary refresh while preserving the server-returned Surface and session', async () => {
	const clock = createClock();
	...
	assert.equal(queue.calls.length, 1);
	first.resolve(response(201, success({surfaceId: 'server-surface'})));
	const [a, b] = await Promise.all([one, two]);
	assert.equal(a?.grant, b?.grant);
	assert.equal(a?.sessionId, 'ses_test_1');
	assert.equal(a?.surfaceId, 'server-surface');

	clock.advance(14 * 60_000 + 31_000);
	const refreshes = await Promise.all([controller.acquire(), controller.acquire(), controller.acquire()]);
	assert.equal(queue.calls.length, 2);
	assert.equal(refreshes.every((value) => value?.sessionId === 'ses_test_1'), true);
	assert.deepEqual(JSON.parse(queue.calls[1].init.body), {mapId, sessionId: 'ses_test_1', surfaceId: 'server-surface'});
});
...
test('retries only the exact restart response once with a new session identity', async () => {
	...
	for (const [status, body, code] of [
		[409, {code: 'COMMERCIAL_SESSION_RESTART_REQUIRED', retryWithNewSession: false, sessionId: 'ses_test_1'}, 'NATIVE_SESSION_REJECTED'],
		[409, {code: 'COMMERCIAL_SESSION_RESTART_REQUIRED', retryWithNewSession: true, sessionId: 'ses_wrong'}, 'NATIVE_SESSION_REJECTED'],
		...
		const candidate = createHostedNativeSessionController({binding: hosted(), fetch: local.fetch, now: createClock().now, sessionIdFactory: createIds()});
...
test('direct sources never bootstrap, same Hosted binding retains identity, and a different Map cannot reuse authority', async () => {
	...
	const controller = createHostedNativeSessionController({
		binding: {kind: 'direct'}, fetch: queue.fetch, now: createClock().now, sessionIdFactory: createIds(),
	});
