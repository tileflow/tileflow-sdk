import {type TileflowNativeAbortSignal, TileflowNativeSourceError} from './native-source-types';
...
export class NativeSourceCancellation {
  ...
  cancel(): void {
    ...
    for (const listener of listeners) {
      try {
        ...
        /* Cleanup cannot undo retirement. */
      }
    }
  }
  ...
  race<T>(promise: Promise<T>): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      const off = this.onCancel(() =>
        reject(new TileflowNativeSourceError('NATIVE_SOURCE_ABORTED', 'signal')),
      );
      promise.then(
        (value) => {
          off();
