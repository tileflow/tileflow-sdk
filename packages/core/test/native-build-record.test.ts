...
test('binds the native-v1 preparation ABI and exact transformation scope in a strict v2 record', () => {
  ...
  for (const invalid of [
    {...record, schemaVersion: 1}, {...record, preparationVersion: 'unknown'},
    ...
    {...record, transformations: [{...transformation(), layers: [
      {inputLayer: 1, outputStart: 2, outputCount: 2, properties: ['line-cap']},
    ]}]},
    {...record, transformations: [{...transformation(), layers: [
...
test('orders style receipts deterministically without mutating callers or conflating hashes', () => {
  ...
  assert.notEqual(record.transformations[0]!.inputStyleSha256, record.transformations[0]!.loweredStyleSha256);
});
