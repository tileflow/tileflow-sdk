...
const policy = {mapId, resourceOrigins: [apiOrigin], resourceScopes: ['style', 'font'] as const, tilesetIds: []};
const faces = [{id: 'Brand Regular', family: 'Brand', url, format: 'ttf' as const, style: 'normal' as const, weight: '400' as const}];
const style = {
	version: 8, sources: {},
	layers: [{id: 'text', type: 'symbol', layout: {'text-field': 'A', 'text-font': ['Brand Regular']}}],
	'font-faces': {'Brand Regular': [{url, 'font-family': 'Brand', 'font-style': 'normal', 'font-weight': 400}]},
};

function input(value: Record<string, unknown> = style) {
	return {styleUrl, policy, fontFaces: faces, current: () => true,
		async accept() {}, discriminate: (value: string) => `${value}?__tf_native_context=one`,
		async read(request: string) { return {url: request, value: JSON.parse(JSON.stringify(value)), bytes: JSON.stringify(value).length}; },
	};
}
...
test('managed fonts retain the native-v1 array shape and exact font attributes', async () => {
	...
	assert.deepEqual(wire['Brand Regular'], [{url: `${url}?__tf_native_context=one`, 'font-family': 'Brand', 'font-style': 'normal', 'font-weight': 400}]);
...
test('font metadata cannot silently point at another resource or unsupported native format', async () => {
	await assert.rejects(projectNativeResources({...input(), fontFaces: [{...faces[0], url: `${apiOrigin}/fonts/fb_other/regular.ttf`}]}));
	for (const value of [
		{'Brand Regular': url},
