...
test('keeps the editorial city recipe on the versioned Streets contract', () => {
  ...
  assert.equal(style.metadata?.['tileflow:basemapVersion'], 1);
...
test('workspace cartography commands prepare the packaged CLI before running it', async () => {
  ...
  for (const sceneName of Object.keys(project.scenes)) {
    assert.match(workspacePackage.scripts['visual:cartography'] ?? '', new RegExp(sceneName));
    assert.match(
      workspacePackage.scripts['visual:cartography:update'] ?? '',
      new RegExp(sceneName),
    );
  }
});
