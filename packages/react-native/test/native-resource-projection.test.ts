...
const policy = Object.freeze({
  mapId,
  resourceOrigins: [apiOrigin, 'https://tiles.tileflow.test'],
...
function fixture(
  ...
  const ports = {
    ...
    discriminate(url: string) {
      return `${url}${url.includes('?') ? '&' : '?'}__tf_native_context=one`;
    },
...
test('projects exact style TileJSON tile sprite glyph and font closure without a second document fetch', async () => {
  ...
  const source = {
    ...
    layers: [
      {
        ...
        source: 'first',
        'source-layer': 'place',
        layout: {'text-field': 'A', 'text-font': ['Noto Sans Regular']},
      },
    ],
    ...
    'font-faces': {
      'Noto Sans Regular': [
        {url: fontUrl, 'font-family': 'Noto Sans', 'font-style': 'normal', 'font-weight': 400},
      ],
    },
  };
  ...
  const f = fixture(
    ...
    {
      tilejson: '3.0.0',
      tiles: ['https://tiles.tileflow.test/tiles/world/release/{z}/{x}/{y}.pbf?revision=one'],
      minzoom: 0,
      maxzoom: 14,
    },
    [
  ...
  assert.deepEqual(f.accepted.get(`${apiOrigin}/fonts/{fontstack}/{range}.pbf`)!.fontStacks, [
    'Noto Sans Regular',
  ]);
...
test('foreign resources retain all URL identity without receiving authority', async () => {
  ...
  const f = fixture({
    version: 8,
    sources: {foreign: {type: 'vector', tiles: [url]}},
    layers: [],
    sprite: 'https://outside.example.test/sprite?x=2',
  });
  const result = await projectNativeResources(f.ports);
  assert.deepEqual(
    (result.style.sources as Record<string, Record<string, unknown>>).foreign.tiles,
    [url],
  );
...
test('owned origin does not authorize unknown paths other Maps or foreign tilesets', async () => {
  ...
  await assert.rejects(
    projectNativeResources({
      ...f.ports,
      styleUrl: `${apiOrigin}/maps/map_fedcba9876543210/light.json`,
    }),
  );
...
test('rejects recursive ambiguous oversized or dynamically unbounded graph inputs', async () => {
  ...
  const f = fixture(
    {version: 8, sources: {bad: {type: 'vector', url: '/tiles/world/tiles.json'}}, layers: []},
    {url: '/tiles/world/tiles.json'},
  );
  await assert.rejects(projectNativeResources(f.ports));
...
test('non-session projection never creates a protected catalog or context discriminator', async () => {
  const f = fixture({
    version: 8,
    sources: {tiles: {type: 'vector', tiles: ['/tiles/world/{z}/{x}/{y}.pbf']}},
    layers: [],
  });
