    ...
    test(`invalid replacements do not revive superseded sources: ${order}; rejects=${staleRejects}`, async () => {
      ...
      for (const index of order) {
        if (index < 2 && staleRejects)
          requests[index]!.response.reject(new Error('Remote private details.'));
        else requests[index]!.response.resolve(transport().response);
      }
      await Promise.all([first, second, third]);
      assert.deepEqual(
        states.filter((state) => state.status === 'ready').map((state) => state.generation),
        [5],
      );
      assert.deepEqual(
        states.filter((state) => state.status === 'error').map((state) => state.generation),
        [2, 4],
      );
...
test('reentrant replacement suppresses older notifications and repeated disposal stays inert', async () => {
  ...
  controller.subscribe((state) => {
    if (state.status === 'loading' && state.generation === 1)
      replacement = controller.replace(source);
  });
  controller.subscribe((state) => {
  ...
  controller.subscribe(() => {
    throw new Error('Unexpected notification after disposal.');
  });
