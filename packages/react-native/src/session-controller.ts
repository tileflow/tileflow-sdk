...
type SafeError = Readonly<{code: HostedNativeSessionErrorCode; kind: HostedNativeSessionErrorKind}>;
...
export function createHostedNativeSessionController(input: {
	...
	if (!isRecord(input) || typeof input.fetch !== 'function' || typeof input.now !== 'function' || typeof input.sessionIdFactory !== 'function') {
	...
	function rotateIfRequired(at: number) {
		...
		if (at - activeSession.startedAt < ROTATION_AGE_MS && activeSession.requestCount < ROTATION_REQUEST_LIMIT) return;
		const previous = activeSession;
		activeSession = createSession(previous.authority?.public.surfaceId ?? previous.requestedSurfaceId, at);
	...
	async function ensureAuthority(session: SessionRecord, at: number) {
		...
		if (binding.kind !== 'hosted') throw new HostedNativeSessionError('NATIVE_SESSION_REPLACED');
	...
	async function fetchResponse(hostedBinding: HostedBinding, session: SessionRecord, abort: SessionAbortController) {
		...
		try {
			pending = Promise.resolve(input.fetch(`${hostedBinding.apiOrigin}/v1/sessions/start`, init));
		} catch {
	...
	return Object.freeze({
		...
		subscribe(listener) {
			if (typeof listener !== 'function') throw new HostedNativeSessionError('NATIVE_SESSION_INPUT_INVALID');
...
function normalizeBinding(value: HostedNativeSessionBinding): Binding {
	try {
		...
		if (
			value.kind !== 'hosted' ||
			Object.keys(value).some((key) => !['kind', 'apiOrigin', 'credential', 'mapId', 'surfaceId'].includes(key)) ||
...
function sameHostedBinding(left: HostedBinding, right: HostedBinding) {
	return left.apiOrigin === right.apiOrigin && left.credential === right.credential && left.mapId === right.mapId;
}
...
function snapshotIdle(binding: Binding, session: SessionRecord | null, lifecycle: Lifecycle): HostedNativeSessionState {
	return binding.kind === 'direct' || !session
		? Object.freeze({status: 'idle', source: 'direct', lifecycle})
		: Object.freeze({status: 'idle', source: 'hosted', lifecycle, mapId: binding.mapId, sessionId: session.sessionId});
}
...
function parseOrigins(value: unknown) {
	if (!Array.isArray(value) || value.length < 1 || value.length > 2) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
	const result: string[] = [];
	for (const item of value) {
		if (typeof item !== 'string') throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
		let origin: string;
		try {
		...
		if (result.includes(origin)) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
		result.push(origin);
	}
	return result;
}
...
function parseScopes(value: unknown): NativeSessionResourceScope[] {
	if (!Array.isArray(value) || value.length < 1 || value.length > ALLOWED_SCOPES.size) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
	const result: NativeSessionResourceScope[] = [];
	for (const item of value) {
		if (typeof item !== 'string' || !ALLOWED_SCOPES.has(item as NativeSessionResourceScope) || result.includes(item as NativeSessionResourceScope)) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
...
function parseTilesets(value: unknown) {
	if (!Array.isArray(value) || value.length > MAX_TILESETS) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
	const result: string[] = [];
	for (const item of value) {
		if (typeof item !== 'string' || !COMPACT_ID.test(item) || result.includes(item)) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
		result.push(item);
	}
	return result;
}
...
function parseExactIso(value: unknown) {
	if (typeof value !== 'string' || !EXACT_ISO.test(value)) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
	const parsed = Date.parse(value);
	if (!Number.isFinite(parsed) || new Date(parsed).toISOString() !== value) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
	return parsed;
}
...
async function readBoundedResponse(response: HostedNativeSessionFetchResponse, abort: SessionAbortController) {
	...
	try {
		if (!response || !Number.isSafeInteger(response.status) || !response.headers || typeof response.headers.get !== 'function' || !response.body || typeof response.body.getReader !== 'function') throw new Error();
	...
	try {
		for (;;) {
			...
			if (next.done) break;
			if (!(next.value instanceof Uint8Array)) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_INVALID');
			size += next.value.byteLength;
			if (size > RESPONSE_BYTE_LIMIT) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_TOO_LARGE');
...
function decodeUtf8(bytes: Uint8Array) {
	...
	for (let index = 0; index < bytes.length; ) {
		...
		if (index + remaining > bytes.length) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_UTF8_INVALID');
		for (let count = 0; count < remaining; count += 1) {
			const byte = bytes[index++];
			if ((byte & 0xc0) !== 0x80) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_UTF8_INVALID');
			codePoint = (codePoint << 6) | (byte & 0x3f);
		}
		if (codePoint < minimum || codePoint > 0x10ffff || (codePoint >= 0xd800 && codePoint <= 0xdfff)) throw new HostedNativeSessionError('NATIVE_SESSION_RESPONSE_UTF8_INVALID');
...
function normalizeError(error: unknown, fallback: HostedNativeSessionErrorCode = 'NATIVE_SESSION_RESPONSE_INVALID') {
