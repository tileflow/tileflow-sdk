...
test('a native cancel callback that starts a newer query cannot be overwritten by the cancelled caller', async () => {
	...
	const native = queryFixture(); adapter.replaceStyle(native.lease);
	...
	native.setQuery(() => ({result: raw.promise, cancel() {
		native.setQuery((request) => ({result: Promise.resolve({request, features: [poiFeature('newest')]}), cancel() {}}));
...
test('a style lease can retire reentrantly during ownership inspection without exposing native errors', async () => {
	...
	let retire = false;
	adapter.replaceStyle({...native.lease, isCurrent() {
...
test('throwing current-style inspection is a structured diagnostic rather than a leaked native exception', async () => {
	const adapter = createNativePoiAdapter(); const native = queryFixture();
	let broken = false;
	adapter.replaceStyle({...native.lease, isCurrent() {
...
test('controlled semantic activation stays pending until acknowledgement and cannot survive source retirement', async () => {
	...
	const native = queryFixture(); native.setFeatures([poiFeature('feature')]);
	const owner = createNativeInteractionOwner({interactions: [poiBinding], interactionState: {popup: null}}, {
...
test('hostile input replacement cannot overwrite a newer owner reconciliation', () => {
	...
	const raw = new Proxy({}, {getPrototypeOf() {
		owner.update({annotations: [{id: 'newest', kind: 'marker', ariaLabel: 'New', coordinate: [1, 2]}]});
...
test('an activation observer starting a newer touch cancels the older popup request', async () => {
	const native = queryFixture(); native.setFeatures([poiFeature('first')]);
	...
	await replacement;
	assert.deepEqual(owner.getSnapshot().state, {popup: {kind: 'semantic-feature', domain: 'poi', featureId: 'second'}});
	owner.dispose();
});
