...
export type ManifestStringOperations = {
	...
	minLength<T extends string | unknown[]>(schema: ManifestSchema<T>, minimum: number): ManifestSchema<T>;
	maxLength<T extends string | unknown[]>(schema: ManifestSchema<T>, maximum: number): ManifestSchema<T>;
	regex(schema: ManifestSchema<string>, pattern: RegExp, message: string): ManifestSchema<string>;
	refine<T>(schema: ManifestSchema<T>, check: (value: T) => boolean, message: string): ManifestSchema<T>;
};
...
export type ManifestSchemaOperations = ManifestStringOperations & {
	...
	record<T>(key: ManifestSchema<string>, value: ManifestSchema<T>): ManifestSchema<Record<string, T>>;
