...
test('native uses the canonical manifest grammar and preserves delivery/view/theme constraints', () => {
  ...
  for (const styleUrl of [
    ...
    'style.json',
    '//foreign.test/s',
    'https://bücher.example.test/style.json',
    'https://host.test/%2Fsecret',
    'https://user:secret@host.test/s',
    'file:///s',
    'https://host.test/s#fragment',
  ]) {
    const value = manifest();
    value.maps.streets.themes.light.styleUrl = styleUrl;
    compare(value);
  }
  for (const pitch of [-1, 0, 85, 86]) {
    const value = manifest();
...
test('native retains duplicate font and unknown-field rejection at every nested schema level', () => {
  for (const target of ['view', 'theme', 'font', 'systemThemes']) {
    ...
    Object.assign(value.maps.streets.themes.light, {
      fontFaces: [{family: 'Fixture', source: './font.ttf'}],
    });
    ...
    Object.assign(
      target === 'view'
        ...
        : target === 'systemThemes'
          ? entry.systemThemes
          : target === 'font'
            ? entry.themes.light.fontFaces[0]
            : entry.themes.light,
      {extra: true},
    );
    compare(value);
  ...
  Object.assign(value.maps.streets.themes.light, {
    fontFaces: [
      {family: 'Fixture', source: './a.ttf'},
      {family: 'Fixture', source: './b.ttf', style: 'normal', weight: '400'},
    ],
  });
  compare(value);
});
