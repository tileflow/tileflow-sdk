...
export function createNativeCameraPort(native: NativeCamera) {
	...
	const cancel = (run: Run) => {
		...
		if (running === run) {
			try { void Promise.resolve(native.cancelCamera(run.command.token.sequence)).catch(() => undefined); }
			catch { /* Native lifetime remains owned by the surface teardown. */ }
		}
		idle();
	};
	const drain = () => {
		...
		void Promise.resolve().then(async () => {
			scheduled = false;
			if (running || disposed) { idle(); return; }
			const run = pending;
			pending = undefined;
			if (!run || run.stopped) { idle(); return; }
			running = run;
			try {
				const receipt = await native.applyCamera(run.command.token.sequence, run.command.view);
				if (disposed || run.stopped) return;
				if (!receipt || receipt.command !== run.command.token.sequence ||
					!sameCameraView(snapshotCameraView(receipt.view), run.command.view)) throw failure();
				run.resolve();
			} catch { if (!run.stopped) run.reject(failure()); }
			finally { running = undefined; if (pending) drain(); else idle(); }
		});
	};
	...
	return Object.freeze({
		...
		whenIdle(): Promise<void> {
			return running || pending || scheduled ? new Promise((resolve) => waiters.add(resolve)) : Promise.resolve();
		},
		dispose(): void {
			if (disposed) return;
			disposed = true;
