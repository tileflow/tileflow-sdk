...
import type {MapCameraProps, MapErrorEvent, MapLoadEvent, MapReadinessChangeEvent, MapSelection, MapThemeChangeEvent} from './contract';
...
export type NativeRendererEvent = MapErrorEvent | MapLoadEvent | MapReadinessChangeEvent | MapThemeChangeEvent;
export type NativeRendererSurfaces = Readonly<{
	attach(root: number, notify: (event: NativeSurfaceEvent) => void, failure: () => void): Promise<NativeSurface>;
	retireRoot(root: number): Promise<void>;
}>;
const blank = Object.freeze({version: 8, sources: Object.freeze({}), layers: Object.freeze([])});
const selection = (target: NativeRendererTarget): MapSelection => Object.freeze({
...
export function createNativeRendererOwner(
	...
	ports: Readonly<{surfaces: NativeRendererSurfaces; emit(event: NativeRendererEvent): void; changed(): void}>,
) {
	let disposed = false;
	let foreground = true;
	let revision = 0;
	...
	const emit = (event: NativeRendererEvent) => {
		if (disposed) return;
		try { void Promise.resolve(ports.emit(Object.freeze(event))).catch(() => undefined); } catch { /* Observer failure is not a renderer failure. */ }
	};
	const readiness = createNativeReadiness((status) => {
		if (disposed) return;
		const version = transaction;
		if (status === 'ready' && pendingSuccess && !rollback) {
			committed = active; pendingSuccess = false;
			emit({type: 'readiness-change', generation: eventGeneration, status});
			if (!disposed && transaction === version) emit({type: 'theme-change', phase: 'ready', generation: eventGeneration, map: active.source.map.name, currentTheme: selection(active).theme});
	...
	const cameraPort = createNativeCameraPort({
		applyCamera(command, view) {
			if (!surface || disposed || !loaded) return Promise.reject(new NativePreparationError());
			return surface.applyCamera(command, view);
		},
		cancelCamera(command) { return surface ? surface.cancelCamera(command) : Promise.resolve({cancelled: true}); },
	});
	const camera = createMapCameraController(cameraPort, () => fail());

	function invalidate() {
		barrierEpoch++; needsCommit = true; readiness.invalidate();
		changed();
	}
	function apply(target: NativeRendererTarget, restoring: boolean) {
		if (disposed) return;
		const version = ++transaction;
		if (styleSequence >= Number.MAX_SAFE_INTEGER) { readiness.fail(); return; }
		active = target; rollback = restoring; pendingSuccess = !restoring;
		preloading = false; loaded = false; gesture = undefined; settlement = undefined;
		token = `style_${++styleSequence}`;
		const expected = token;
		barrierEpoch++; needsCommit = true;
		readiness.begin(expected);
		if (!surface) return;
		const native = surface;
		const work = Promise.resolve().then(async () => {
			if (disposed || version !== transaction) return;
			await native.expectStyle(expected);
			if (disposed || version !== transaction || surface !== native) return;
			const layers = target.style.layers;
			if (!Array.isArray(layers) || layers.length >= 4096 || layers.some((layer) =>
				!layer || typeof layer !== 'object' || typeof layer.id !== 'string' || layer.id.startsWith('__tileflow_native_style_'))) throw new NativePreparationError();
			const style = freezeNativePreparedJson({...target.style, layers: [...layers, {
				id: `__tileflow_native_style_${expected}`, type: 'background', layout: {visibility: 'none'}, paint: {'background-opacity': 0},
			}]});
			snapshot = Object.freeze({key, style, revision: ++revision});
			if (!restoring) emit({type: 'theme-change', phase: 'applying', generation: eventGeneration, map: target.source.map.name,
				targetTheme: selection(target).theme, ...(committed ? {currentTheme: selection(committed).theme} : {})});
	...
	function fail() {
		...
		else { ++transaction; needsCommit = false; readiness.fail(); }
	}
	function native(event: NativeSurfaceEvent) {
		if (disposed || !surface || event.surface !== surface.id || event.style !== token || !foreground) return;
		const version = transaction;
		if (event.kind === 'error') { fail(); return; }
		if (event.kind === 'invalidate') { readiness.native(event); barrierEpoch++; needsCommit = true; changed(); return; }
		if (event.kind === 'style') {
			...
			try {
				...
				track(cameraPort.whenIdle().then(() => { if (!disposed && version === transaction) invalidate(); }));
			} catch { fail(); }
			return;
		}
		if (event.kind === 'render') { if (!preloading && loaded && !gesture && !settlement) readiness.native(event); return; }
		if (!cameraMounted || !loaded) return;
		if (event.kind === 'gesture-start') {
			const started = camera.startGesture();
			if (!started) return;
			gesture = {native: event.gesture, token: started}; settlement = undefined;
			readiness.invalidate(); needsCommit = true; barrierEpoch++;
			...
			const completed = gesture.token; gesture = undefined; settlement = completed;
	...
	return Object.freeze({
		...
		attach(rootTag: number): Promise<void> {
			...
			if (disposed || !Number.isSafeInteger(rootTag) || rootTag < 1) return Promise.reject(new NativePreparationError());
			root = rootTag;
			...
			attached = pending; track(pending);
			return pending;
		},
		preload(generation: number): void {
			if (disposed) return;
			eventGeneration = generation; preloading = true;
			invalidate();
			emit({type: 'theme-change', phase: 'preloading', generation, map: active.source.map.name,
				targetTheme: selection(active).theme, ...(committed ? {currentTheme: selection(committed).theme} : {})});
		},
		...
		reuseTarget(source: NativeRendererTarget['source']): void {
			...
			active = {...committed, source}; committed = active; eventGeneration = source.generation;
			preloading = false; pendingSuccess = true; rollback = false; invalidate();
		},
		preparationFailed(generation: number): void {
			if (disposed) return;
			eventGeneration = generation; preloading = false;
			emit({type: 'theme-change', phase: 'error', generation, map: active.source.map.name,
				...(committed ? {currentTheme: selection(committed).theme} : {})});
			if (!committed) { readiness.fail(); return; }
			if (active !== committed) apply(committed, true);
		...
		afterCommit(props: MapCameraProps): void {
			...
			try {
				...
				const completed = settlement; settlement = undefined;
				if (completed) camera.settleGesture(completed);
			} catch { fail(); return; }
			if (!surface || !loaded || !foreground || preloading || gesture || !needsCommit || barrier) return;
			const nativeSurface = surface; const expected = token; const epoch = barrierEpoch; const version = transaction;
			...
			}).catch(() => { if (!disposed && version === transaction && epoch === barrierEpoch && foreground) fail(); });
			barrier = work;
			track(work.then(() => {
				if (barrier === work) barrier = undefined;
				if (!disposed && needsCommit && foreground && loaded && !gesture && !preloading) changed();
			}));
		},
		background(): void {
			if (disposed) return;
			foreground = false; gesture = undefined; settlement = undefined; invalidate();
		},
		resume(): void {
			if (disposed) return;
			foreground = true;
		...
		dispose(): Promise<void> {
			...
			disposed = true; ++transaction; ++barrierEpoch;
			readiness.dispose(); camera.dispose(); cameraPort.dispose(); gesture = undefined; settlement = undefined;
