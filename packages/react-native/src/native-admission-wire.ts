...
} from './native-bootstrap-contract';
import type {
...
function decodeBoundedBase64(value: unknown): Uint8Array {
  if (
    typeof value !== 'string' ||
    value.length > Math.ceil(nativeBootstrapLimits.responseBytes / 3) * 4 ||
...
function responseFromWire(
  ...
  if (
    ...
    reply.status < 100 ||
    reply.status > 599 ||
    typeof reply.cacheControl !== 'string' ||
    reply.cacheControl.length > 1024
  )
    throw invalid();
...
export function createNativeAdmissionWire(
  ...
  const bridge: NativeAdmissionBridge = Object.freeze({
    ...
    completeBatch: (id, context, generation, batch, results) =>
      safe(() => native.completeBatch(id, context, generation, batch, results)),
    remove(id) {
  ...
  function fetchForContext(context: string): HostedNativeSessionFetch {
    ...
    return (url, init) => {
      ...
      if (reservations.size >= nativeBootstrapLimits.queueDepth)
      ...
      if (
        ...
        init.body.length > nativeBootstrapLimits.requestBytes ||
        typeof credential !== 'string' ||
        !/^tf_public_[0-9a-f]{48}$/u.test(credential) ||
      ...
      return new Promise<HostedNativeSessionFetchResponse>((resolve, reject) => {
        ...
        const abort = () => {
          ...
          clearBody?.();
          bodyFinished = true;
          if (dispatched && !nativeFinished)
            void safe(() => native.cancelBootstrap(installed.id, context, request)).catch(
              () => undefined,
            );
          if (!settled) {
            settled = true;
        ...
        try {
          ...
          pending = Promise.resolve(
            native.bootstrap(installed.id, context, request, url, credential, init.body),
          );
        } catch {
          nativeFinished = true;
          settled = true;
