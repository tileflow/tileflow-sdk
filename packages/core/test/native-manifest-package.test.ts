...
test('the built native entry acquires, validates and selects without ambient runtime services', async () => {
  const script = `
    ...
    await controller.replace({map: 'main', manifestUrl: 'https://maps.example.test/manifest.json'});
    assert.equal(controller.state.status, 'ready');
    assert.equal(controller.state.theme.name, 'light');
    assert.equal(Object.hasOwn(controller.state, 'kind'), false);
    assert.equal(Object.hasOwn(controller.state.source, 'kind'), false);
    await controller.replace({map: 'missing', manifestUrl: 'https://maps.example.test/manifest.json'});
