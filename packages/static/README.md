...
Maps renderer. Its top-level `mapId` identifies that resolved destination while `scene.map` retains
the portable map name supplied by the application. It is unrelated to the runtime delivery manifest
generated by application build or deploy workflows.
...
unbounded Starter service reports `remainingUnits: null`. A live
