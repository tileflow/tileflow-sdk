...
import type {HostedNativeSessionAbortSignal, HostedNativeSessionFetchInit} from '../src/session-controller';
import {nativeBootstrapLimits, type NativeAdmissionNativeModule, type NativeBootstrapReply} from '../src/native-bootstrap-contract';
import {AdmissionBridgeDouble} from './native-admission-fixture';
import {apiOrigin, credential, createClock, createIds, deferred, grant, mapId, success} from './session-fixture';

class NativeModuleDouble extends AdmissionBridgeDouble implements NativeAdmissionNativeModule {
	readonly bootstraps: Array<{installation: string; context: string; request: string; url: string; credential: string; body: string}> = [];
	readonly cancellations: string[] = [];
	readonly started = deferred<void>();
	readonly reply = deferred<NativeBootstrapReply>();
	async bootstrap(installation: string, context: string, request: string, url: string, credential: string, body: string) {
		this.bootstraps.push({installation, context, request, url, credential, body});
		this.started.resolve();
		return this.reply.promise;
	}
	async cancelBootstrap(_installation: string, _context: string, request: string): Promise<{cancelled: true}> {
...
function abortFixture() {
	...
	return {signal, abort() { aborted = true; for (const listener of [...listeners]) listener(); }, get listeners() { return listeners.size; }};
}
function init(signal: HostedNativeSessionAbortSignal): HostedNativeSessionFetchInit {
	return {body: JSON.stringify({mapId, sessionId: 'ses_test_1', surfaceId: 'store-locator'}), credentials: 'omit',
		headers: {'Content-Type': 'application/json', 'X-Tileflow-Mobile-Client': credential}, method: 'POST', signal};
}
...
test('abort before native dispatch creates no request; abort during native wait invalidates late bytes', async () => {
	for (const before of [true, false]) {
		...
		const pending = wire.fetchForContext('installation_1.1')(`${apiOrigin}/v1/sessions/start`, init(abort.signal));
		const rejected = assert.rejects(pending, {code: 'NATIVE_ADMISSION_CANCELLED'});
		if (!before) { await native.started.promise; abort.abort(); }
		await rejected;
		native.reply.resolve(response());
...
test('bootstrap errors and malformed or oversized native payloads are secret-free', async () => {
	for (const invalid of ['error', 'base64', 'oversize', 'status'] as const) {
		const {native, wire} = fixture();
		await wire.bridge.install();
		const pending = wire.fetchForContext('installation_1.1')(`${apiOrigin}/v1/sessions/start`, init(abortFixture().signal));
		...
		if (invalid === 'oversize') native.reply.resolve(response('x'.repeat(nativeBootstrapLimits.responseBytes + 1)));
...
test('another context or installation never reuses bootstrap request identity', async () => {
	...
	const pending = [first(`${apiOrigin}/v1/sessions/start`, init(abortFixture().signal)), second(`${apiOrigin}/v1/sessions/start`, init(abortFixture().signal))];
	native.reply.resolve(response());
	...
	await assert.rejects(first(`${apiOrigin}/v1/sessions/start`, init(abortFixture().signal)), {code: 'NATIVE_ADMISSION_CANCELLED'});
...
test('the real controller reaches admission through the native bootstrap and batch boundary', async () => {
	...
	const owner = createNativeAdmissionOwner({bridge: wire.bridge, sessionFetch: wire.fetchForContext});
	const resource = {url: 'https://tiles.tileflow.test/world/0/0/0.pbf', scope: 'tile' as const, tilesetId: 'world'};
	const map = await owner.openMap({binding: {kind: 'hosted', apiOrigin, credential, mapId, surfaceId: 'store-locator'},
		now: clock.now, sessionIdFactory: createIds(), resources: [resource]});
