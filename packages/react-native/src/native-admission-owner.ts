...
export function createNativeAdmissionOwner(options: {
	...
	function install(): Promise<Readonly<{installation: string}>> {
		...
		status = 'installing';
		try {
			installed = bridge.install().then((ack) => {
	...
	async function handleBatch(context: Context, event: Extract<NativeAdmissionEvent, {kind: 'batch'}>) {
		...
		const acquisitions = event.tickets.map(async (ticket): Promise<HostedNativeSessionAuthority | null | false> => {
	...
	function onEvent(event: NativeAdmissionEvent) {
		...
		if (event.kind === 'response') {
			if (Number.isInteger(event.status) && event.status >= 100 && event.status <= 599) {
				try { options.onObservation?.(Object.freeze({context: context.id, status: event.status})); } catch { /* Observers cannot change admission. */ }
			}
			return;
		}
		...
		try {
			...
			context.handle = Object.freeze({
				...
				discriminateForTest(url: string) {
					...
					try { return discriminateNativeResourceForTest(url, context.id); } catch { throw cancel('NATIVE_ADMISSION_INVALID'); }
				},
				retire() {
