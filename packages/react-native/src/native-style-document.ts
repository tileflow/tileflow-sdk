...
export class NativePreparationError extends Error {
	...
	constructor() { super('Native style preparation failed.'); this.name = 'NativePreparationError'; }
}
...
function decode(bytes: Uint8Array): string {
	...
	for (let index = 0; index < bytes.length;) {
		...
		let count: number;
		let minimum: number;
		if (first <= 0x7f) { code = first; count = 0; minimum = 0; }
		...
		if (code < minimum || code > 0x10ffff || (code >= 0xd800 && code <= 0xdfff)) throw new NativePreparationError();
		text += String.fromCodePoint(code);
		if (text.length >= 4096) { pieces.push(text); text = ''; }
	}
	pieces.push(text);
...
export function freezeNativePreparedJson(value: unknown): Readonly<Record<string, unknown>> {
	...
	const visit = (input: unknown, depth: number): void => {
		if (++nodes > nativePreparationLimits.nodes || depth > nativePreparationLimits.depth) throw new NativePreparationError();
		if (input === null || typeof input === 'boolean') return;
		if (typeof input === 'number' && Number.isFinite(input)) return;
		if (typeof input === 'string') return;
		if (!input || typeof input !== 'object' || parents.has(input)) throw new NativePreparationError();
		const array = Array.isArray(input);
		if (!array && Object.getPrototypeOf(input) !== Object.prototype && Object.getPrototypeOf(input) !== null) throw new NativePreparationError();
		const keys = Reflect.ownKeys(input);
		if (array && (input.length > nativePreparationLimits.nodes || Object.keys(input).length !== input.length)) throw new NativePreparationError();
		parents.add(input);
		for (const key of keys) {
			if (array && key === 'length') continue;
			if (typeof key !== 'string' || key === '__proto__' || key === 'toJSON') throw new NativePreparationError();
			const property = Object.getOwnPropertyDescriptor(input, key);
			if (!property || !property.enumerable || !('value' in property)) throw new NativePreparationError();
			visit(key, depth + 1);
	...
	if (!value || typeof value !== 'object' || Array.isArray(value)) throw new NativePreparationError();
	visit(value, 0);
...
export async function readNativeStyleDocument(
	...
	let length = 0;
	try {
		if (!current() || !Number.isSafeInteger(maximumBytes) || maximumBytes < 1 || maximumBytes > nativePreparationLimits.styleBytes) throw new NativePreparationError();
		...
		for (;;) {
			...
			if (next.done !== false || !(next.value instanceof Uint8Array) || !next.value.length || next.value.length > bound) throw new NativePreparationError();
		...
		chunks.length = 0;
		let text: string;
		try { text = decode(bytes); } finally { bytes.fill(0); }
		...
		try { void Promise.resolve(operation.cancel()).catch(() => undefined); } catch { /* Caller retains retry ownership. */ }
