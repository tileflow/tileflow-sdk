...
function receipt() {
	return {
		kind: 'tileflow-map-deployment-result', schemaVersion: 2, changed: true,
		mapId, deploymentId: 'dep_fixture', version: 7,
		renderers: {
			web: {themes: {light: {styleUrl: `${apiUrl}/maps/${mapId}/light.json`}}},
			native: {profile: 'native-v1', manifestUrl: `${apiUrl}/maps/${mapId}/native/manifest.json`,
				themes: {light: {styleUrl: `${apiUrl}/maps/${mapId}/native/v7/light.json`, revision: 'a'.repeat(64)}}},
		},
	};
}

test('native selection rejects absent target and unsafe origin before local config or remote work', async () => {
	for (const patch of [{mapId: undefined}, {mapId: 'invalid'}, {apiUrl: 'http://api.example.test'}]) {
		let work = 0;
		await assert.rejects(runHostedNativeDeploy({...options, ...patch}, {
			source: {kind: 'cli'}, async resolveApi() { work++; return null; },
			async prepare() { work++; return prepared(); },
			async loadManifest() { work++; return null; },
			async writeManifest() { work++; return ''; },
		}));
		assert.equal(work, 0);
	}
});
...
test('complete preflight and existing-manifest checks precede authentication and schema-2 publication', async () => {
	...
	const output = await runHostedNativeDeploy(options, {
		source: {kind: 'cli'},
		async prepare() { events.push('preflight'); return {...candidate, async dispose() { events.push('dispose'); }}; },
		async loadManifest() { events.push('existing'); return null; },
		async resolveApi() { events.push('auth'); return {apiUrl, apiKey: 'tf_cap_fixture', mapId}; },
		...
		async writeManifest(path, value) { events.push('write'); written = value; return path; },
	});
...
test('invalid native preflight cannot perform even authentication', async () => {
	let remote = 0;
	await assert.rejects(runHostedNativeDeploy(options, {
		source: {kind: 'cli'}, async prepare() { throw new Error('Native incompatibility.'); },
		async loadManifest() { return null; }, async resolveApi() { remote++; return null; },
		async writeManifest() { assert.fail('No manifest replacement.'); },
	}));
...
test('failed, mismatched or legacy responses preserve the previous manifest and dispose preparation', async () => {
	for (const reply of [
		...
		const candidate = await prepared();
		await assert.rejects(runHostedNativeDeploy(options, {
			source: {kind: 'cli'}, async prepare() { return {...candidate, async dispose() { disposed++; }}; },
			async loadManifest() { return null; }, async resolveApi() { return {apiUrl, apiKey: 'tf_cap_fixture', mapId}; },
			request: {fetch: async () => reply()}, async writeManifest() { writes++; return ''; },
		}));
...
test('an idempotent server retry writes the confirmed manifest without creating another destination', async () => {
	...
	const result = await runHostedNativeDeploy(options, {
		source: {kind: 'cli'}, async prepare() { return candidate; }, async loadManifest() { return null; },
		async resolveApi() { return {apiUrl, apiKey: 'tf_cap_fixture', mapId}; },
		request: {fetch: async () => { publications++; return Response.json({...receipt(), changed: false}); }},
		async writeManifest(path) { return path; },
	});
