...
import {createMapCameraController, type CameraCommand} from '../src/camera-controller';
...
test('JavaScript camera props cannot bypass ownership or complete-view validation', () => {
	const invalid: unknown[] = [
		null, [], 1, {view: full()}, {view: {zoom: 4}, onViewChange: ignore},
		{view: full(), initialView: {}, onViewChange: ignore}, {view: full(), onViewChange: null},
		{initialView: null}, {onViewChange: 'private'}, {view: null, onViewChange: ignore},
		{initialView: {padding: 1}}, Object.create({view: full()}),
	];
...
test('Core owns numeric ranges, unknown fields, safe records and coordinate order', () => {
	for (const candidate of [
		{...full(), zoom: NaN}, {...full(), zoom: 25}, {...full(), zoom: -1},
		{...full(), bearing: 181}, {...full(), bearing: -181},
		{...full(), pitch: 86}, {...full(), pitch: -1}, {...full(), pitch: Infinity},
		{...full(), center: [181, 20]}, {...full(), center: [0, 91]},
		{...full(), center: [-181, 20]}, {...full(), center: [0, -91]},
		{...full(), center: [0]}, {...full(), center: [0, 1, 2]},
		{...full(), center: {lng: 1, lat: 2}}, {...full(), zoom: undefined},
		{...full(), padding: 1}, Object.assign(Object.create({}), full()),
	]) {
		const t = setup();
		t.camera.mount({view: full(), onViewChange: ignore});
		const before = t.camera.view;
		assert.throws(() => t.camera.update({view: candidate, onViewChange: ignore} as never), {code: 'CAMERA_INPUT_INVALID'});
		assert.equal(t.camera.view, before);
		assert.equal(t.commands.length, 1);
	}
	const t = setup();
	t.camera.mount({view: {center: [-180, 90], zoom: 24, bearing: -180, pitch: 85}, onViewChange: ignore});
...
test('accessors are not evaluated and invalid native observations do not corrupt live view', () => {
	let reads = 0;
	const t = setup();
	const input = {get view() { reads++; throw new Error('private'); }, onViewChange: ignore};
	assert.throws(() => t.camera.mount(input as never), {code: 'CAMERA_INPUT_INVALID'});
	const center = [0, 20];
	Object.defineProperty(center, '0', {enumerable: true, get() { reads++; throw new Error('private'); }});
	assert.throws(() => t.camera.mount({view: {...full(), center}, onViewChange: ignore} as never), {code: 'CAMERA_INPUT_INVALID'});
	assert.equal(reads, 0);
	t.camera.mount({});
	const token = t.camera.startGesture()!;
	const before = t.camera.view;
	assert.throws(() => t.camera.changeGesture(token, {zoom: 8} as never), {code: 'CAMERA_INPUT_INVALID'});
	assert.throws(() => t.camera.endGesture(token, {zoom: 8} as never), {code: 'CAMERA_INPUT_INVALID'});
	assert.equal(t.camera.view, before);
...
test('source generation and acquisition are independent of camera commands and restoration', async () => {
	let acquisitions = 0;
	const source = createTileflowNativeSourceController({acquire() {
		acquisitions++;
