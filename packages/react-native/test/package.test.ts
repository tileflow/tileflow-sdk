...
function runtimeImports(text: string): string[] {
	...
	const visit = (node: ts.Node) => {
		...
		if (ts.isCallExpression(node) && (node.expression.kind === ts.SyntaxKind.ImportKeyword || (ts.isIdentifier(node.expression) && ['require', '__require'].includes(node.expression.text)))) result.push(node.getText());
...
test('remains private with exact native peers and the ordinary package/license boundaries', async () => {
	...
	assert.deepEqual(manifest.peerDependencies, {'@maplibre/maplibre-react-native': '11.3.10', react: '19.2.0', 'react-native': '0.83.10'});
	for (const [name, version] of Object.entries(manifest.peerDependencies)) assert.equal(manifest.devDependencies[name], version);
	assert.deepEqual(manifest.dependencies, {'@tileflow/core': 'workspace:>=0.1.0-alpha.16 <0.1.0-beta.0'});
	assert.deepEqual(manifest.exports, {'.': {types: './dist/index.d.ts', import: './dist/index.js', default: './dist/index.js'}});
	...
	for (const path of [
		'dist/index.d.ts', 'dist/internal/native-appearance.js', 'dist/internal/native-appearance.d.ts',
		'dist/internal/initial-view.js', 'dist/internal/source-state.js',
		'dist/internal/session.js', 'dist/internal/session.d.ts',
		'dist/internal/native-admission.js', 'dist/internal/native-admission.d.ts',
		'dist/internal/native-admission-bridge.js', 'dist/internal/native-admission-bridge.d.ts',
...
test('the contract imports standalone with no peers, globals or renderer evaluation', async (t) => {
	...
	const {stdout, stderr} = await exec(process.execPath, ['--input-type=module', '--eval', `
		import assert from 'node:assert/strict';
...
test('only explicit private platform adapters evaluate React Native and no output evaluates MapLibre', async () => {
	for (const entry of ['index', 'internal/native-appearance', 'internal/initial-view', 'internal/source-state', 'internal/session', 'internal/native-admission', 'internal/native-admission-bridge']) {
		const imports = runtimeImports(await readFile(new URL(`dist/${entry}.js`, root), 'utf8'));
		assert.equal(imports.some((name) => name.includes('maplibre')), false, entry);
		if (entry === 'internal/native-appearance') assert.deepEqual(imports, ['react-native']);
		else if (entry === 'internal/native-admission-bridge') assert.equal(imports.includes('react-native'), true, entry);
	...
	for (const name of ['MapBaseProps', 'MapOptions', 'MapRef', 'MapSourceState', 'MapInitialViewInputs']) assert.ok(declarations.includes(name), name);
	assert.doesNotMatch(declarations, /declare (?:const|function|class) Map\b/u);
	assert.doesNotMatch(declarations, /HostedNativeSession|NativeSessionAuthority|NativeAdmission|NativeBootstrap/u);
});
