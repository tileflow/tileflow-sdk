...
export async function assertOwnedPath(root, candidate) {
  ...
  for (;;) {
    try {
      const physical = resolve(await realpath(existing), ...missing);
      requireMobile(isWithin(root, physical), 'WORKSPACE_UNSAFE');
      return physical;
    } catch (error) {
      if (error.code !== 'ENOENT') throw error;
      const parent = dirname(existing);
      requireMobile(parent !== existing, 'WORKSPACE_UNSAFE');
...
export async function copyMobileInputs(source, destination, excluded = new Set()) {
  ...
  if (metadata.isDirectory()) {
    ...
    requireMobile(
      metadata.isFile() && metadata.size <= 128 * 1024 * 1024,
      'WORKSPACE_UNSAFE',
      'stage',
    );
...
export async function readMobileJson(root, path) {
  try {
    return JSON.parse((await readMobileFile(root, path, 8 * 1024 * 1024)).toString('utf8'));
  } catch (error) {
...
export async function createMobileContext(root, options = {}) {
  ...
  for (const directory of ['home', 'tmp', 'logs', 'cache', 'gradle'])
  ...
  Object.assign(env, {
    ...
    CP_HOME_DIR: join(root, 'home', '.cocoapods'),
    CP_CACHE_DIR: join(root, 'cache', 'cocoapods'),
    CI: '1',
    NO_COLOR: '1',
    LANG: 'en_US.UTF-8',
    LC_ALL: 'en_US.UTF-8',
    ...
    async run(command, args, settings = {}) {
      ...
      try {
        ...
        throw error instanceof MobileSmokeError
          ? error
          : new MobileSmokeError('COMMAND_FAILED', settings.step);
      }
    },
  };
}
