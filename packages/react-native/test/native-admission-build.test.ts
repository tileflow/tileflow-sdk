...
test('native build metadata is explicit, pinned, and does not activate networking', async () => {
  ...
  const config = createRequire(import.meta.url)(
    fileURLToPath(new URL('react-native.config.cjs', root)),
  );
  ...
  assert.equal(
    config.dependency.platforms.android.packageInstance,
    'new TileflowNativeAdmissionPackage()',
  );
  ...
  assert.match(
    await text('android/src/main/AndroidManifest.xml'),
    /android\.permission\.INTERNET/u,
  );
  const podspec = await text('TileflowNativeAdmission.podspec');
  assert.match(podspec, /delete_suffix\(['"]-development['"]\)/u);
  assert.match(podspec, /platforms\s*=\s*\{\s*:ios\s*=>\s*['"]15\.1['"]\s*\}/u);
...
test('native adapters have no global protocol registration or grant logger', async () => {
  for (const path of [
    ...
    'ios/TFAdmissionNetwork.mm',
    'ios/TFAdmissionInstallation.mm',
    'ios/TFNativeAdmissionURLProtocol.mm',
    'ios/TileflowNativeAdmission.mm',
  ]) {
    const source = await text(path);
    assert.doesNotMatch(
      source,
      /TransformRequestManager|HttpLoggingInterceptor|\bTimber\.|\bLog\.|\bprintln\s*\(|\bNSLog\s*\(|\bos_log\s*\(/u,
      path,
    );
    assert.doesNotMatch(source, /registerClass\s*:/u, path);
  }
  const android = await text(
    'android/src/main/java/dev/tileflow/reactnative/AdmissionBootstrapOkHttpNetwork.kt',
  );
