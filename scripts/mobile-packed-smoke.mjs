...
export async function runMobileSmokePlan(mode, operations) {
  ...
  if (mode !== 'packed') {
    for (const consumer of consumers) {
      ...
      requireMobile(
        output?.consumer === consumer.kind &&
          output.platform === mode &&
  ...
  return {
    ...
    packages: packages.tarballs.map(({name, version, identity}) => ({
      name,
      version,
      artifact: identity,
    })),
    templates: mobileConsumers.map((kind) => ({
      package: templates[kind].name,
      version: templates[kind].version,
      artifact: templates[kind].identity,
    })),
...
export async function runMobileSmoke(options, dependencies = {}) {
  const repository =
    dependencies.repository ?? resolve(fileURLToPath(new URL('..', import.meta.url)));
  ...
  return withMobileWorkspace(
    async (root) => {
      const context = await createMobileContext(root, dependencies);
      const result = await runMobileSmokePlan(options.mode, {
        prerequisites: () =>
          checkMobilePrerequisites(context, options.mode, manifest.packageManager, pnpm, platform),
        pack: () => packMobilePackages(context, repository, pnpm),
        templates: () => prepareMobileTemplates(context),
        consumer: (kind, packages, templates) =>
          createMobileConsumer(context, kind, packages, templates),
        native: (mode, consumer) =>
          mode === 'android'
            ? buildMobileAndroid(context, consumer)
            : buildMobileIos(context, consumer),
      });
...
async function main() {
  ...
  try {
    ...
    const result = await runMobileSmoke(options, {
      ...
      onPreserved: (root) =>
        process.stderr.write(`Preserved diagnostic workspace (not a durable identity): ${root}\n`),
    });
    ...
    process.stdout.write(
      `${JSON.stringify({schemaVersion: 1, command: 'smoke:mobile', ok: false, ...(options ? {mode: options.mode} : {}), error: projectMobileFailure(error)})}\n`,
    );
