...
export function createPortableIdentitySchemas(operations: ManifestStringOperations) {
	const bounded = operations.maxLength(
		operations.minLength(operations.string(), 1), tileflowPortableIdMaximumLength,
	);
	const pattern = operations.regex(
		bounded, tileflowPortableIdPattern, 'Expected lowercase kebab-case beginning with a letter',
	);
	const tileflowPortableIdSchema = operations.refine(
		pattern,
		(name) => !['constructor', 'prototype'].includes(name) &&
	...
	const tileflowThemeNameSchema = operations.refine(
		tileflowPortableIdSchema, isTileflowThemeName,
