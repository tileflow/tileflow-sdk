...
import {createTileflowBuildProvenance, createTileflowStyles, prepareTileflowLocalTilesets} from '@tileflow/dev/artifacts';
import {assertValidTileflowConfig, getTileflowMapNames, loadTileflowConfigWithInputs} from '@tileflow/dev/config';
...
import {requestHostedJson, uploadHostedIconPackage, type HostedApi, type HostedRequestOptions} from './hosted-client';
import {inspectTileflowHostedCompatibility, prepareTileflowHostedThemeFamily} from './hosted-preflight';
...
type Ports = Readonly<{
	...
	loadManifest(path: string, options: {overwriteSelfHosted: boolean}): Promise<TileflowRuntimeManifest | null>;
...
function requireNativeTarget(options: NativeHostedDeployOptions): {apiOrigin: string; mapId: string} {
	...
	if (!apiOrigin.startsWith('https://')) throw failure('Hosted Native deployment requires an HTTPS API origin.');
...
export async function prepareNativeHostedDeploy(options: NativeHostedDeployOptions): Promise<PreparedNativeDeploy> {
	const {apiOrigin} = requireNativeTarget(options);
	const loaded = await withTileflowConfigSecretsHidden(() => loadTileflowConfigWithInputs(options.config));
	...
	if (!mapName || !names.includes(mapName)) throw failure('Select one configured map with --map for this Native deployment.');
	...
	const compiledIcons = await compileTileflowIconPackages(project, {
		baseDirectory, cwd: process.cwd(), target: 'hosted',
		icons: {
	...
	const iconPackage = binding ? compiledIcons.packages.find((entry) => entry.contentHash === binding.packageHash) : undefined;
	const sprite = binding ? `${apiOrigin}/sprites/preflight/${binding.packageHash}/sprite` : undefined;
	const mapAssets = Object.fromEntries(compiledIcons.bindings.map((entry) => [entry.mapName, {
		icons: {ids: entry.iconIds, sprite: `${apiOrigin}/sprites/preflight/${entry.packageHash}/sprite`},
	}]));
	...
	const local = await prepareTileflowLocalTilesets(project, styles, {
		assetBaseUrl: apiOrigin, baseDirectory, cwd: process.cwd(),
	});
	try {
		const fonts = await prepareTileflowStyleFonts(project, local.styles, {
			assetBaseUrl: `${apiOrigin}/fonts/preflight`, baseDirectory, cwd: process.cwd(), target: 'hosted',
		});
		...
		if (Object.keys(fonts.bundles).length)
			throw failure('Hosted deploy does not yet support package-owned web fonts. Use an explicit public glyph provider.');
		const assets = (iconPackage?.files ?? []).map((file) => ({
			contentType: file.contentType, fileName: `icons/${mapName}/${file.fileName}`, source: file.source,
		}));
		const provenance = await createTileflowBuildProvenance(process.cwd());
		const resolved = parseResolvedTileflowMap(map);
		const inputStyles: Record<string, MapLibreStyle> = JSON.parse(serializeCanonicalJson(fonts.styles[mapName]!));
		const createArtifact = async (selectedSprite: string | undefined) => {
			const selected: Record<string, MapLibreStyle> = JSON.parse(serializeCanonicalJson(inputStyles));
			if (binding) {
				if (!sprite || !selectedSprite) throw failure('Missing confirmed sprite binding.');
				for (const style of Object.values(selected)) {
					if (style.sprite !== sprite) throw failure('Compiled sprite identity changed before publication.');
					style.sprite = selectedSprite;
				}
			}
			const family = prepareTileflowHostedThemeFamily(mapName, map, selected);
			const buildManifest = await createTileflowMapBuildManifest({[mapName]: {
				assets, lineage: project.mapMetadata?.[mapName]?.lineage ?? [{id: map.id, mapVersion: map.version}],
				map, sourceAssets: {
					fonts: fonts.sourceIdentities[mapName] ?? [], icons: compiledIcons.sourceIdentities[mapName] ?? [],
					...(compiledIcons.compositions[mapName] ? {iconComposition: compiledIcons.compositions[mapName]} : {}),
				}, styles: family.styles,
			}}, {provenance});
			return prepareTileflowHostedNativeDeployment({mapId: mapName, buildManifest,
				styles: family.styles, teamSources: family.teamSources, assets,
				...(resolved.view ? {view: resolved.view} : {}),
			});
		};
		const artifact = await createArtifact(sprite);
		return {
			mapName, artifact, packages: compiledIcons.packages,
...
export async function runHostedNativeDeploy(options: NativeHostedDeployOptions, ports: Ports) {
	...
	const prepared = await (ports.prepare ?? prepareNativeHostedDeploy)({...options, apiUrl: apiOrigin});
	try {
		...
		if (preflight.mapId !== prepared.mapName) throw failure('The prepared map does not match its artifact.');
		const existing = await ports.loadManifest(options.manifest, {
			overwriteSelfHosted: options.overwriteSelfHostedManifest === true,
		});
		const previousOrigin = existing?.apiUrl ?? (existing ? Object.values(existing.maps)[0]?.apiUrl : undefined);
		...
		if (!api) return null;
		if (api.apiUrl !== apiOrigin || api.mapId !== mapId || !api.apiKey || api.apiKey.length > 8192 || /[\p{Cc}]/u.test(api.apiKey))
		...
		for (const icon of prepared.packages) {
			...
			let uploaded;
			try { uploaded = await (ports.uploadIcon ?? uploadHostedIconPackage)(api, icon, ports.request); }
			catch { throw failure('Native deployment icon upload failed. The local manifest was preserved.'); }
			if (!uploaded.ok) throw failure('Native deployment icon upload failed. Retry the same artifact.');
		...
		const artifact = await parseTileflowRendererDeploymentArtifact(await prepared.retarget(sprites));
		if (artifact.mapId !== preflight.mapId ||
		...
		const iconPackage = prepared.iconBinding
			? prepared.packages.find((entry) => entry.contentHash === prepared.iconBinding!.packageHash) : undefined;
		if (prepared.iconComposition && prepared.iconComposition.packageHash !== iconPackage?.contentHash)
			throw failure('Icon composition does not match the confirmed effective package.');
		let response;
		try {
			response = await requestHostedJson(apiOrigin, '/v1/styles', {
				method: 'POST', headers: {Authorization: `Bearer ${api.apiKey}`,
					'X-Tileflow-Map-Id': mapId, 'Content-Type': 'application/json'},
				body: serializeCanonicalJson({artifact, environment: prepared.mapName, managedMapId: mapId,
					usageMode: 'session', source: ports.source,
					...(iconPackage && prepared.iconBinding ? {iconPackage: {
						contentHash: iconPackage.contentHash, label: prepared.iconBinding.label,
					}} : {}),
					...(prepared.iconComposition ? {iconComposition: prepared.iconComposition} : {}),
				}),
			}, ports.request);
		} catch { throw failure('Renderer deployment response was unavailable. Retry the same artifact; the local manifest was preserved.'); }
		if (!response.ok || !response.json) throw failure('Renderer deployment failed. The local manifest was preserved.');
		const parsed = tileflowRendererDeploymentResponseSchema.safeParse(response.body);
		if (!parsed.success) throw failure('Invalid renderer deployment response. The local manifest was preserved.');
		...
		if (result.mapId !== mapId || result.renderers.native.manifestUrl !== expectedManifestUrl ||
			...
			throw failure('Renderer deployment did not confirm the requested Map and complete theme families.');
		for (const name of names) {
			if (result.renderers.web.themes[name]!.styleUrl !== `${apiOrigin}/maps/${mapId}/${name}.json` ||
				result.renderers.native.themes[name]!.styleUrl !== `${apiOrigin}/maps/${mapId}/native/v${result.version}/${name}.json`)
		...
		const manifest = parseTileflowRuntimeManifest({apiUrl: apiOrigin, version: 1, maps: {
			...(existing?.maps ?? {}), [prepared.mapName]: {
				defaultTheme: build.defaultTheme, environment: prepared.mapName, mapId,
				...(build.systemThemes ? {systemThemes: build.systemThemes} : {}),
				...(artifact.view ? {view: artifact.view} : {}), usageMode: 'session', worldGeneration: 'v1',
				themes: Object.fromEntries(names.map((name) => [name, {
					colorScheme: build.themes[name]!.colorScheme,
					fontFaces: getTileflowStyleFontFaces(artifact.renderers.web.styles[name] as MapLibreStyle),
		...
		return Object.freeze({manifestPath, mapName: prepared.mapName, mapId, deploymentId: result.deploymentId,
			version: result.version, changed: result.changed, nativeManifestUrl: expectedManifestUrl});
