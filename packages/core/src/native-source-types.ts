...
export type TileflowNativeSourceState =
  ...
  | {
      ...
      readonly generation: number;
      readonly error: TileflowNativeSourceError;
    };
...
const messages = {
  ...
  NATIVE_MANIFEST_RESPONSE_INVALID:
    'Expected a bounded byte reader and an explicit final response URL.',
  ...
  NATIVE_THEME_INVALID:
    'Expected a declared concrete theme or an explicitly supplied system color scheme.',
} as const;
...
export class TileflowNativeSourceError extends Error {
  ...
  constructor(code: TileflowNativeSourceErrorCode, field: TileflowNativeSourceErrorField) {
    ...
    this.field = field;
    this.kind =
      code === 'NATIVE_SOURCE_ABORTED' || code === 'NATIVE_SOURCE_DISPOSED'
        ? 'cancelled'
        : 'terminal';
  }
}
...
export function normalizeNativeSourceError(error: unknown): TileflowNativeSourceError {
  try {
    ...
    /* Do not inspect remote exception messages or causes. */
  }
