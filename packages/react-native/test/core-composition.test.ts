...
function ready(): Extract<TileflowNativeSourceState, {status: 'ready'}> {
	return {
		status: 'ready', generation: 2,
		source: {map: 'streets', manifestUrl: 'https://maps.example.test/manifest.json?secret=value'},
...
test('projects immutable Tileflow identity without renderer kinds, URLs, bodies or exception details', () => {
	const source = ready();
	const projected = projectMapSourceState(source);
	assert.deepEqual(projected, {
		status: 'ready', generation: 2, map: 'streets',
		theme: {name: 'light', colorScheme: 'light'},
	});
	assert.ok(Object.isFrozen(projected));
	if (projected?.status !== 'ready') assert.fail('Expected Tileflow selection.');
	assert.ok(Object.isFrozen(projected.theme));
	assert.equal(Object.isFrozen(source), false);
	assert.equal(JSON.stringify(projected).includes('secret'), false);
	assert.equal('kind' in projected, false);
...
test('initial-view composition delegates to Core without changing precedence or error shape', () => {
	const input = {
		...
		mapOptionsView: {zoom: 9, pitch: 30}, view: {zoom: 11},
	};
