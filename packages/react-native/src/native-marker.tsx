...
const button = Object.freeze({minWidth: 44, minHeight: 44, alignItems: 'center' as const, justifyContent: 'center' as const});
const dot = Object.freeze({width: 18, height: 18, borderRadius: 9, borderWidth: 2, borderColor: '#ffffff'});
...
export function NativeAnnotationMarker<TAnnotation extends TileflowAnnotation>({
	annotation, state, enabled, sceneKey, lifecycle, renderMarker,
}: {
	...
	const model = useMemo(() => nativeMarkerModel(annotation, state, enabled), [annotation, state, enabled]);
	const rendered = resolveNativeMarkerContent(model.context, renderMarker);
	let color = defaultColor;
	let invalidColor = false;
	if (annotation.marker?.color) {
		try {
			if (typeof processColor(annotation.marker.color) === 'number') color = annotation.marker.color;
			else invalidColor = true;
		} catch { invalidColor = true; }
	}
	const diagnostic = rendered.diagnostic ?? (!rendered.content && invalidColor ? 'INVALID_FIELD' : undefined);
	useLayoutEffect(() => {
