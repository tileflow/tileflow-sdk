...
async function fixture() {
	...
	const module = {exports: {} as {NativeAnnotationMarker: (props: Record<string, unknown>) => React.ReactElement<Record<string, any>>}};
	const require = (id: string): unknown => {
		...
		if (id === 'react-native') return {
			Pressable: 'NativePressable', View: 'NativeView', processColor: () => 0xff2563eb,
		};
...
const annotation: TileflowAnnotation = {
	id: 'place', kind: 'marker', coordinate: [1, 2], ariaLabel: 'Accessible place',
	data: {capacity: 12},
};
...
test('default and custom content retain one authoritative accessible activation wrapper', async () => {
	...
	for (const content of [undefined, custom]) {
		const marker = f.render({
			annotation, state: {popup: null}, enabled: true, sceneKey: 'map',
			lifecycle: {}, renderMarker: content ? () => content : undefined,
		});
...
test('Pressable and native Marker deliveries share one activation and accessibility uses the same target', async () => {
	...
	interactions.update({annotations: [annotation], onInteractionEvent: (event) => events.push(event)});
	...
	const marker = f.render({annotation: current, state: {popup: null}, enabled: true, sceneKey: 'map', lifecycle});
	const button = marker.props.children;
	let stopped = 0;
...
test('custom renderer diagnostics are delivered after element assembly, never during render', async () => {
	...
	const marker = f.render({
		...
		renderMarker() { throw new Error('Application rendering failure'); },
	});
