...
import {NativeSurfaceError, type NativeSurface, type NativeSurfaceEvent, type NativeSurfaceModule} from './native-surface-contract';

const methods = ['attachSurface', 'expectStyle', 'commitLayout', 'requestFrame', 'applyCamera', 'cancelCamera', 'acknowledgeSurface', 'retireSurface', 'retireRoot'] as const;
...
function eventFromNative(value: unknown): NativeSurfaceEvent {
	const surface = own(value, 'surface'); const style = own(value, 'style');
	const sequence = own(value, 'sequence'); const layout = own(value, 'layout');
	const kind = own(value, 'kind');
	if (!isNativeToken(surface) || !isNativeToken(style) || !positive(sequence) || !positive(layout)) throw new NativeSurfaceError();
	const keys = Reflect.ownKeys(value as object);
	if (kind === 'gesture-start' || kind === 'gesture-change' || kind === 'gesture-end') {
		const gesture = own(value, 'gesture');
		if (!positive(gesture) || keys.length !== 7 || keys.some((key) => typeof key !== 'string' || !['surface', 'style', 'sequence', 'layout', 'kind', 'gesture', 'view'].includes(key))) throw new NativeSurfaceError();
		return Object.freeze({surface, style, sequence, layout, kind, gesture, view: snapshotCameraView(own(value, 'view'))});
	}
	if (!['style', 'render', 'invalidate', 'error'].includes(String(kind)) || keys.length !== 5 ||
		keys.some((key) => typeof key !== 'string' || !['surface', 'style', 'sequence', 'layout', 'kind'].includes(key))) throw new NativeSurfaceError();
...
export function createNativeSurfaceTransport(
	...
	function module(): NativeSurfaceModule {
		try {
			...
			if (!value || methods.some((name) => typeof value[name] !== 'function')) throw new NativeSurfaceError();
			return value;
		} catch { throw new NativeSurfaceError(); }
	}
	async function safe<T>(work: () => Promise<T>): Promise<T> {
		try { return await work(); } catch { throw new NativeSurfaceError(); }
	}
	return Object.freeze({
		available(): void { module(); },
		async attach(root: number, notify: (event: NativeSurfaceEvent) => void, failure: () => void): Promise<NativeSurface> {
			...
			const fail = () => {
				...
				failed = true;
				try { failure(); } catch { /* Application observers cannot own native lifetime. */ }
			};
			const check = () => { if (!live || failed || module() !== native) throw new NativeSurfaceError(); };
			try {
				unsubscribe = listen((value) => {
					if (!live || !id) return;
					try { if (own(value, 'surface') !== id) return; } catch { return; }
					try {
						check();
				...
				if (!ack || !isNativeToken(ack.surface) || module() !== native) throw new NativeSurfaceError();
				id = ack.surface;
			} catch {
				live = false;
				unsubscribe?.();
			...
			return Object.freeze({
				...
				async expectStyle(style) {
					...
					check(); if (ack?.accepted !== true) throw new NativeSurfaceError();
				},
				async commitLayout(style) {
					check();
					const ack = await safe(() => native.commitLayout(surface, style));
					check(); if (!positive(ack?.layout)) throw new NativeSurfaceError();
					return ack.layout;
				},
				async requestFrame(style) {
					check();
					const ack = await safe(() => native.requestFrame(surface, style));
					check(); if (ack?.requested !== true) throw new NativeSurfaceError();
				},
				async applyCamera(command, view) {
					check();
					if (!positive(command)) throw new NativeSurfaceError();
					const ack = await safe(() => native.applyCamera(surface, command, snapshotCameraView(view)));
					check();
				...
				retire(): Promise<void> {
					...
					const attempt = safe(() => native.retireSurface(surface)).then((ack) => {
						...
						if (!released) { released = true; reservations--; }
					});
					retirement = attempt;
