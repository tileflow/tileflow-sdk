...
const configuration = () => snapshotMobileConfiguration({
	apiOrigin: 'https://api.example.test', credential: credential(),
});
...
test('non-session metadata is not read through incidental getters', async () => {
	...
	let reads = 0;
	Object.defineProperty(input.map, 'apiUrl', {get() { reads++; throw new Error(credential()); }});
...
test('reentrant reflection cannot publish an older Hosted binding', async () => {
	...
	const input = new Proxy(source(), {
		getOwnPropertyDescriptor(target, property) {
			if (!replaced) { replaced = true; replacement = resolver.replace(direct); }
