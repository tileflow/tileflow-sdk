  ...
  return {
    source: {
      ...
      theme: {
        ...
        styleUrl: `https://maps.example.test/${name}.json`,
      },
...
test('source preparation and a style-loaded event alone cannot establish rendered readiness', async () => {
  ...
  f.emit('render');
  assert.equal(
    f.events.filter((event) => event.type === 'readiness-change' && event.status === 'ready')
      .length,
    1,
  );
...
test('theme transactions and rollback keep the same native surface and last live camera', async () => {
  ...
  assert.equal(
    f.events.some(
      (event) => event.type === 'theme-change' && event.phase === 'ready' && event.generation === 2,
    ),
    false,
  );
  assert.equal(
    f.events.some(
      (event) => event.type === 'theme-change' && event.phase === 'error' && event.generation === 2,
    ),
    true,
  );
  assert.equal(f.events.at(-1)!.status, 'ready');
  await f.owner.dispose();
  assert.equal(f.detach(), 1);
});
...
test('controlled gestures settle only after committed props and do not echo prop commands', async () => {
  ...
  f.owner.afterCommit({view: final, onViewChange: props.onViewChange});
  await f.owner.whenIdle();
  assert.equal(f.commands.length, before);
  assert.equal(observed.length, 1);
  f.owner.afterCommit({...props, view: {...final, zoom: 7}});
  await f.owner.whenIdle();
