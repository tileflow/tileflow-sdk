import {type TileflowRuntimeManifest, tileflowRuntimeManifestLimits} from './manifest-types';
import {parseTileflowRuntimeManifest} from './native-manifest-schema';
import {
  decodeNativeManifestUtf8,
  hasBoundedNativeManifestDepth,
  nativeManifestUtf8ByteLength,
} from './native-manifest-utf8';
import {cancelNativeOperation, NativeSourceCancellation} from './native-source-cancellation';
import {
...
export async function loadTileflowNativeManifest(
  ...
  try {
    return await loadNativeManifest(
      manifestUrl,
      input.acquire as TileflowNativeManifestAcquire,
      {
...
export async function loadNativeManifest(
  ...
  try {
    requestUrl = resolveTileflowNativeManifestUrl(manifestUrl, network);
  } catch {
  ...
  try {
    operation = acquire(
      requestUrl,
      Object.freeze({maximumBytes: tileflowNativeManifestLimits.maximumBytes}),
    );
    if (
      !operation ||
  ...
  try {
    const headers = Promise.resolve(operation!.response).then(
      (response) => {
        try {
          if (
            !response ||
            !response.reader ||
            typeof response.reader.read !== 'function' ||
            typeof response.reader.cancel !== 'function'
          ) {
            throw new Error();
          }
    ...
    let finalUrl: string;
    try {
      finalUrl = resolveTileflowNativeManifestUrl(response.url, network);
    } catch {
      throw new TileflowNativeSourceError('NATIVE_MANIFEST_URL_INVALID', 'manifestUrl');
    }
    ...
    if (response.status === 401 || response.status === 403)
      throw new TileflowNativeSourceError('NATIVE_MANIFEST_ACCESS_DENIED', 'response');
    if (response.status === 404)
      throw new TileflowNativeSourceError('NATIVE_MANIFEST_NOT_FOUND', 'response');
    ...
    while (true) {
      ...
      const allowance = Math.min(
        tileflowNativeManifestLimits.maximumChunkBytes,
        buffer.byteLength - length + 1,
      );
      ...
      cancellation.check();
      if (!next || typeof next.done !== 'boolean')
        throw new TileflowNativeSourceError('NATIVE_MANIFEST_RESPONSE_INVALID', 'response');
      if (next.done) {
      ...
      if (!(next.value instanceof Uint8Array) || next.value.byteLength === 0)
        throw new TileflowNativeSourceError('NATIVE_MANIFEST_RESPONSE_INVALID', 'response');
      if (next.value.byteLength > buffer.byteLength - length)
        throw new TileflowNativeSourceError('NATIVE_MANIFEST_TOO_LARGE', 'body');
      if (next.value.byteLength > allowance)
        throw new TileflowNativeSourceError('NATIVE_MANIFEST_RESPONSE_INVALID', 'response');
    ...
    let text: string;
    try {
      text = decodeNativeManifestUtf8(buffer.subarray(0, length));
    } catch {
      throw new TileflowNativeSourceError('NATIVE_MANIFEST_UTF8_INVALID', 'body');
    }
    ...
    try {
      ...
      throw new TileflowNativeSourceError('NATIVE_MANIFEST_JSON_INVALID', 'body');
    }
    let manifest: TileflowRuntimeManifest;
    try {
      manifest = parseTileflowRuntimeManifest(json);
    } catch {
      throw new TileflowNativeSourceError('NATIVE_MANIFEST_INVALID', 'body');
    }
    ...
    const resolveResource = (value: string, owner: string) => {
      ...
      resolvedBytes +=
        nativeManifestUtf8ByteLength(JSON.stringify(resolved)) -
        nativeManifestUtf8ByteLength(JSON.stringify(value));
