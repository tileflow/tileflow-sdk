...
- (instancetype)initWithScheduler:(id<TFAdmissionScheduler>)scheduler network:(id<TFAdmissionNetwork>)network emit:(void (^)(NSDictionary *))emit {
	if ((self = [super init])) {
		...
		_manager = MLNNetworkConfiguration.sharedManager;
		_previous = _manager.sessionConfiguration;
		_previousDelegate = _manager.delegate;
		// A custom session factory bypasses sessionConfiguration. Reject it
		// instead of silently installing an interceptor that cannot own traffic.
		if (_previous.identifier || [_previousDelegate respondsToSelector:@selector(sessionForNetworkConfiguration:)] ||
		...
		if (![self ownsConfiguration]) { [self remove]; [self invalid]; }
	}
	return self;
}
...
- (BOOL)ownsConfiguration {
	@try {
		...
		return self.active && current == self.installed && self.manager.delegate == self.previousDelegate &&
