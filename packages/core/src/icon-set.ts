...
export const tileflowIconSetReferenceSchema = z.string().max(130).regex(
	new RegExp(`^@${slug}/${slug}$`, 'u'),
	'Expected a canonical @team/set reference without a version suffix',
).refine((value) => value.slice(1).split('/').every((part) => part.length <= 64),
	'Team and set slugs must contain at most 64 characters');

export const tileflowIconSetSourceSchema = z.object({
...
export function iconSet(reference: TileflowIconSetReference): TileflowIconSetSource {
	return Object.freeze(tileflowIconSetSourceSchema.parse({kind: 'icon-set', reference})) as TileflowIconSetSource;
}
...
export const tileflowIconSetTeamIdSchema = z.string().min(1).max(128).regex(/^[A-Za-z0-9_-]+$/u);
...
export class TileflowIconSetError extends Error {
	...
	constructor(readonly code: TileflowIconSetErrorCode, message: string, options?: ErrorOptions) {
...
export function collectTileflowIconSetReferences(sources: readonly TileflowIconSource[]): TileflowIconSetReference[] {
	if (sources.length > tileflowIconSourceLimit) {
		throw new TileflowIconSetError('ICON_SET_INVALID', `A map supports at most ${tileflowIconSourceLimit} icon contributors`);
	}
	...
	for (const source of sources) {
		...
		if (!parsed.success || seen.has(source.reference)) {
			throw new TileflowIconSetError('ICON_SET_INVALID', 'Icon set references must be canonical and unique within a map');
		}
