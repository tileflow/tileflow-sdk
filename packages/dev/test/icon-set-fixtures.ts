...
export async function setFixture(
  ...
  version = 1,
): Promise<{pin: TileflowIconSetPin; artifact: CompiledTileflowIconPackage}> {
  ...
  return {
    ...
    pin: {
      ...
      version,
      packageId: `icp_${artifact.contentHash.slice(0, 16)}`,
...
export function lockFor(sets: Record<string, TileflowIconSetPin>): TileflowIconsLockfileV1 {
  return {lockfileVersion: 1, sets};
}
