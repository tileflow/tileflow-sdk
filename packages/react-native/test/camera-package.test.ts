...
test('camera is packaged privately while root, peers, exports and existing adapters stay unchanged', async () => {
  ...
  for (const name of ['MapProps', 'MapCameraProps', 'MapViewChangeEvent'])
    assert.ok(declarations.includes(name), name);
  assert.doesNotMatch(
    declarations,
    /declare (?:function|const|class) Map\b|createMapCameraController|CameraCommand|CameraToken|settleGesture/u,
  );
  ...
  assert.doesNotMatch(
    readme,
    /not a complete `MapProps` interface|ownership is not part of `MapBaseProps`/u,
  );
});
