...
import {assertTileflowNativeCompiledStyles, prepareTileflowNativeStyles} from '../src/native-artifacts';
...
test('prepares Streets and every theme deterministically without changing logical map revision', async (t) => {
  ...
  for (const artifacts of [first, second, web]) t.after(() => disposeTileflowBuildArtifacts(artifacts));
  assert.deepEqual(first.files, second.files);
  assert.deepEqual(Object.keys(first.styles.main!).sort(), Object.keys(web.styles.main!).sort());
  assert.equal(first.buildManifest.maps.main!.mapRevisionSha256, web.buildManifest.maps.main!.mapRevisionSha256);
  ...
  assert.equal(first.nativeBuild.buildManifestSha256, digest(serializeCanonicalJson(first.buildManifest)));
...
test('uses independent web/native output inventories and retargets absolute asset roots once', async (t) => {
  const cwd = await fixture(t);
  const common = {cwd, outDir: 'public/tileflow', styleBaseUrl: '/tileflow', assetBaseUrl: '/tileflow'};
  ...
  for (const entry of Object.values(native.maps.main.themes) as {styleUrl: string}[]) {
    ...
    assert.match(style.sprite, /^\/tileflow\/native\/generations\/[a-f0-9]{64}\/icons\/main\/sprite$/u);
...
test('keeps icon inheritance and both generated sprite densities byte-identical', async (t) => {
  ...
  await writeFile(join(cwd, 'icons/coffee.svg'), '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M1 1h22v22H1z"/></svg>');
  ...
  assert.deepEqual(native.assets.filter(({fileName}) => fileName.startsWith('icons/')), web.assets.filter(({fileName}) => fileName.startsWith('icons/')));
  ...
  assert.throws(() => prepareTileflowNativeStyles(web.styles, badAssets), TileflowNativeCompatibilityError);
});

test('maps a licensed custom TTF by its exact full name without changing map identity', async (t) => {
  const cwd = await fixture(t);
  await mkdir(join(cwd, 'fonts'));
  await cp(new URL('../../maps/assets/cyberpunk/fonts/Oxanium-Medium.ttf', import.meta.url), join(cwd, 'fonts/face.ttf'));
  await cp(new URL('../../maps/assets/cyberpunk/fonts/LICENSE.txt', import.meta.url), join(cwd, 'fonts/LICENSE.txt'));
  await writeFile(join(cwd, 'tileflow.config.ts'), `import {defineMap,defineTheme} from '@tileflow/core';
...
export default defineMap({id:'main',version:1,extends:streets,themes,fonts:['./fonts']});\n`);
  ...
  const native = await createTileflowBuildArtifacts({cwd, renderer: 'native', styleBaseUrl: '/tileflow'});
  t.after(() => disposeTileflowBuildArtifacts(web));
  t.after(() => disposeTileflowBuildArtifacts(native));
  assert.equal(native.buildManifest.maps.main!.mapRevisionSha256, web.buildManifest.maps.main!.mapRevisionSha256);
  assert.deepEqual(native.buildManifest.maps.main!.sourceAssets, web.buildManifest.maps.main!.sourceAssets);
  for (const theme of Object.keys(native.styles.main!)) {
    const style = native.styles.main![theme]! as MapLibreStyle & {'font-faces': Record<string, string>};
    assert.deepEqual(Object.keys(style['font-faces']), ['Oxanium Medium']);
    assert.match(style['font-faces']['Oxanium Medium']!, /^\/tileflow\/native\/fonts\/oxanium-medium-[a-f0-9]{64}\.ttf$/u);
    assert.equal(style.metadata?.['tileflow:fontFaces'], undefined);
    assert.notEqual(native.buildManifest.maps.main!.themes[theme]!.styleSha256, web.buildManifest.maps.main!.themes[theme]!.styleSha256);
  }
  const immutable = native.files.find(({fileName}) => /^generations\/.*\/styles\/main\/light\.json$/u.test(fileName));
  assert.ok(immutable);
  ...
  await assert.rejects(createTileflowBuildArtifacts({cwd, renderer: 'native'}), (error: unknown) => {
...
for (const [label, extra, expectedPath] of [
  ...
  test(`rejects ${label} before replacing an existing native output`, async (t) => {
    ...
    await assert.rejects(writeTileflowBuildArtifacts({cwd, renderer: 'native', outDir: 'output'}), (error: unknown) => {
      assert.ok(error instanceof TileflowNativeCompatibilityError);
...
test('forwards the renderer into watched generations and retains the last valid one on failure', async (t) => {
  const cwd = await fixture(t);
  const session = await createTileflowArtifactSession({cwd, renderer: 'native', watch: true, debounceMs: 10});
  try {
...
test('preserves native context in existing diagnostic envelopes without treating pointers as filesystem paths', () => {
  const error = new TileflowNativeCompatibilityError([
    createTileflowNativeDiagnostic('NATIVE_UNSUPPORTED_SOURCE', '/maps/main/themes/light/style/sources/world/url'),
  ]);
  const expected = [...error.issues];
  assert.deepEqual(createTileflowArtifactDiagnostics(error, '/tmp/project'), expected);
  const failure = createTileflowCommandFailureDocument('validate', error, '/tmp/project', {code: 'VALIDATION_FAILED', phase: 'validation'});
  ...
  const ordinary = createTileflowCommandFailureDocument('validate', new Error('Invalid input.'), '/tmp/project', {code: 'VALIDATION_FAILED', phase: 'validation'});
...
test('preserves the authored portable view instead of applying native runtime camera defaults', async (t) => {
  const cwd = await fixture(t);
  await writeFile(join(cwd, 'tileflow.config.ts'), config('view:{pitch:70,zoom:12,center:[-3.7,40.4]},'));
...
test('rejects an asset URL that exceeds the native limit only after generation retargeting', async (t) => {
  ...
  await assert.rejects(createTileflowBuildArtifacts({cwd, renderer: 'native', assetBaseUrl}), (error: unknown) => {
    assert.ok(error instanceof TileflowNativeCompatibilityError);
    assert.ok(error.issues.some(({code, path}) => code === 'NATIVE_UNSUPPORTED_SOURCE' && path.endsWith('/sprite')));
    return true;
  });
});
