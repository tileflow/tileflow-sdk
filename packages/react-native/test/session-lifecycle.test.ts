...
test('9,999/10,000 eligible-request races rotate atomically at the transport boundary', async () => {
  const queue = createFetchQueue([
    ...
    response(201, success({sessionId: 'ses_test_2', surfaceId: 'default'})),
  ]);
  ...
  assert.deepEqual(
    queue.calls.map((call) => JSON.parse(call.init.body).sessionId),
    ['ses_test_1', 'ses_test_2'],
  );
});

test('six-hour rotation is checked before admission and concurrent callers share the replacement bootstrap', async () => {
...
test('background preserves identity; resume refreshes near-expiry/expired authority but does not depend on timers', async () => {
  ...
  const queue = createFetchQueue([
    ...
    response(
      ...
      success({
        surfaceId: 'default',
        issuedAt: '2026-09-15T20:30:00.000Z',
        serverTime: '2026-09-15T20:30:00.000Z',
        expiresAt: '2026-09-15T20:45:00.000Z',
      }),
    ),
  ]);
  const controller = createHostedNativeSessionController({
...
test('two controllers with identical inputs never share session identity, work, counters or authority', async () => {
  ...
  const a = createHostedNativeSessionController({
    ...
    fetch: aFetch.fetch,
    now: clock.now,
    sessionIdFactory: createIds('ses_a'),
  });
  const b = createHostedNativeSessionController({
    binding: hosted(),
    fetch: bFetch.fetch,
    now: clock.now,
    sessionIdFactory: createIds('ses_b'),
  });
...
test('dispose aborts owned work, is idempotent, and late fetch completion cannot revive state', async () => {
  ...
  pending.resolve(response(201, success({surfaceId: 'default'})));
...
test('replacement retires an already-started bootstrap and its late completion cannot affect the new Map', async () => {
  const replacementMapId = 'map_abcdef0123456789';
  const pending = deferred<ReturnType<typeof response>>();
  const queue = createFetchQueue([
    () => pending.promise,
    response(
  ...
  await queue.waitForCalls(1);
  controller.replaceBinding({...hosted(), mapId: replacementMapId});
  assert.equal(await code(old), 'NATIVE_SESSION_REPLACED');
  pending.resolve(response(201, success({surfaceId: 'default'})));

  const next = await controller.acquire();
  assert.equal(next?.mapId, replacementMapId);
  assert.equal(next?.sessionId, 'ses_test_2');
  assert.equal(queue.calls.length, 2);
  assertSafeState(controller.state, [credential, grant]);
});

test('observer reentrancy can replace a binding before fetch without consuming stale network work', async () => {
  const replacementMapId = 'map_abcdef0123456789';
  const queue = createFetchQueue([
    response(
      201,
...
test('mutation attempts cannot alter retained authority or safe snapshots', async () => {
  const queue = createFetchQueue([response(201, success({surfaceId: 'default'}))]);
  const controller = createHostedNativeSessionController({
    binding: hosted(),
    fetch: queue.fetch,
    now: createClock().now,
