...
export function projectMapSourceState(state: TileflowNativeSourceState | undefined): MapSourceState | undefined {
  if (!state) return undefined;
  if (state.status === 'loading') return Object.freeze({status: 'loading', generation: state.generation});
  if (state.status === 'error') {
    return Object.freeze({
      status: 'error',
      generation: state.generation,
      error: Object.freeze({code: state.error.code, field: state.error.field, kind: state.error.kind}),
    });
  }
  ...
  return Object.freeze({
    status: 'ready', kind: 'tileflow', generation: state.generation,
    map: state.map.name,
