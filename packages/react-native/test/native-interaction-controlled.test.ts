...
const annotation = {
	id: 'one', kind: 'marker', ariaLabel: 'Place', coordinate: [1, 2],
...
test('acknowledging a stale controlled close releases the previous binding identity', () => {
	const binding = {
		id: 'details', target: {kind: 'annotation', id: 'one'},
		popup: {content: {kind: 'text', text: 'Bound details'}},
	};
	const open: TileflowInteractionState = {popup: {kind: 'annotation', id: 'one'}};
	const owner = createNativeInteractionOwner({annotations: [annotation], interactions: [binding], interactionState: open});
