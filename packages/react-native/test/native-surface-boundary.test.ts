...
test('native surface adapters observe only their owned view and redact all native errors', async () => {
  for (const path of [
    ...
    assert.doesNotMatch(
      value,
      /\bLog\.|\bLogger\.|\bTimber\.|\bNSLog\s*\(|\bos_log\s*\(|registerClass|method_exchangeImplementations/u,
    );
    assert.doesNotMatch(
      value,
      /X-Tileflow-Native-Grant|X-Tileflow-Mobile-Client|tf_public_[0-9a-f]{48}/u,
    );
  }
  ...
  assert.match(
    ios,
    /\[camera handleImperativeStop:pitchStop\][\s\S]*\[camera handleImperativeStop:stop\]/u,
  );
  assert.match(
    ios,
    /\[self\.previous mapViewDidFinishRenderingFrame:mapView fullyRendered:fully frameEncodingTime:encoding frameRenderingTime:rendering\]/u,
  );
  assert.match(
    ios,
    /\[self\.previous mapViewDidFinishRenderingFrame:mapView fullyRendered:fully renderingStats:stats\]/u,
  );
...
test('iOS surface deadlines and retirement callbacks preserve typed cancellation and attachment identity', async () => {
  ...
  assert.match(
    ios,
    /if \(registered == attachment\) \[owner\.surfaces removeObjectForKey:identifier\];/u,
  );
  const retained = ios.match(/\[surface\.detached addObject:\^\{([\s\S]*?)\n\t\}\];/u);
  assert.ok(retained);
  assert.doesNotMatch(retained[1], /surface\.identifier/u);
});
