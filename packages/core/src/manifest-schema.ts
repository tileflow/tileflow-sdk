import type {ManifestRefinement, ManifestSchema, ManifestSchemaOperations} from './manifest-schema-operations';
import {
...
export function createTileflowRuntimeManifestSchema(
	...
	const safeTextSchema = operations.refine(
		operations.refine(
			...
			(value) => value === value.trim(), 'Expected text without surrounding whitespace',
		),
		(value) => !/[\p{Cc}\\]/u.test(value), 'Expected safe text',
	);
	const publicResourceUrlSchema = operations.refine(
		safeTextSchema, isPublicResourceUrl,
		'Expected an HTTP(S), root-relative, or path-relative public URL',
	);
	const apiOriginSchema = operations.refine(
		safeTextSchema, isApiOrigin,
	...
	const viewSchema = operations.object<TileflowViewConfig>({
		...
		center: operations.optional(operations.tuplePair(operations.number(-180, 180), operations.number(-90, 90))),
	...
	const fontFaceSchema = operations.object<TileflowStyleFontFace>({
		...
		weight: operations.optional(operations.enum(['100', '200', '300', '400', '500', '600', '700', '800', '900'])),
	});
	const fontFacesSchema = operations.superRefine(
		operations.maxLength(operations.array(fontFaceSchema), 16), validateUniqueFontFaces,
	);
	...
	const manifestMapSchema = operations.superRefine(operations.object<TileflowRuntimeManifestMapEntry>({
		...
		systemThemes: operations.optional(operations.object<{dark: string; light: string}>({
	...
	const canonicalManifestSchema = operations.superRefine(operations.object<TileflowRuntimeManifest>({
		apiUrl: operations.optional(apiOriginSchema),
