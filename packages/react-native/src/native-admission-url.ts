...
export const isNativeToken = (value: unknown): value is string =>
  typeof value === 'string' && /^[A-Za-z0-9_.-]{1,96}$/u.test(value) && !value.startsWith('tf_');
...
export function normalizeNativeResources(
  input: readonly NativeAdmissionResource[],
): readonly NativeAdmissionResource[] {
  if (!Array.isArray(input) || input.length > nativeAdmissionLimits.resources)
    throw new Error('Invalid native resource catalog');
  ...
  return Object.freeze(
    input.map((resource) => {
      if (
        ...
        resource.url.length > nativeAdmissionLimits.urlCharacters - 128 ||
...
export function authorityAllowsResource(
  authority: HostedNativeSessionAuthority,
...
export function discriminateNativeResourceForTest(url: string, context: string): string {
  if (!isNativeToken(context) || hasReservedNativeContext(url))
    throw new Error('Invalid native context');
  const result = `${url}${url.includes('?') ? '&' : '?'}${nativeContextParameter}=${context}`;
  if (result.length > nativeAdmissionLimits.urlCharacters)
    throw new Error('Invalid native context');
  return result;
}
