...
const renderMarker: MapMarkerRenderer<Place> = (context: MapMarkerRenderContext<Place>) => {
	...
	context.nativeMap;
	return <Text>{name}: {capacity}</Text>;
};
...
const explicit: ReactElement = <Map<Place> {...props}><View /></Map>;
const inferred = <Map source={source} annotations={annotations} renderMarker={({annotation}) => {
	const capacity: number = annotation.data.capacity;
	return <Text>{capacity}</Text>;
}} />;
const uncontrolled = <Map source={source} annotations={annotations} defaultInteractionState={state} />;
