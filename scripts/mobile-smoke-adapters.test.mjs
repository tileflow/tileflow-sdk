...
import {createMobileContext, withMobileWorkspace, writeMobileFile, writeMobileJson} from './mobile-smoke-workspace.mjs';

const output = (stdout = '') => ({stdout: Buffer.from(stdout), stderr: Buffer.alloc(0)});
const nativeConfig = {dependency: {platforms: {android: {
	sourceDir: 'android',
...
test('staged build and pack adapters use pnpm, fresh output and license inputs without touching source', async (t) => {
	...
	for (const file of ['package.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml', 'tsconfig.base.json', 'docs/modules-api-reference.json']) {
		await writeFixture(join(repository, file), '{}\n');
	}
	await writeFixture(join(repository, 'LICENSE'), 'Fixture license\n');
	await writeFixture(join(repository, 'scripts', 'package-license.mjs'), '// Fixture hook; not executed.\n');
	const baseFiles = ['package.json', 'LICENSE', 'dist/index.js', 'dist/index.d.ts'];
	const nativeFiles = ['react-native.config.cjs', 'android/build.gradle', 'android/src/main/java/Fixture.kt', 'ios/tileflow_native_admission.rb', 'ios/Fixture.mm', 'TileflowNativeAdmission.podspec'];
	for (const name of mobilePackages) {
		const manifest = {
			name: `@tileflow/${name}`, version: '0.0.0-development',
			exports: {'.': {types: './dist/index.d.ts', import: './dist/index.js'}},
			...(name === 'react-native' ? {dependencies: {'@tileflow/core': 'workspace:>=0.1.0-alpha.16 <0.1.0-beta.0', '@tileflow/interactions': 'workspace:*'}} : {}),
		};
		await writeFixture(join(repository, 'packages', name, 'package.json'), JSON.stringify(manifest));
		await writeFixture(join(repository, 'packages', name, 'dist', 'stale.js'), 'stale');
		if (name === 'react-native') for (const file of nativeFiles) await writeFixture(join(repository, 'packages', name, file), file === 'react-native.config.cjs' ? `module.exports = ${JSON.stringify(nativeConfig)};` : 'fixture');
	}
	const calls = [];
	await withMobileWorkspace(async (root) => {
		const archives = new Map();
		const context = await createMobileContext(root, {run: async (command, args, options) => {
			...
			if (command === 'pnpm' && args[0] === 'install') {
				...
				for (const name of mobilePackages) await assert.rejects(access(join(options.cwd, 'packages', name, 'dist')), {code: 'ENOENT'});
				return output();
			}
			if (command === 'pnpm' && args[0] === '--filter') {
				const name = args[1].slice('@tileflow/'.length);
				for (const file of ['index.js', 'index.d.ts']) await writeFixture(join(options.cwd, 'packages', name, 'dist', file), 'export {};');
				return output();
			}
			if (command === 'pnpm' && args[0] === 'pack') {
				...
				for (const file of files) contents.set(file, file === 'LICENSE' ? Buffer.from('Fixture license\n') : await readFile(join(options.cwd, file)));
				archives.set(path, contents);
				return output(JSON.stringify({name: manifest.name, version: manifest.version, filename: path}));
			}
			if (command === 'tar') {
				const contents = archives.get(args[1]);
				assert.ok(contents);
				if (args[0] === '-tzf') return output([...contents.keys()].map((file) => `package/${file}\n`).join(''));
				if (args[0] === '-tvzf') return output([...contents.keys()].map((file) => `-rw-r--r-- package/${file}\n`).join(''));
				assert.equal(args[0], '-xzf');
				for (const [file, bytes] of contents) await writeFixture(join(args[3], 'package', file), bytes);
				return output();
			}
		...
		assert.deepEqual(packed.tarballs.map(({name}) => name), mobilePackages.map((name) => `@tileflow/${name}`));
		assert.ok(packed.tarballs.every(({identity}) => /^[a-f0-9]{64}$/u.test(identity.sha256)));
		assert.equal(calls.filter(([command, operation]) => command === 'pnpm' && operation === 'pack').length, 3);
		assert.equal(calls.some(([command]) => command === 'npm'), false);
	}, {parent});
	for (const name of mobilePackages) {
		const source = JSON.parse(await readFile(join(repository, 'packages', name, 'package.json'), 'utf8'));
		assert.equal(source.version, '0.0.0-development');
		assert.equal(await readFile(join(repository, 'packages', name, 'dist', 'stale.js'), 'utf8'), 'stale');
...
for (const kind of ['expo', 'bare']) {
	test(`${kind} Android adapter checks Release APK metadata and embedded Hermes`, async (t) => {
		...
		await withMobileWorkspace(async (root) => {
			...
			await writeMobileFile(root, join(android, 'gradle', 'wrapper', 'gradle-wrapper.properties'), 'distributionUrl=https\\://services.gradle.org/distributions/gradle-9.0.0-bin.zip\n');
			let bundle = bytecode();
			const context = await createMobileContext(root, {run: async (command, args) => {
				if (command === 'unzip') {
				...
				await writeMobileJson(root, join(directory, 'output-metadata.json'), {variantName: 'release', elements: [{outputFile: 'app-release.apk'}]});
				await writeMobileFile(root, join(directory, 'app-release.apk'), 'Mock binary, not a native build.');
				return output();
			}});
	...
	test(`${kind} iOS adapter discovers the app from build settings and rejects non-Hermes output`, async (t) => {
		const parent = await fixtureParent(t);
		await withMobileWorkspace(async (root) => {
			const consumer = {kind, directory: join(root, 'consumers', kind), scheme: kind === 'expo' ? 'HelloWorld' : 'TileflowBareSmoke'};
			...
			const context = await createMobileContext(root, {run: async (command, args) => {
				if (command === 'pod') {
					...
					await writeMobileFile(root, join(consumer.directory, 'ios', 'Podfile.lock'), 'TileflowNativeAdmission (0.1.0-alpha.16)\nMapLibreReactNative (11.3.10)\n');
					return output();
				}
				assert.equal(command, 'xcodebuild');
				assert.ok(args.includes('CODE_SIGNING_ALLOWED=NO'));
				if (args.includes('-showBuildSettings')) return output(JSON.stringify([{target: consumer.scheme, buildSettings: {WRAPPER_EXTENSION: 'app', CONFIGURATION: 'Release', PLATFORM_NAME: 'iphonesimulator', TARGET_BUILD_DIR: productDirectory, WRAPPER_NAME: wrapper, EXECUTABLE_NAME: consumer.scheme}}]));
				await writeMobileFile(root, join(productDirectory, wrapper, consumer.scheme), 'Mock executable, not a native build.');
