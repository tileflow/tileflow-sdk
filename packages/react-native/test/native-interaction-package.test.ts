...
test('the interaction foundation imports only portable root contracts and its private neutral modules', async () => {
	for (const name of modules) {
		...
		for (const specifier of imports(source)) {
			assert.ok(specifier === '@tileflow/interactions' || modules.some((value) => specifier === `./${value}`), specifier);
		}
...
test('mounted public types do not export native handles, private ports or selection presentation', async () => {
	...
	assert.equal(manifest.dependencies['@tileflow/interactions'], 'workspace:>=0.1.0-alpha.16 <0.1.0-beta.0');
	...
	assert.doesNotMatch(output, /react-native|maplibre-gl|@tileflow\/interactions\/maplibre|\bwindow\.|\bdocument\./u);
	...
	assert.match(declarations, /MapMarkerRenderer/u);
	assert.match(declarations, /MapMarkerRenderContext/u);
	assert.match(declarations, /onInteractionStateChange/u);
	assert.match(declarations, /defaultInteractionState/u);
	assert.match(declarations, /annotations\??:/u);
	assert.match(declarations, /TAnnotation extends TileflowAnnotation/u);
	...
	assert.doesNotMatch(index, /from '\.\/(?:native-|mounted-|session-)|\b(?:Popup|Callout|Tooltip|Sheet|Panel|Modal)\b/u);
...
test('mounted code composes only markers and native children, never a selected-content surface', async () => {
	...
	for (const source of [map, marker]) {
		assert.doesNotMatch(source, /renderPopup|renderTooltip|renderSelection|ViewAnnotation|createPortal|<[^>]*(?:Popup|Callout|Tooltip|Sheet|Panel|Modal)\b/u);
