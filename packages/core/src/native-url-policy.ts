...
/** Only the read-only fields used by Tileflow's URL policy. */
export type NativeParsedUrl = Readonly<{
  href: string;
  origin: string;
  protocol: string;
  hostname: string;
...
/** Internal composition seam; applications do not provide or configure the parser. */
export function createNativeUrlPolicy(parse: (value: string, base?: string) => NativeParsedUrl) {
  ...
  function resolveTileflowNativeManifestUrl(
    value: unknown,
    options: TileflowNativeNetworkOptions = {},
  ): string {
    assertOptionsObject(options, 'developmentOrigin');
   ...
   */
  function resolveTileflowNativeResourceUrl(
    value: unknown,
    options: TileflowNativeResourceUrlOptions,
  ): string {
    assertOptionsObject(options, 'documentUrl');
    const developmentOrigin = resolveDevelopmentOrigin(options.developmentOrigin);
    ...
    let attempt = 0;
    while (value.includes(prefix) || document.href.includes(prefix)) {
      prefix = `__tileflow_native_template_${attempt++}_`;
    }
  ...
  function assertOptionsObject(value: unknown, field: 'documentUrl' | 'developmentOrigin'): void {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
      throw new TileflowNativeUrlError(
        field === 'documentUrl' ? 'NATIVE_URL_INVALID' : 'NATIVE_URL_DEVELOPMENT_ORIGIN_INVALID',
        field,
      );
    }
  }
  ...
  function parseUrl(value: string, field: TileflowNativeUrlField, base?: string): NativeParsedUrl {
    let url: NativeParsedUrl;
    try {
  ...
  return {resolveTileflowNativeManifestUrl, resolveTileflowNativeResourceUrl};
}
