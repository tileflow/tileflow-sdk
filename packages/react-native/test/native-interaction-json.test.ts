...
test('sparse equality includes length and exact occupied indices without treating holes as null', () => {
  ...
  assert.equal(
    nativeInteractionValuesEqual({nested: left}, {nested: snapshotNativeInteractionJson(left)}),
    true,
  );
  ...
  const same = prepareNativeInteractionInputs(
    {annotations: [{...annotation, data: {nested: snapshotNativeInteractionJson(left)}}]},
    previous,
  );
  ...
  assert.deepEqual(
    planNativeAnnotations(same.annotations, changed.annotations).update.map((entry) => entry.id),
    ['one'],
  );
});
...
test('sparse copying preserves the aggregate property byte node and depth limits', () => {
  ...
  for (let index = 0; index < tileflowInteractionLimits.maxDocumentDepth; index++)
    deep = {child: deep};
  const invalid = [
...
test('oversized sparse arrays fail from their length before keys are enumerated', () => {
  ...
  assert.throws(() => snapshotNativeInteractionJson(data));
  const result = prepareNativeInteractionInputs({
    annotations: [{id: 'one', kind: 'marker', ariaLabel: 'Place', coordinate: [0, 0], data}],
  });
...
test('the descriptor copy rejects array keys introduced after the first portable audit', () => {
  for (const key of ['extra', '01', Symbol('extra')]) {
    ...
    const data = new Proxy([null], {
      ownKeys(target) {
        if (++enumerations === 2)
          Object.defineProperty(target, key, {value: null, enumerable: true});
        return Reflect.ownKeys(target);
      },
    });
    assert.throws(() => snapshotNativeInteractionJson(data));
