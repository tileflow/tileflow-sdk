...
test('hostile documents fail without invoking accessors or reflecting their contents', () => {
  ...
  const hostile = Object.defineProperty(marker(), 'data', {
    ...
    get() {
      reads++;
      throw new Error('https://private.example.test/raw');
    },
  });
  const cycle: unknown[] = [];
  cycle.push(cycle);
...
test('annotation plans retain compatible hosts, update by ID, and remove deterministically', () => {
  const previous = prepareNativeInteractionInputs({
    annotations: [marker('one'), marker('two'), marker('gone')],
  });
      ...
      annotations: [
        {
          ...
          data: {name: 'Updated'},
          coordinate: [4, 5],
          popup: {content: {kind: 'view', name: 'details'}},
        },
        marker('one'),
        marker('new'),
      ],
  ...
  assert.deepEqual(
    plan.update.map((value) => value.id),
    ['two'],
  );
  assert.deepEqual(
    plan.create.map((value) => value.id),
    ['new'],
  );
  ...
  const rejected = prepareNativeInteractionInputs(
    {annotations: [marker('one'), marker('one')]},
    next,
  );
...
test('the portable annotation count limit is exact and data key order does not recreate or update a host', () => {
  const annotations = Array.from(
    {length: tileflowInteractionLimits.maxAnnotations},
    (_, index) => ({
      id: `p${index}`,
      kind: 'marker',
      ariaLabel: 'P',
  ...
  const tooMany = prepareNativeInteractionInputs(
    {annotations: [...annotations, marker('overflow')]},
    full,
  );
  ...
  const reordered = prepareNativeInteractionInputs(
    {annotations: [{...marker(), data: {b: 2, a: 1}}]},
    one,
  );
