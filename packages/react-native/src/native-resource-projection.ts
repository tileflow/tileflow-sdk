...
function owned(
	...
	if (scope === 'style') {
		...
		if (
			...
			!/^[a-z0-9][a-z0-9-]{0,63}\.json$/u.test(path.slice(prefix.length))
		)
			throw invalid();
...
function suffix(url: string, extension: string): string {
	...
	return query < 0
		? `${url}${extension}`
		: `${url.slice(0, query)}${extension}${url.slice(query)}`;
}
...
function fontMetadata(input: readonly Record<string, unknown>[] | undefined): readonly FontMetadata[] {
