...
export function runMobileCommand(command, args, options = {}) {
  ...
  if (options.signal?.aborted)
    return Promise.reject(new MobileSmokeError('COMMAND_CANCELLED', options.step));
  return new Promise((resolveRun, rejectRun) => {
    ...
    const stop = (code) => {
      ...
      escalation = setTimeout(() => {
        try {
          ...
          /* close remains the cleanup barrier. */
        }
    ...
    child.once('error', (error) => {
      failure ??= new MobileSmokeError(
        error.code === 'ENOENT' ? 'PREREQUISITE_MISSING' : 'COMMAND_FAILED',
        options.step,
      );
