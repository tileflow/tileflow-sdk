...
test('single-layer filters reconstruct exact provenance and preserve layer and hit order', async () => {
  ...
  const port = createNativeInteractionQuery(
    style,
    () => true,
    async (point, options) => {
  ...
  const result = (await port.lease.query(request).result) as {
    request: unknown;
    features: unknown[];
  };
  assert.equal(result.request, request);
  assert.deepEqual(
    calls.map((call) => call.layers),
    [['food-label'], ['food-icon'], ['hotel']],
  );
  assert.ok(calls.every((call) => call.point[0] === 10 && call.point[1] === 20));
  assert.deepEqual(result.features, [
    {
      id: 7,
      properties: {name: 'Place 7'},
      layer: {id: 'food-label'},
      source: 'world',
      sourceLayer: 'poi',
    },
...
test('undeclared, duplicate, wrong-order and mismatched physical layers never reach native queries', async () => {
  ...
  const invalid = createNativeInteractionQuery(
    wrong,
    () => true,
    async () => {
      calls++;
  ...
  await assert.rejects(
    invalid.lease.query({point: touch.point, layers: ['food-label'], limit: 512}).result,
  );
...
test('retiring one map does not cancel another map or share its physical attribution', async () => {
  const left = createNativeInteractionQuery(
    finalizedStyle(),
    () => true,
    async () => [hit(1)],
  );
  const right = createNativeInteractionQuery(
    finalizedStyle(),
    () => true,
    async () => [hit(2)],
  );
