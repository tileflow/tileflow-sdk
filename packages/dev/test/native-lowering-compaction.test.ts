...
function nodes(value: unknown): number {
  ...
  return (
    1 + Object.entries(value).reduce((total, [key, child]) => total + nodes(key) + nodes(child), 0)
  );
}
...
function evaluate(value: any, properties: Record<string, unknown>): any {
  ...
  switch (value[0]) {
    ...
    case 'all':
      return args.every((child) => evaluate(child, properties));
    case 'any':
      return args.some((child) => evaluate(child, properties));
    case 'case': {
...
test('copying a sort key does not preserve feature draw order across simultaneous branches', () => {
  ...
  const lowered = lowerNativeStyleRepresentation(original);
  const features = [
    {id: 'low', class: 'minor', priority: 0},
    {id: 'high', class: 'primary', priority: 1},
  ];
  ...
  const after = lowered.style.layers.flatMap((layer) => {
    ...
    return features
      .filter((feature) => active([layer], feature).length)
...
test('does not turn a flat finite case into a predicate exceeding the style depth budget', () => {
  ...
  const depth = (value: unknown): number =>
    !value || typeof value !== 'object' ? 0 : 1 + Math.max(0, ...Object.values(value).map(depth));
