...
function setup() {
	const commands: Array<{command: CameraCommand; done: ReturnType<typeof deferred>; cancels: number}> = [];
	const errors: CameraControllerError[] = [];
	const port: CameraPort = {
		apply(command) {
			const request = {command, done: deferred(), cancels: 0};
			commands.push(request);
			return {finished: request.done.promise, cancel() { request.cancels++; }};
		},
	};
	const camera = createMapCameraController(port, (error) => { errors.push(error); });
...
test('mount composes a partial seed once; later seed changes do not reset a live instance', async () => {
	...
	t.camera.mount({initialView: {zoom: 8}}, {
		mapOptionsView: {pitch: 20}, manifestView: {center: [10, 30], zoom: 5, bearing: -10},
	});
	assert.deepEqual(t.commands[0]!.command.view, {center: [10, 30], zoom: 8, bearing: -10, pitch: 20});
	await t.finish();
...
test('an unadopted gesture returns once to the latest received controlled view', async () => {
	...
	await t.finish();
	assert.deepEqual(events.map((event) => event.view.zoom), [8, 9]);
	assert.equal(t.commands.length, 2);
...
test('uncontrolled change observation is optional and never grants the callback camera ownership', async () => {
	for (const observed of [false, true]) {
		const t = setup();
		const events: MapViewChangeEvent[] = [];
		t.camera.mount({onViewChange: observed ? (event) => { events.push(event); } : undefined});
		await t.finish();
		const gesture = t.camera.startGesture()!;
...
test('programmatic observations never echo, and gesture start retires an unfinished command', async () => {
	const t = setup();
	const events: MapViewChangeEvent[] = [];
	t.camera.mount({view: view(), onViewChange: (event) => { events.push(event); }});
...
for (const order of [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]]) {
	test(`obsolete command completions cannot replace current view in order ${order}`, async () => {
		...
		assert.deepEqual(t.commands.map((command) => command.cancels), [1, 1, 0]);
...
test('restoration reaffirms live uncontrolled view or controlled authority without resetting the seed', async () => {
	for (const controlled of [false, true]) {
		const t = setup();
		const props: MapCameraProps = controlled ? {view: view(), onViewChange: ignore} : {initialView: {zoom: 2}};
...
test('invalid replacements preserve the last mode, view and observer', async () => {
	...
	assert.throws(() => initial.camera.update({view: view(), onViewChange: ignore}), {code: 'CAMERA_MODE_CHANGE'});
});
	...
	const camera = createMapCameraController({
		apply() {
			if (++calls === 1) {
				...
				return {finished: old.promise, cancel() { oldCancel++; }};
			}
