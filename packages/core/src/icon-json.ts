...
export function parseTileflowIconJson(text: string, maximumBytes: number): unknown {
	...
	let position = 0;
	const fail = (): never => {
		throw new TileflowIconSetError('ICON_LOCK_INVALID', `Invalid or duplicate-key icon JSON at offset ${position}`);
	};
	const whitespace = () => {
	...
	const value = (depth: number): unknown => {
		...
		for (const [literal, parsed] of [['true', true], ['false', false], ['null', null]] as const) {
			if (text.startsWith(literal, position)) { position += literal.length; return parsed; }
		}
