...
import {
  ...
  type NativeInteractionQuery,
} from './native-interaction-query';
...
export function createMountedMapInteractions(
  ...
  const invoke = (work: () => unknown) => {
    ...
    if (initialNotifications) {
      initialNotifications.push(work);
      return;
    }
    try {
      void Promise.resolve(work()).catch(() => undefined);
    } catch {
      /* Observer errors cannot alter native ownership. */
    }
  };
  const notify = () => {
    if (!disposed) invoke(changed);
  };
  ...
  const sync = () => {
    ...
    try {
      ...
      if (
        next &&
        (!mounted || !next.isCurrent() || next.key !== mounted.key || next.style !== mounted.style)
      )
        next = undefined;
    } catch {
      next = undefined;
