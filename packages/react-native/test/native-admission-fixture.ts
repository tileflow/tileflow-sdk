...
export class AdmissionBridgeDouble implements NativeAdmissionBridge {
	...
	readonly registrations = new Map<string, {mapId: string | null; resources: readonly NativeAdmissionResource[]}>();
	...
	sequence = 0;
	batchSequence = 0;
	private pending = new Map<string, {context: string; resolve: (results: readonly NativeAdmissionResult[]) => void}>();
	...
	async registerContext(_installation: string, registration: {mapId: string | null; resources: readonly NativeAdmissionResource[]}): Promise<NativeContextAck> {
	...
	async completeBatch(_installation: string, context: string, _generation: number, batch: string, results: readonly NativeAdmissionResult[]) {
	...
	async remove(installation: string) {
		...
		for (const context of [...this.registrations.keys()]) await this.retireContext(installation, context);
	...
	batch(context: string, urls: readonly string[], generation = 1) {
		...
		this.listener({kind: 'batch', installation: this.installation, context, generation, batch, tickets});
