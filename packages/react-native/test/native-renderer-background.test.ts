...
function fixture() {
	...
	const emit = (kind: NativeSurfaceEvent['kind'], fields: Record<string, unknown> = {}) =>
		listener({surface: 'surface', style: owner.token, sequence: ++sequence, layout: 2, kind, ...fields} as NativeSurfaceEvent);
...
test('a current style accepted while backgrounded becomes usable only after resume and a new native barrier', async () => {
	...
	assert.equal(f.events.some((event) => event.type === 'load'), false);
	...
	f.emit('render');
	assert.equal(f.events.some((event) => event.type === 'readiness-change' && event.status === 'ready'), true);
...
test('background and layout loss retire the active gesture before controlled reconciliation resumes', async () => {
	...
	const owner = createNativeRendererOwner('renderer', {source, style: {version: 8, sources: {}, layers: []}},
		{view: initial, onViewChange: (event) => observed.push(event)}, {
			surfaces: {async attach(_root, notify) {
				const surface: NativeSurface = {id: 'surface', async expectStyle() {}, async commitLayout() { return 2; }, async requestFrame() {},
					async applyCamera(command, view) { return {command, view}; }, async cancelCamera() { return {cancelled: true}; }, async retire() {}};
				listener = notify; return surface;
			}, async retireRoot() {}}, emit() {}, changed() {},
		});
	let listener!: (event: NativeSurfaceEvent) => void;
	let sequence = 0;
	await owner.attach(1); await owner.whenIdle();
	listener({surface: 'surface', style: owner.token, sequence: ++sequence, layout: 2, kind: 'style'});
	await owner.whenIdle();
	const moved = {...initial, zoom: 7};
	listener({surface: 'surface', style: owner.token, sequence: ++sequence, layout: 2, kind: 'gesture-start', gesture: 1, view: initial});
	listener({surface: 'surface', style: owner.token, sequence: ++sequence, layout: 2, kind: 'gesture-change', gesture: 1, view: moved});
	owner.layoutChanged();
	owner.background();
