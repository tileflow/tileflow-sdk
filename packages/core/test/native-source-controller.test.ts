...
function controlled() {
	...
	const acquire: TileflowNativeManifestAcquire = () => {
		...
		return {response: request.response.promise, cancel() { request.cancelled++; }};
	};
...
test('publishes immutable loading/ready snapshots and canonical manifest-driven selection', async () => {
	...
	assert.deepEqual(states.map(({status, generation}) => [status, generation]), [['loading', 1], ['ready', 1]]);
	...
	assert.equal(c.state.theme.styleUrl, 'https://maps.example.test/native/styles/streets/light.json');
...
test('default, concrete and injected system selection share the runtime precedence', async () => {
	for (const [options, theme] of [
		[{}, 'light'], [{colorScheme: 'dark'}, 'light'], [{theme: 'dark'}, 'dark'],
		[{theme: 'light', colorScheme: 'dark'}, 'light'], [{theme: 'system', colorScheme: 'dark'}, 'dark'],
	] as const) {
		const t = transport();
		const c = createTileflowNativeSourceController({acquire: t.acquire});
...
for (const order of [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]]) {
	test(`only the latest generation commits in response order ${order}`, async () => {
		...
		const completions = [controller.replace(source), controller.replace(source, {theme: 'dark'}), controller.replace(source)];
		assert.equal(requests.length, 3);
		assert.deepEqual(requests.map(({cancelled}) => cancelled), [1, 1, 0]);
		...
		assert.deepEqual(states.map(({status, generation}) => [status, generation]), [
			['loading', 1], ['loading', 2], ['loading', 3], ['ready', 3],
		]);
...
test('a retired rejection and abort cannot overwrite a newer ready state', async () => {
	...
	await old;
	assert.deepEqual(states.map(({status, generation}) => [status, generation]), [['loading', 1], ['loading', 2], ['ready', 2]]);
});
...
test('invalid replacement retires pending work and cannot re-publish previous data', async () => {
	...
	assert.deepEqual(states.map(({status, generation}) => [status, generation]), [['loading', 1], ['loading', 2], ['error', 2]]);
	const t = transport();
	const c = createTileflowNativeSourceController({acquire: t.acquire});
	await c.replace(source);
...
test('dispose is idempotent and suppresses late callbacks and replacements', async () => {
	...
	assert.deepEqual(states.map(({status}) => status), ['loading']);
...
test('snapshots inputs before async work and ignores observer failures/reentrant stale notifications', async () => {
	...
	controller.subscribe(() => { throw new Error('observer-only'); });
	const pending = controller.replace(input, options);
	input.map = 'changed'; options.theme = 'changed';
	requests[0]!.response.resolve(transport().response);
	await pending;
	...
	next.controller.subscribe((state) => { calls.push(state.generation); });
