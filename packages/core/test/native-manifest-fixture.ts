		...
		maps: {
			streets: {
				...
				themes: {
					...
					light: {colorScheme: 'light', styleUrl: './styles/streets/light.json', revision: 'light-v1'},
				},
			},
		},
	};
...
export function transport(input = bytes(), url = manifestUrl, status = 200) {
	...
	const response: TileflowNativeManifestResponse = {
		...
		reader: {
			...
			cancel() { readerCancels++; },
		},
	};
	const acquire: TileflowNativeManifestAcquire = (url, options) => {
		calls++; requests.push({url, maximumBytes: options.maximumBytes});
		return {response: Promise.resolve(response), cancel() { operationCancels++; }};
	};
	return {acquire, response, reads, requests,
		get calls() { return calls; },
		get operationCancels() { return operationCancels; },
		get readerCancels() { return readerCancels; },
	};
}
