...
import {
  ...
  tileflowIconsLockfileMaximumBytes,
...
export async function readTileflowIconsLockfile(
  ...
  if (text === null)
    throw new TileflowIconSetError(
      'ICON_LOCK_MISSING',
      'Missing tileflow.icons.lock.json; install exact icon set revisions explicitly',
    );
...
export async function writeTileflowIconsLockfile(
  baseDirectory: string,
  ...
  try {
    ...
    throw new TileflowIconSetError(
      'ICON_LOCK_CONFLICT',
      'Another icon lock writer is active; a stale .writing guard may be removed only after confirming its writer has exited',
      {cause},
    );
  }
  ...
  try {
    ...
    if (actual !== expectedContents)
      throw new TileflowIconSetError(
        'ICON_LOCK_CONFLICT',
        'Icon lock changed since it was read; reload and retry without discarding other pins',
      );
    ...
    if ((await readLockText(destination)) !== actual)
      throw new TileflowIconSetError(
        'ICON_LOCK_CONFLICT',
        'Icon lock changed while preparing an update',
      );
...
async function readLockText(path: string): Promise<string | null> {
  try {
    ...
    if (!entry.isFile() || entry.isSymbolicLink())
      throw new TileflowIconSetError(
        'ICON_LOCK_INVALID',
        'The icon lock must be a regular file, not a symlink',
      );
    return new TextDecoder('utf-8', {fatal: true}).decode(
      await readSafeFile(path, tileflowIconsLockfileMaximumBytes, false),
    );
  } catch (cause) {
