...
test('retirement callbacks cannot dispatch a stale command after a newer prop update', async () => {
	...
	assert.deepEqual(commands.map((command) => command.view.zoom), [2, 9]);
...
test('a malformed command completion cancels its known operation and emits only a safe diagnostic', () => {
	...
	const camera = createMapCameraController({apply() {
		return {finished: undefined as never, cancel() { cancelled++; }};
...
test('throwing command/cleanup/owner callbacks cannot leak their cause or prevent disposal', () => {
	let cancelled = 0;
	const errors: CameraControllerError[] = [];
	const camera = createMapCameraController({apply() { throw new Error('Renderer secret'); }}, (error) => {
		errors.push(error);
	...
	camera.dispose();
	const next = createMapCameraController({apply() {
		return {finished: new Promise<void>(() => undefined), cancel() { cancelled++; throw new Error('Cleanup secret'); }};
	}});
	next.mount({});
	assert.doesNotThrow(() => { next.dispose(); next.dispose(); });
	assert.equal(cancelled, 1);
});
...
test('reentrant input reflection cannot commit an older prop snapshot over a newer update', () => {
	...
	camera.mount({view: view(2), onViewChange: ignore});
	let replaced = false;
	const input = new Proxy({view: view(4), onViewChange: ignore}, {
		getPrototypeOf(target) {
			if (!replaced) { replaced = true; camera.update({view: view(7), onViewChange: ignore}); }
			return Reflect.getPrototypeOf(target);
		},
	});
	camera.update(input);
	assert.deepEqual(commands.map((command) => command.view.zoom), [2, 7]);
});
