...
test('the exact versioned style route admits safe positive versions and no aliases', () => {
  for (const version of [1, 7, Number.MAX_SAFE_INTEGER]) {
    assert.equal(
      nativeVersionedStyleMatchesMap(
        `${apiOrigin}/maps/${mapId}/native/v${version}/light.json`,
        mapId,
      ),
      true,
    );
  ...
  for (const invalid of [
    url.replace('/v7/', '/v0/'),
    url.replace('/v7/', '/v07/'),
    url.replace('/v7/', '/v9007199254740992/'),
    url.replace('/v7/', '/v-1/'),
    url.replace('/native/', '/%6eative/'),
    url.replace('light.json', '%6cight.json'),
    `${url}?map=${mapId}`,
    `${url}#fragment`,
...
test('projection acknowledges the exact versioned style before reading protected bytes', async () => {
  ...
  const result = await projectNativeResources({
    ...
    policy,
    current: () => true,
    discriminate: (value) => value,
    async accept(resources) {
  ...
  for (const invalid of [url.replace(mapId, 'map_abcdefghijklmnop'), `${url}?alias=1`]) {
    await assert.rejects(
      projectNativeResources({
        ...
        discriminate: (value) => value,
        async accept() {
          assert.fail('Invalid style cannot enter the catalog.');
        },
        async read() {
          assert.fail('Invalid style cannot acquire authority.');
        },
      }),
    );
  }
