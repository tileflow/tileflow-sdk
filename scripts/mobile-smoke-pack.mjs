...
import {assertOwnedPath, copyMobileInputs, mobileFileIdentity, readMobileFile, readMobileJson, writeMobileJson} from './mobile-smoke-workspace.mjs';

export function parseCommandJson(result) {
	try { return JSON.parse(result.stdout.toString('utf8')); }
	catch { requireMobile(false, 'COMMAND_FAILED'); }
}
export async function extractMobileTarball(context, path, destination) {
	...
	const names = await context.run('tar', ['-tzf', path], {step: 'pack', maxOutputBytes: 8 * 1024 * 1024});
	const types = await context.run('tar', ['-tvzf', path], {step: 'pack', maxOutputBytes: 8 * 1024 * 1024});
	const files = assertArchiveListing(names.stdout.toString(), types.stdout.toString());
	await mkdir(destination, {recursive: false});
	await context.run('tar', ['-xzf', path, '-C', destination, '--no-same-owner', '--no-same-permissions'], {step: 'pack'});
	for (const file of files) await readMobileFile(context.root, join(destination, 'package', file), 128 * 1024 * 1024);
...
export async function packMobilePackages(context, repository, pnpm) {
	...
	for (const directory of mobilePackages) {
		await context.run(pnpm.command, [...pnpm.prefix, '--filter', `@tileflow/${directory}`, 'run', 'build'], {cwd: stage, step: 'stage'});
	}
	for (const directory of mobilePackages) {
		const path = join(stage, 'packages', directory, 'package.json');
		await writeMobileJson(context.root, path, stageMobileManifest(await readMobileJson(context.root, path)));
	}
	...
	for (const directory of mobilePackages) {
		...
		const result = parseCommandJson(await context.run(pnpm.command, [...pnpm.prefix, ...pnpmPackArguments(packs)], {cwd, step: 'pack'}));
		requireMobile(result.name === `@tileflow/${directory}` && result.version === mobileVersions.package && typeof result.filename === 'string', 'PACK_INVALID', 'pack');
		const path = resolve(cwd, result.filename);
		requireMobile(path.endsWith('.tgz') && resolve(path, '..') === packs, 'PACK_INVALID', 'pack');
		const extracted = await extractMobileTarball(context, path, join(context.root, `unpacked-${directory}`));
		...
		if (directory === 'react-native') {
			const config = parseCommandJson(await context.run(process.execPath, ['-e', 'process.stdout.write(JSON.stringify(require(process.argv[1])))', join(extracted.directory, 'react-native.config.cjs')], {step: 'pack'}));
			assertPackedAndroidConfig(config);
		}
		tarballs.push({name: manifest.name, version: manifest.version, path, ...extracted, identity: await mobileFileIdentity(context.root, path, `${directory}.tgz`)});
	}
...
export async function packMobileTemplate(context, name, version, directory) {
	...
	const result = parseCommandJson(await context.run('npm', ['pack', `${name}@${version}`, '--ignore-scripts', '--pack-destination', packs, '--json'], {step: 'templates'}));
	requireMobile(result.length === 1 && result[0].name === name && result[0].version === version && /^[a-z0-9._-]+\.tgz$/u.test(result[0].filename), 'TEMPLATE_INVALID', 'templates');
	const path = join(packs, result[0].filename);
	const extracted = await extractMobileTarball(context, path, join(context.root, `template-${directory}`));
	return {name, version, path, ...extracted, identity: await mobileFileIdentity(context.root, path, `${directory}-template.tgz`)};
}
